...
struct ListGraph {
	...
	inline static constexpr int INF = std::numeric_limits<int>::max() / 2;

	private:
	std::size_t _size;

	public:
	std::vector<std::vector<Edge<W>>> edges;

	ListGraph(std::size_t size) : _size(size), edges(size) {}

	std::size_t size() const { return this->_size; }

	void add_edge(std::size_t from, std::size_t to, W weight) {
	...
	W weight(std::size_t from, std::size_t to) const {
		W weight = this->INF;
		for(const auto& e : this->edges[from]) {
			if(e.to == to) {
	...
	std::vector<Edge<W>> neighbors(std::size_t n) const {
...
struct MatrixGraph {
	...
	MatrixGraph(std::size_t size)
		: _size(size), edges(size, std::vector(size, false)),
		  weights(size, std::vector(size, static_cast<W>(0))) {}
	...
	void add_edge(std::size_t from, std::size_t to) {
		this->edges[from][to] = true;
		this->weights[from][to] = static_cast<W>(1);
	}

	void remove_edge(std::size_t from, std::size_t to) {
		this->edges[from][to] = false;
		this->weights[from][to] = static_cast<W>(0);
	}
