...
(deftest function-helpers
  ...
  (let ((f (utility.base:pa #'list :$0 :$*)))
    (ok (equal '(1 (2 3)) (funcall f 1 2 3))))
  (ok (equal '(2 4 6)
             (funcall (utility.base:pa #'mapcar
                                       #'(lambda (x) (* (1+ x) 2))
                                       :$0)
                      '(0 1 2))))
  (let ((x 10))
    (let ((f (utility.base:pa* #'+ x :$0)))
      ...
      (ok (= 13 (funcall f 3)))))
  (let ((x 10))
    (let ((f (utility.base:pa* #'list x :$*)))
      (setf x 100)
      (ok (equal '(10 (2 3)) (funcall f 2 3)))))
  (let ((x 10))
    (let ((f (utility.base:pa* #'mapcar
                               #'(lambda (y) (+ x y))
                               :$0)))
      (setf x 100)
      (ok (equal '(101 102 103)
                 (funcall f '(1 2 3))))))
  (ok (equal '(2 4 6)
             (mapcar (utility.base:fn (* (1+ :$0) 2))
                     '(0 1 2))))
  (ok (= 8
         (funcall (utility.base:fn (expt :$0 :$1))
                  2 3)))
  (ok (= 8
         (funcall (utility.base:fn (funcall #'(lambda (x y) (expt x y))
                                            :$0
                                            :$1))
                  2 3)))
  (ok (equal '(1 (2 3 4))
             (funcall (utility.base:fn (list :$0 :$*))
                      1 2 3 4)))
  (let ((x 10))
    (let ((f (utility.base:fn* (+ x :$0))))
      (setf x 100)
      (ok (= 13 (funcall f 3)))))
  (let ((x 10))
    (let ((f (utility.base:fn* (funcall #'(lambda (y) (+ x y :$0))
                                        1))))
      (setf x 100)
      (ok (= 14 (funcall f 3)))))
  (let ((x 10))
    (let ((f (utility.base:fn* (list x :$*))))
      (setf x 100)
      (ok (equal '(10 (2 3)) (funcall f 2 3))))))
