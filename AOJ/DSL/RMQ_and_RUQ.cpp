...
class LazySegmentTree {
	...
	void debug_print() {
		...
		REP(i, this->size * 2 - 1) {
			...
			cout << "(" << this->lazy[i].first << "," << this->lazy[i].second
				 << ") ";
		}
		cout << endl;
	}
...
int main() {
	int n, q;
	cin >> n >> q;
	VLL a(n, numeric_limits<int>::max());

	LazySegmentTree<ll, pair<ll, bool>> lst(
		a,
		[](ll a, ll b) { return min(a, b); },
		static_cast<ll>(numeric_limits<int>::max()),
		[](ll a, pair<ll, bool> b) { return b.second ? b.first : a; },
		[](pair<ll, bool> a, pair<ll, bool> b) { return b.second ? b : a; },
		make_pair(0LL, false));
	VLL results;
	REP(i, q) {
		int t;
		...
		if(t == 0) {
			...
			cin >> s >> t >> x;
			t++;
			lst.update(s, t, make_pair(x, true));
		} else {
			int s, t;
			cin >> s >> t;
			t++;
			results.push_back(lst.find(s, t));
		}
		// lst.debug_print();
	}
