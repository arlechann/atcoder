...
int main() {
	...
	LazySegmentTree<ll, pair<ll, int>> lst(
		a,
		[](ll a, ll b) { return a + b; },
		0LL,
		[](ll a, pair<ll, int> b) { return a + b.first; },
		[](pair<ll, int> a, pair<ll, int> b) {
			return make_pair(a.first + b.first * a.second, a.second);
		},
		make_pair(0LL, 1));
	VLL results;
	REP(i, q) {
		...
		if(t == 0) {
			...
			cin >> s >> t >> x;
			s--;
			lst.update(s, t, make_pair(x, 0));
		} else {
			int index;
			cin >> index;
			results.push_back(lst.find(index - 1, index));
		}
		// lst.debug_print();
	}
