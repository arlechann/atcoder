...
class UnionFind {
	...
	std::vector<int> group_left_edge;
	std::vector<int> node_left_edge;

	public:
	UnionFind(size_t size, const vector<int>& d)
		: parents(size), rank(size, 0), tree_size(size, 1), group_left_edge(d),
		  node_left_edge(d) {
	...
	bool merge(size_t a, size_t b) {
		...
		this->node_left_edge[ar]--;
		this->node_left_edge[br]--;
		this->group_left_edge[ar] += this->group_left_edge[br];
		this->group_left_edge[ar] -= 2;
	...
	int group_left(size_t n) { return this->group_left_edge[this->root(n)]; }
	int node_left(size_t n) { return this->node_left_edge[this->root(n)]; }
