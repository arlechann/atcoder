...
int main() {
	...
	vector<vector<Edge>> edges(n, vector<Edge>(n, {false, 0}));
	REP(i, m) {
		edges[a[i] - 1][b[i] - 1] = Edge(true, c[i]);
		edges[b[i] - 1][a[i] - 1] = Edge(true, c[i]);
	}
	auto distances = warshall_floyd(edges);

	VI towns(R);
	REP(i, R) { towns[i] = r[i] - 1; }
	sort(ALL(towns));

	ll shortest = INF;
	call_with_all_permutations(towns, [&](VI& t) {
		ll d = 0;
		REP(i, R - 1) { d += distances[t[i]][t[i + 1]]; }
		chmin(shortest, d);
		return false;
	});
