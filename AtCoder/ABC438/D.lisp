...
(defun main ()
  (input* ((n fixnum)
           ...
           (b (vector fixnum n))
           (c (vector fixnum n)))
    (let* ((b-cumsum (cumulate b))
           (c-cumsum (cumulate c))
           (ab-cumsum (cumulate (map 'vector #'(lambda (a b) (- a b)) a b)))
           (ab-st (make-segment-tree (1+ n) #'max (- *inf*) :initial-contents (coerce ab-cumsum 'list)))
           (result (loop for y from 2 below n
                         maximize (let* ((c-sum (- (aref c-cumsum n) (aref c-cumsum y)))
                                         (b-sum (aref b-cumsum y))
                                         (a-sum (segment-tree-fold ab-st 1 y)))
                                    (+ a-sum b-sum c-sum)))))
      (println result))))
