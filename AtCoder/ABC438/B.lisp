...
(defun main ()
  (input* ((n fixnum)
           ...
           (s string)
           (tt string))
    (let ((s (aprog1 (make-array n)
               (dotimes (i n)
                 (setf (aref it i) (char-digit (aref s i))))))
          (tt (aprog1 (make-array m)
                (dotimes (i m)
                  (setf (aref it i) (char-digit (aref tt i)))))))
      (println (loop for k from 0 to (- n m)
                     minimize (reduce #'(lambda (acc p) (+ acc (apply #'distance p)))
                                      (map 'list #'list (subvec/shared s :start k :end (+ k m)) tt)
                                      :initial-value 0))))))
