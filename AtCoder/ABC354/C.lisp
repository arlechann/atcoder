...
(defconstant +inf+ 1000000001)

(defun card-number (card) (first card))
(defun card-attack (card) (second card))
(defun card-cost (card) (third card))

(defun main ()
  (input* ((n fixnum)
           (ac (vector (list fixnum 2) n)))
    (let ((cards (make-array n)))
      (dotimes (i n)
        (setf (aref cards i)
              (cons i (aref ac i))))
      (sortf cards (lambda (a b)
                     (< (card-attack a) (card-attack b))))
      (let ((min-cost +inf+)
            (result nil))
        (dotimes (i n)
          (let* ((i (- n i 1))
                 (card (aref cards i))
                 (cost (card-cost card)))
            (when (minp cost min-cost)
              (setf min-cost cost)
              (push card result))))
        (sortf result (lambda (a b)
                        (< (card-number a) (card-number b))))
        (format t "~A~%" (length result))
        (dolist (card result)
          (format t "~A " (1+ (card-number card))))
        (terpri)))))
