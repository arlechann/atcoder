  ...
  (sb-sequence:with-sequence-iterator (iterator limit from-end-p step endp element set-element index copy)
    ...
    (labels ((rec (iterator)
               ...
               (if (not #?(funcall endp options iterator limit from-end-p))
                   (progn (push #'(lambda ()
                                    (rec (funcall step options iterator)))
                                *stack*)
...
(defun solve ()
  (input* ((n fixnum)
           (wp (vector (cons* fixnum fixnum) n)))
    (sortf wp #'(lambda (a b)
                  (let ((wa (car a))
                        (pa (cdr a))
                        (wb (car b))
                    ...
                    (if (= wa wb)
                        (< pa pb)
                        (< wa wb)))))
    (let* ((w (map 'vector #'car wp))
           (p (map 'vector #'cdr wp))
           (w-cumsum (cumulate w))
           (p-cumsum (cumulate p))
           (result (meguru-method 0 (1+ n) #'(lambda (i)
                                               (let ((weight (aref w-cumsum i))
                                                     (power (- (aref p-cumsum n) (aref p-cumsum i))))
                                                 (>= power weight))))))
      (println result))))
