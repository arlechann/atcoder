...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> String {
			let u = self.n;
			let bg = self.n - self.k;
			let lt = self.k - 1;

			let p_k = 1.0 / u as f64;
			let p_bg = bg as f64 / u as f64;
			let p_lt = lt as f64 / u as f64;
			...
			let mut result =
				(p_k * p_bg * p_lt)
					...
					+ (p_bg * p_lt * p_k)
					+ (p_lt * p_bg * p_k)
					+ (p_k * p_k * p_k) + (p_k * p_k * p_lt)
					+ (p_k * p_k * p_bg) + (p_k * p_lt * p_k)
					+ (p_k * p_bg * p_k) + (p_lt * p_k * p_k)
					+ (p_bg * p_k * p_k);
			format!("{:.18}", result)
		}
	}
