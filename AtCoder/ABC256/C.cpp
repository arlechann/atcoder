...
bool check(const VI2D& board, const VI& h, const VI& w) {
	REP(i, 3) {
		int row = 0;
		REP(j, 3) {
			row += board[i][j];
...
int solve(VI2D& board, int r, int c, const VI& h, const VI& w) {
	if(r == 2 && c == 2) {
		int row_sum = board[0][2] + board[1][2];
		int row_val = w[c] - row_sum;
		if(row_val < 1) {
			return 0;
		}
		int col_sum = board[2][0] + board[2][1];
		int col_val = h[r] - col_sum;
		if(col_val < 1) {
			return 0;
		}
		if(row_val != col_val) {
	...
	int next_row = r + (c + 1) / 3;
	int next_col = (c + 1) % 3;
	if(r == 2) {
		int row_sum = board[0][c] + board[1][c];
		board[r][c] = w[c] - row_sum;
		if(board[r][c] < 1) {
			return 0;
		}
		return solve(board, next_row, next_col, h, w);
	}
	if(c == 2) {
		int col_sum = board[r][0] + board[r][1];
		board[r][c] = h[r] - col_sum;
		if(board[r][c] < 1) {
			return 0;
		}
		return solve(board, next_row, next_col, h, w);
	...
	RANGE(i, 1, 30) {
		board[r][c] = i;
		int patterns = solve(board, next_row, next_col, h, w);
		ret += patterns;
	}
	return ret;
}
...
int main() {
	...
	VI2D board(3, VI(3, 0));
	int result = solve(board, 0, 0, h, w);
