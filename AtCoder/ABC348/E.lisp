...
(let ((tree-size-memo (make-hash-table)))
  (defun tree-size (graph now parent)
    (let ((val (gethash now tree-size-memo nil)))
      (when val (return-from tree-size val)))
    (setf (gethash now *tree-size-memo*)
          (let ((children (graph-neighbors graph now)))
            (reduce #'+
                    (mapcar (lambda (child)
                              (tree-size child now))
                            children)
...
(defun main ()
  (input* ((n fixnum)
           (ab (vector (list fixnum1 fixnum1) (1- n)))
           (c (vector fixnum n)))
