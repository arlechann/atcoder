...
(defun squared-distance (p1 p2)
  (+ (square (diff (point-x p1)
                   (point-x p2)))
     (square (diff (point-y p1)
                   (point-y p2)))))

(defun main ()
  (input* ((n fixnum)
           (xy (vector (list fixnum fixnum) n)))
    (dovector (p1 xy)
      (let ((d-max 0)
            (d-max-index nil))
        (dotimes (i n)
          (let ((p2 (aref xy i)))
            (unless (and (= (point-x p1) (point-x p2))
                         (= (point-y p1) (point-y p2)))
              (let ((d-p2 (squared-distance p1 p2)))
                (unless (>= d-max d-p2)
                  (setf d-max d-p2)
                  (setf d-max-index i))))))
        (format t "~A~%" (1+ d-max-index))))))
