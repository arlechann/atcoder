...
bool dijkstra(const vector<string>& maze,
			  const VI2D& medicines,
			  int sy,
			  int sx,
			  int gy,
			  int gx) {
	...
	int w = maze[0].size();
	VI2D used(h, VI(w, 0));
	VI2D energies(h, VI(w, -1));

	energies[sy][sx] = max(0, medicines[sy][sx]);
	priority_queue<pair<int, pair<int, int>>> pq;
	pq.push(make_pair(energies[sy][sx], make_pair(sy, sx)));
	while(!pq.empty()) {
		auto [energy, point] = pq.top();
		pq.pop();
		int y = point.first;
		int x = point.second;
		// cout << "energy: " << energy << ", y: " << (y + 1) << ", x: " << (x +
		// 1) << endl; if(used[y][x]) { 	continue;
		// }
		if(y == gy && x == gx) {
			return true;
		}
		...
		REP(i, 4) {
			...
			int alt = max(energy - 1, medicines[ny][nx]);
			if(alt <= energies[ny][nx]) {
				continue;
			}
			energies[ny][nx] = alt;
			pq.push(make_pair(alt, make_pair(ny, nx)));
		}
	}
	return false;
}
...
int main() {
	...
	VI2D medicines(h, VI(w, -1));
	REP(i, n) {
		medicines[r[i]][c[i]] = e[i];
	}
	...
	bool result = dijkstra(a, medicines, sy, sx, gy, gx);
	if(result) {
		cout << "Yes" << endl;
	} else {
		cout << "No" << endl;
