...
ll square_distance(ll x1, ll y1, ll x2, ll y2) {
...
int main() {
	...
	UnionFind uf(n + 2);
	REP(i, n) {
		if(square_distance(sx, sy, x[i], y[i]) == square(r[i])) {
			uf.merge(s_index, i);
		}
		if(square_distance(tx, ty, x[i], y[i]) == square(r[i])) {
			uf.merge(t_index, i);
		}
	}
	...
	REP(i, n) {
		REP(j, i) {
			if(square_distance(x[i], y[i], x[j], y[j]) <
			   square(diff(r[i], r[j]))) {
				continue;
			}
			if(square_distance(x[i], y[i], x[j], y[j]) <= square(r[i] + r[j])) {
				uf.merge(i, j);
			}
		}
	}

	if(uf.is_same(s_index, t_index)) {
		cout << "Yes" << endl;
	} else {
		cout << "No" << endl;
	}
