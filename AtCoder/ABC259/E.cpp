...
template <typename T>
constexpr bool chmaxeq(T& m, T x) {
	if(m > x) {
		return false;
	}
	m = x;
...
int main() {
	...
	unordered_map<ll, ll> lcm_map1, lcm_map2;
	REP(i, n) {
		REP(j, m[i]) {
			ll prime = p[i][j];
			ll tmp_e = lcm_map1[prime];
			if(chmaxeq(lcm_map1[prime], e[i][j])) {
				lcm_map2[prime] = tmp_e;
			}
		}
	}
	...
	REP(i, n) {
		...
		REP(j, m[i]) {
			ll prime = p[i][j];
			if(lcm_map1[prime] == e[i][j] &&
			   lcm_map1[prime] != lcm_map2[prime]) {
				is_new = true;
			}
		}
		if(is_new) {
