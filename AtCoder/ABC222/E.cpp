...
void bfs(const Graph& graph,
	...
	while(!q.empty()) {
		...
		for(auto&& e : graph.edges[edge.to]) {
			...
			distances[e.to] = distances[edge.to] + 1;
			q.push(Edge(e.to));
			used[e.to] = true;
			if(e.to == g) {
				goto loop;
...
loop:
	...
	int now = g;
	while(distances[now] != 0) {
		EACH(next, graph.edges[now]) {
			if(distances[next.to] == distances[now] - 1) {
				count[min<int>(now, next.to)][max<int>(now, next.to)]++;
				now = next.to;
				break;
			}
		}
...
int main() {
	...
	Graph g(n);
	REP(i, n - 1) { g.link(u[i], v[i]); }

	unordered_map<int, unordered_map<int, int>> count;

	REP(i, m - 1) { bfs(g, count, a[i], a[i + 1]); }

	auto dp = make_vector({n, K_MAX * 2 + 1}, mint(0));
	dp[0][K_MAX] = mint(1);
	REP(i, n - 1) {
		REP(j, K_MAX * 2) {
			int used = count[min(u[i], v[i])][max(u[i], v[i])];
			dp[i + 1][j] = dp[i][j + used] + dp[i][j - used];
		}
	}

	cout << dp[n - 1][K_MAX + k] << endl;
	return 0;
}
