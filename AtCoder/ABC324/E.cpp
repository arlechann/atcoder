...
int main() {
	...
	REP(i, n) {
		...
		ti = 0;
		REP(i, ss_size) {
			if(ti < t_size && ss[ss_size - i - 1] == t[t_size - ti - 1]) {
				ti++;
			}
		}
		suffix.push_back(ti);
	...
	ll result = 0;
	REP(i, n) {
		int rest = t_size - spx[i];
		int suffix_size = suffix.size();
		int suffix_index = bin_search<int>(suffix_size, -1, [&](int index) {
			if(index == suffix_size) {
				return true;
			}
			return suffix[index] >= rest;
		});
		result += suffix_size - suffix_index;
	}
