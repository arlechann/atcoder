...
(defun main ()
  (input* ((n fixnum)
           ...
           (sx fixnum)
           (sy fixnum)
           (xy (vector (cons* fixnum fixnum) n))
           (dc (vector (cons* symbol fixnum) m)))
    (let ((y-x-houses (make-hash-table))
          (y-x-houses-visited (make-hash-table))
          (x-y-houses (make-hash-table))
          (x-y-houses-visited (make-hash-table)))
      (do-seq (xy xy)
        (let ((x (car xy))
              (y (cdr xy)))
          (let ((y-vec (gethash x x-y-houses
                              (make-array 0 :adjustable t :fill-pointer t)))
                (x-vec (gethash y y-x-houses
                                (make-array 0 :adjustable t :fill-pointer t))))
            (vector-push-extend y y-vec)
            (vector-push-extend x x-vec)
            (setf (gethash x x-y-houses) y-vec
                  (gethash y y-x-houses) x-vec))))
      (loop for y being the hash-key
              using (hash-value vec) of y-x-houses
            do (sort vec #'<)
               (setf (gethash y y-x-houses-visited)
                     (make-array (length vec) :initial-element 0)))
      (loop for x being the hash-key
              using (hash-value vec) of x-y-houses
            do (sort vec #'<)
               (setf (gethash x x-y-houses-visited)
                     (make-array (length vec) :initial-element 0)))
      (let ((now-x sx)
            (now-y sy))
        (loop for (d . c) across dc
              for dx = (* c (ecase d
                              ((u d) 0)
                              (r 1)
                              (l -1)))
              ...
              do (case d
                   ((u d)
                    (let ((y-houses (gethash new-x x-y-houses))
                          (y-houses-visited (gethash new-x x-y-houses-visited))
                          (min-y (min now-y new-y))
                          (max-y (max now-y new-y)))
                      (unless (null y-houses)
                        (let ((low-index
                                (lower-bound y-houses min-y))
                              (up-index
                                (upper-bound y-houses max-y)))
                          (incf (aref y-houses-visited low-index))
                          (when (< up-index (length y-houses-visited))
                            (decf (aref y-houses-visited up-index)))))))
                   ((l r)
                    (let ((x-houses (gethash new-y y-x-houses))
                          (x-houses-visited (gethash new-y y-x-houses-visited))
                          (min-x (min now-x new-x))
                          (max-x (max now-x new-x)))
                      (unless (null x-houses)
                        (let ((low-index
                                (lower-bound x-houses min-x))
                              (up-index
                                (upper-bound x-houses max-x)))
                          (incf (aref x-houses-visited low-index))
                          (when (< up-index (length x-houses-visited))
                            (decf (aref x-houses-visited up-index))))))))
                 (setf now-x new-x
                       now-y new-y))
        (let ((points (make-hash-table :test #'equal)))
          (loop for y being the hash-key
                  using (hash-value xs) of y-x-houses
                for visited = (gethash y y-x-houses-visited)
                do (loop for i from 1 below (length visited)
                         do (incf (aref visited i) (aref visited (1- i))))
                   (loop for i below (length visited)
                         unless (zerop (aref visited i))
                           do (setf (gethash (cons (aref xs i) y) points) 1)))
          (loop for x being the hash-key
                  using (hash-value ys) of x-y-houses
                for visited = (gethash x x-y-houses-visited)
                do (loop for i from 1 below (length visited)
                         do (incf (aref visited i) (aref visited (1- i))))
                   (loop for i below (length visited)
                         unless (zerop (aref visited i))
                           do (setf (gethash (cons x (aref ys i)) points) 1)))
          ;#?(loop for p being the hash-key of points collect p)
          (let ((result (loop for x being the hash-value of points
                              sum x)))
            (format t "~A ~A ~A~%" now-x now-y result)))))))
