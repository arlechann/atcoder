...
(defun main ()
  (input* ((h fixnum)
           ...
           (s (vector string h))
           (tt string))
    (let ((maze (aprog1 (make-array (list h w) :initial-element t)
                  (dotimes (i h)
                    (dotimes (j w)
                      (when (char= (aref (aref s i) j) #\#)
                        (setf (aref it i j) nil))))))
          (house (aprog1 (make-array (list h w) :initial-element 0)
                   (dotimes (i h)
                     (dotimes (j w)
                       (when (char= (aref (aref s i) j) #\@)
                         (setf (aref it i j) 1))))))
          (visited (make-array (list h w) :initial-element 0)))
      (flet ((in-maze-p (y x) (and (<= 0 y h) (<= 0 x w))))
        (let ((now-y y)
              (now-x x))
          (loop for dir across tt
                for dy = (case dir
                           (#\U -1)
                           (#\D 1)
                           (#\L 0)
                ...
                for new-y = (+ now-y dy)
                and new-x = (+ now-x dx) 
                when (and (aref maze new-y new-x)
                          (in-maze-p new-y new-x))
                  do (setf (aref visited new-y new-x) 1
                           now-y new-y
                           now-x new-x))
          (let ((result 0))
            (dotimes (i h)
              (dotimes (j w)
                (incf result (logand (aref house i j)
                                     (aref visited i j)))))
            (format t "~A ~A ~A~%" (1+ now-y) (1+ now-x) result)))))))
