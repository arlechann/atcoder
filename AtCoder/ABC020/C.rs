		...
		pub fn solve(&self) -> i64 {
			...
			for i in 0..self.h {
				for j in 0..self.w {
					if self.s[i][j] == 'S' {
						start = (i, j);
					} else if self.s[i][j] == 'G' {
						goal = (i, j);
					}
				}
			}

			bin_search(1 as i64, self.t as i64, |x| {
				let map = self
					.s
					.iter()
					.map(|l| {
				...
				distances[goal.0][goal.1].unwrap() <= self.t
			})
		}
	}

	fn dijkstra(map: &[Vec<i64>], start: (usize, usize)) -> Vec<Vec<Option<i64>>> {
		use std::cmp::Reverse;
		use std::collections::BinaryHeap;

		const INFi64: i64 = 1_000_000_000_000_000_000;
		const DYDX: [(isize, isize); 4] = [(0, 1), (-1, 0), (0, -1), (1, 0)];

		let h = map.len();
		let w = map[0].len();

		let mut distances: Vec<Vec<Option<i64>>> = vec![vec![None; w]; h];
		let mut pq = BinaryHeap::new();

		distances[start.0][start.1] = Some(0);
		pq.push(Reverse((0, start)));
		while let Some(Reverse((distance, node))) = pq.pop() {
			if distance > distances[node.0][node.1].unwrap_or(INFi64) {
				continue;
			}
			for &(dy, dx) in DYDX.iter() {
				...
				let ny = (node.0 as isize + dy) as usize;
				let nx = (node.1 as isize + dx) as usize;
				let next_node = (ny, nx);
				let cost = map[ny][nx];
				let next_distance = distance + cost;
				if next_distance < distances[next_node.0][next_node.1].unwrap_or(INFi64) {
					distances[next_node.0][next_node.1] = Some(next_distance);
