...
int main() {
	...
	ll result = bin_search<ll>(A_MAX, -1, [&](ll x) {
		VLL2D lt(n + 1, VLL(n + 1, 0));
		REP(i, n) {
			REP(j, n) { lt[i + 1][j + 1] = static_cast<ll>(a[i][j] <= x); }
		}
		REP(i, n) {
			REP(j, n) {
				lt[i + 1][j + 1] += lt[i + 1][j] + lt[i][j + 1] - lt[i][j];
			}
		}
		REP(i, n - k + 1) {
			REP(j, n - k + 1) {
				if(lt[i + k][j + k] - lt[i + k][j] - lt[i][j + k] + lt[i][j] >
				   median) {
					return true;
				}
			}
