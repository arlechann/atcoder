...
(defun string-to-chars (str) (map 'list #'identity str))
(defun chars-to-string (lst) (map 'string #'identity lst))

(defun combination (lst)
  (let ((ret nil))
    (labels ((rec (lst acc)
               (if (null lst)
                   (push (reverse acc) ret)
                   (progn (rec (cdr lst) acc)
