...
(defun set-sizes (queries)
  (let* ((q (length queries))
         (ret (make-array q :initial-element 0 :element-type 'fixnum))
         (used (make-hash-table)))
    (dotimes (i q)
      (let ((query (aref queries i)))
        (cond ((gethash query used nil)
               (setf (gethash query used) nil
                     (aref ret i) (1- (aref ret (1- i)))))
              (t (setf (gethash query used) t
                       (aref ret i) (1+ (if (zerop i)
                                            0 (aref ret (1- i)))))))))
    ret))
...
(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (x (vector fixnum1 q)))
    (let* ((result (make-array n :initial-element 0 :element-type 'fixnum))
           (set-sizes (set-sizes x))
           (inserted (make-hash-table))
           (sizes-cumsum (cumsum set-sizes)))
      (dotimes (i q)
        (let* ((query (aref x i))
               (insert-turn (gethash query inserted nil)))
          (cond (insert-turn
                 (setf (gethash query inserted) nil)
                 (incf (aref result query)
                       (- (aref sizes-cumsum i)
                          (aref sizes-cumsum insert-turn))))
                (t (setf (gethash query inserted) i)))))
      (dotimes (i n)
        (let ((insert-turn (gethash i inserted nil)))
          (when insert-turn
            (incf (aref result i)
                  (- (aref sizes-cumsum q)
                     (aref sizes-cumsum insert-turn))))))
