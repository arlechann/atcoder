...
(defun solve (a b c)
  (let ((x 0) (y 0))
    (dotimes (i 60)
      (when (logbitp i c)
        (cond ((< a b)
               (setf y (logior y (ash 1 i)))
               (decf b))
              (t (setf x (logior x (ash 1 i)))
                 (decf a)))))
    (dotimes (i 60)
      (unless (or (logbitp i c)
                  (zerop a)
                  (zerop b))
        (setf x (logior x (ash 1 i))
              y (logior y (ash 1 i)))
        (decf a)
        (decf b)))
...
(defun test ()
  (test-case "3 4 7" "28 27")
  (test-case "34 56 998244353" "-1")
  (test-case "39 47 530423800524412070" "540431255696862041 10008854347644927")
  )
