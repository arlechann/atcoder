...
int main() {
	...
	auto dp = make_vector(
		{h, w, 2, 2},
		INFLL); // dp[行][列][行が反転されているか][列が反転されているか]
	REP(i, h) {
		REP(j, w) {
			if(i == 0 && j == 0) {
				dp[0][0][0][0] = 0;
				...
				dp[0][j][0][0] = a[0][j - 1] == a[0][j] ? dp[0][j - 1][0][0]
														: dp[0][j - 1][0][1];
				dp[0][j][0][1] = (a[0][j - 1] == a[0][j] ? dp[0][j - 1][0][1]
														 : dp[0][j - 1][0][0]) +
								 c[j];
				dp[0][j][1][0] = a[0][j - 1] == a[0][j] ? dp[0][j - 1][1][0]
														: dp[0][j - 1][1][1];
				dp[0][j][1][1] = (a[0][j - 1] == a[0][j] ? dp[0][j - 1][1][1]
														 : dp[0][j - 1][1][0]) +
								 c[j];
			} else if(j == 0) {
				dp[i][0][0][0] = a[i - 1][0] == a[i][0] ? dp[i - 1][0][0][0]
														: dp[i - 1][0][1][0];
				dp[i][0][0][1] = a[i - 1][0] == a[i][0] ? dp[i - 1][0][0][1]
														: dp[i - 1][0][1][1];
				dp[i][0][1][0] = (a[i - 1][0] == a[i][0] ? dp[i - 1][0][1][0]
														 : dp[i - 1][0][0][0]) +
								 r[i];
				dp[i][0][1][1] = (a[i - 1][0] == a[i][0] ? dp[i - 1][0][1][1]
														 : dp[i - 1][0][0][1]) +
								 r[i];
			} else {
				dp[i][j][0][0] =
					min(a[i][j - 1] == a[i][j] ? dp[i][j - 1][0][0]
											   : dp[i][j - 1][0][1],
						a[i - 1][j] == a[i][j] ? dp[i - 1][j][0][0]
											   : dp[i - 1][j][1][0]);
				dp[i][j][0][1] =
					min((a[i][j - 1] == a[i][j] ? dp[i][j - 1][0][1]
												: dp[i][j - 1][0][0]) +
							c[j],
						a[i - 1][j] == a[i][j] ? dp[i - 1][j][0][1]
											   : dp[i - 1][j][1][1]);
				dp[i][j][1][0] =
					min(a[i][j - 1] == a[i][j] ? dp[i][j - 1][1][0]
											   : dp[i][j - 1][1][1],
						(a[i - 1][j] == a[i][j] ? dp[i - 1][j][1][0]
												: dp[i - 1][j][0][0]) +
							r[i]);
				dp[i][j][1][1] =
					min((a[i][j - 1] == a[i][j] ? dp[i][j - 1][1][1]
												: dp[i][j - 1][1][0]) +
							c[j],
						(a[i - 1][j] == a[i][j] ? dp[i - 1][j][1][1]
												: dp[i - 1][j][0][1]) +
							r[i]);
			}
		}
