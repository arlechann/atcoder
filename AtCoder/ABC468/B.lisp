...
(defun main ()
  (input* ((m fixnum)
           (d fixnum)
           (s string))
    (let* ((cells (map 'vector #'(lambda (c) (if (char= c #\G) t nil)) s))
           (g-pos-list (loop for c across s
                             for i from 0
                             when (char= c #\G)
                               collect i)))
      (loop for g-pos in g-pos-list
            do (loop for k from (max (- g-pos d) 0) to (min (+ g-pos d) (1- m))
                     do (setf (aref cells k) t)))
      (println (loop for cell across cells
                     count (not cell))))))
