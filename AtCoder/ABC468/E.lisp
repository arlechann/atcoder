...
;; a1 * (1/1 + 1/2 + 1/3 + 1/4 + ... + 1/n)
;; a2 * (1/1 + 2*(1/2 + 1/3 + 1/4 + ... + 1/(n-1)) + 1/n)
;; a3 * (1/1 + 2*1/2 + 3*(1/3 + 1/4 + ... + 1/(n-2)) + 2*1/(n-1) + 1/n)

(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum n)))
    (setf mint:*mint-modulus* 998244353)
    (let* ((ma (map 'vector #'mint:make-mint a))
           (inv-cumsum (cumulate (map 'vector
                                      #'(lambda (x)
                                          (mint:mint/ 1 x))
                                      (iota n :start 1))
                                 :op #'mint:mint+
                                 :id (mint:to-mint 0)))
           (result (mint:to-mint 0)))
      (loop with coefficient = (mint:to-mint 0)
            for i from 0 below (floor n 2)
            do (setf coefficient
                     (mint:mint+ coefficient
                                 (mint:mint- (aref inv-cumsum (- n i))
                                             (aref inv-cumsum i)))
                     result
                     (mint:mint+ result
                                 (mint:mint* coefficient (aref ma i)))))
      (loop with coefficient = (mint:to-mint 0)
            for i from (1- n) downto (floor n 2)
            and k from 0
            do (setf coefficient
                     (mint:mint+ coefficient
                                 (mint:mint- (aref inv-cumsum (- n k))
                                             (aref inv-cumsum k)))
                     result
                     (mint:mint+ result
                                 (mint:mint* coefficient (aref ma i)))))
      (println (mint:mint-value result)))))
