...
(defun main ()
  (input* ((s string))
    (let ((n (length s)))
      (labels ((pre-palindrome-count (left right rewrite)
                 (when (or (< left 0)
                           (>= right n))
                   (return-from pre-palindrome-count 0))
                 (when (and rewrite
                            (char/= (aref s left)
                                    (aref s right)))
                   (return-from pre-palindrome-count 0))
                 (1+ (pre-palindrome-count (1- left)
                                           (1+ right)
                                           (or rewrite
                                               (char/= (aref s left)
                                                       (aref s right)))))))
        (println (+ (loop for i from 0 below n
                          sum (pre-palindrome-count i i nil))
                    (loop for i from 0 below (1- n)
                          sum (pre-palindrome-count i (1+ i) nil))))))))
