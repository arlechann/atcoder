...
(defun main ()
  (input* ((s string)
    ...
    (let* ((s-len (length s))
           (t-len (length tt))
           (result (loop with result = 0
                         and s-index = 0
                         and t-index = 0
                         ;; do #?s-index #?t-index #?result
                         when (and (= s-index s-len)
                                   (= t-index t-len))
                           return result
                         if (= s-index s-len)
                           if (char= (aref tt t-index) #\A)
                             do (incf t-index)
                                (incf result)
                           else
                             return -1
                         else if (= t-index t-len)
                           if (char= (aref s s-index) #\A)
                             do (incf s-index)
                                (incf result)
                           else
                             return -1
                         else if (char= (aref s s-index)
                                        (aref tt t-index))
                           do (incf s-index)
                              (incf t-index)
                         else if (char= (aref s s-index) #\A)
                           do (incf s-index)
                              (incf result)
                         else if (char= (aref tt t-index) #\A)
                           do (incf t-index)
                              (incf result)
                         else
                           return -1)))
