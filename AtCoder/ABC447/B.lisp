...
(defun main ()
  (input* ((s string))
    (let* ((count (count-chars s))
           (remove-chars (nmap #'cdr 
                               (remove (reduce #'max count)
                                       (map-with-index 'vector
                                                       #'(lambda (i n) (cons n (index-to-lower i)))
                                                       count)
                                       :key #'car)))
           (result (remove-if #'(lambda (c) (not (find c remove-chars))) s)))
      (println result))))
