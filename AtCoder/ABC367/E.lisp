...
(defun-always singlep (lst) (and lst (null (cdr lst))))
...
(defun main ()
  (input* ((n fixnum)
           ...
           (x (vector fixnum1 n))
           (a (vector fixnum n)))
    (let ((doubling (array-dp doubling ((i 60) (k n))
                      (if (zerop i)
                          (aref x k)
                          (doubling (1- i)
                                    (doubling (1- i) k))))))
      (let ((result (make-array n :initial-contents a)))
        (do-popbit (i k)
          (let ((new-result (make-array n :initial-element 0)))
            (dotimes (k n)
              (setf (aref new-result k)
                    (aref result (funcall doubling i k))))
...
4 3 2 1
" "4 3 2 1")
  (test-case "9 1000000000000000000
3 7 8 5 9 3 7 4 2
9 9 8 2 4 4 3 5 3
" "3 3 3 3 3 3 3 3 3")
  )
