...
(defun main ()
  (input* ((x string))
    (setf x (nreverse (coerce x 'list)))
    (let ((result (nlet rec ((acc nil) (x x) (dotp nil))
                    (if (null x)
                        (coerce acc 'string)
                        (if (and (null acc)
                                 (not dotp)
                                 (or (char= (car x) #\0)
                                     (char= (car x) #\.)))
                            (rec acc (cdr x) (char= (car x) #\.))
                            (rec (cons (car x) acc) (cdr x) (or (char= (car x) #\.) dotp)))))))
      (format t "~A~%" result))))
                                     

(defun test ()
  (test-case "1.012" "1.012")
  (test-case "12.340" "12.34")
  (test-case "99.900" "99.9")
  (test-case "0.000" "0")
  )
