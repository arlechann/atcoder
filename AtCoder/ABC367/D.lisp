...
(defconstant +m-max+ 1000000)

(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (a (vector fixnum n)))
    (let* ((a (concatenate 'vector a a))
           (cumsum-a (cumulate a))
           (cumsum-mod-a (map 'vector (lambda (x) (mod x m)) cumsum-a)))
      (let ((result (loop with count-mod = (make-array (1+ +m-max+) :initial-element 0)
                          and result = 0
                          initially (progn (dotimes (i n)
                                             (incf (aref count-mod (aref cumsum-mod-a i))))
                                           (incf result (1- (aref count-mod 0))))
                          for i from 0 below (1- n)
                          do (decf (aref count-mod (aref cumsum-mod-a i)))
                             (incf (aref count-mod (aref cumsum-mod-a (+ i n))))
                             (incf result (1- (aref count-mod (aref cumsum-mod-a (1+ i)))))
