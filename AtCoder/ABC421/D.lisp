...
(defun main ()
  (input* ((rt fixnum)
           ...
           (m fixnum)
           (l fixnum)
           (sa (vector (cons* fixnum symbol) m))
           (tb (vector (cons* fixnum symbol) l)))
    (when (/= (mod (+ rt ct) 2)
              (mod (+ ra ca) 2))
      (println 0)
      (return-from main))
    (multiple-value-bind (sa tb)
        (loop with moves = 0
              and t-index = 0
              and a-index = 0
              and t-moves = nil
              and a-moves = nil
              until (= moves n)
              for sa2 = (aref sa t-index)
              and tb2 = (aref tb a-index)
              if (= (cdr sa2) (cdr tb2))
                do (push sa2 t-moves)
                   (push tb2 a-moves)
                   (incf t-index)
                   (incf a-index)
                   (incf moves (cdr sa2))
              else if (< (cdr sa2) (cdr tb2))
                do (let ((turn (cdr sa2)))
                     (push sa2 t-moves)
                     (push (cons (car tb2) turn) a-moves)
                     (decf (cdr tb2) turn)
                     (incf t-index)
                     (incf moves turn))
              else
                do (let ((turn (cdr tb2)))
                     (decf (cdr sa2) turn)
                     (push (cons (car sa2) turn) t-moves)
                     (push tb2 a-moves)
                     (incf a-index)
                     (incf moves turn))
              finally (return (values (nreverse t-moves) (nreverse a-moves))))
      #?sa #?tb
      (let ((result (loop with ty = rt
                          and tx = ct
                          and ay = ra
                          and ax = ca
                          for (td . tturn) in sa
                          and (ad . aturn) in tb
                          sum (multiple-value-bind (tdy tdx)
                                  (direction-to-moves td)
                                (multiple-value-bind (ady adx)
                                    (direction-to-moves ad)
                                  (let* ((tdy (* tdy tturn))
                                         (tdx (* tdx tturn))
                                         (ady (* ady aturn))
                                         (adx (* adx aturn)))
...
(defun direction-to-moves (direction)
  (case direction
    (U (values -1 0))
    (D (values 1 0))
    (L (values 0 -1))
...
(defun crossp (ay ax ady adx by bx bdy bdx)
  nil)
...
U 3
" "1")
  (test-case "1000000000 1000000000 -1000000000 -1000000000
3000000000 3 3
L 1000000000
U 1000000000
U 1000000000
