...
(defun main ()
  (input* ((n fixnum)
           (s string))
    (declare (ignorable n))
    (when (char= (aref s 0) #\B)
      (map-into s #'(lambda (c) (if (char= c #\A) #\B #\A)) s))
    (let* ((a-indexes (nmap #'car
                            (delete-if #'(lambda (c) (char= c #\B))
                                       (map-with-index 'list #'(lambda (i c) (cons i c)) s)
                                       :key #'cdr)))
           (result (min (loop for i from 0 by 2
                              for a-index in a-indexes
                              sum (diff i a-index))
