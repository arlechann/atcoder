...
(defun atcoder-clean-pre-text (text)
  (let* ((lines (mapcar #'(lambda (line)
                            (delete #\CR line))
                        (split-string text :separator '(#\Newline))))
...
(defparameter *a-max* 10)

(defun main ()
  (input* ((n fixnum)
           (ab (vector (cons* fixnum fixnum1) n))
           (m fixnum)
           (s (vector string m)))
    (let ((dict (aprog1 (make-array (list (1+ *a-max*) (1+ *a-max*)) :initial-element nil)
                  (dotimes (i (1+ *a-max*))
                    (dotimes (j (1+ *a-max*))
                      (setf (aref it i j) (make-hash-table))))
                  (loop for str across s
                        for len = (length str)
                        do (loop for i from 0
                                 and c across str
                                 do (setf (gethash c (aref it len i)) t))))))
      (loop for str across s
            for len = (length str)
            if (= len n)
              do (print-boolean
                  (loop for (a . b) across ab
                        for c across str
                        always (gethash c (aref dict a b))))
            else
              do (print-boolean nil)))))
