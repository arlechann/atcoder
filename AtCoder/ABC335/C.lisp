...
(eval-when (:compile-toplevel :execute)
  (def-input-reader symbol ()
    '(read)))

(defun query1 (n dragon tail-index)
  (let ((c (read)))
    (let* ((head-index (mod (+ (1- tail-index) n) n))
           (head-pos (aref dragon head-index))
           (head-y (car head-pos))
           (head-x (cdr head-pos)))
      (case c
        (r (setf (aref dragon tail-index) (cons head-y (1+ head-x))))
        (l (setf (aref dragon tail-index) (cons head-y (1- head-x))))
        (u (setf (aref dragon tail-index) (cons (1+ head-y) head-x)))
        (d (setf (aref dragon tail-index) (cons (1- head-y) head-x))))
      (mod (1+ tail-index) n))))

(defun query2 (n dragon tail-index)
  (let ((p (read)))
    (let ((pos (aref dragon (mod (- (+ tail-index n) p) n))))
      (format t "~A ~A~%" (cdr pos) (car pos)))))

(defun query (n q i dragon tail-index)
  (unless (= i q)
    (let ((ty (read)))
      (if (= ty 1)
          (let ((tail-index (query1 n dragon tail-index)))
            (query n q (1+ i) dragon tail-index))
          (progn (query2 n dragon tail-index)
                 (query n q (1+ i) dragon tail-index))))))

(defun main ()
  (input* ((n fixnum)
           (q fixnum))
    (let ((dragon (make-array n
                               :initial-contents (nreverse
                                                  (loop for i from 0 below n
                                                        collect (cons 0 (1+ i)))))))
      (query n q 0 dragon 0))))
