...
(defun dp (graph uf a node value)
  (and (gethash (cons node value) *memo*)
       (setf (gethash (cons node value) *memo*) 
             (if (union-find-unite-p uf node 0)
                 value
                 (let ((node-value (aref a node)))
                   (reduce #'max
                           (mapcar (lambda (next)
                                     (let ((next-value (aref a next)))
                                       (if (> next-value node-value)
                                           0
                                           (dp graph uf a next (1+ value)))))
...
(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (a (vector fixnum n))
           (uv (vector (list fixnum1 fixnum1) m)))
    (let ((uf (make-union-find n)))
      (labels ((node-value (i)
        ...
        (dovector (edge uv)
          (let ((from (car edge))
                (to (cadr edge)))
            (when (= (node-value from)
                     (node-value to))
              (union-find-merge uf from to)))))
      (dotimes (i m)
        (let ((edge (aref uv i)))
          (rplaca edge (union-find::union-find-root uf (car edge)))
          (rplaca (cdr edge) (union-find::union-find-root uf (cadr edge)))))
      (let ((graph (make-graph-from-edges n uv)))
        (let ((result (dp graph uf a (union-find::union-find-root uf 0) 0)))
          (format t "~A~%" result))))))
