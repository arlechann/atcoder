...
int main() {
	...
	auto distances = make_vector({n, n}, static_cast<ll>(INF));
	auto graph = Graph(n);
	REP(i, m) { graph.link(a[i], b[i], c[i]); }
	REP(i, n) { distances[i] = dijkstra(graph, i); }

	// REP(i, n) {
	// 	REP(j, n) {
	// 		cout << (distances[i][j] == INF ? -1 : distances[i][j]) << " ";
	// 	}
	// 	cout << endl;
	// }
	...
	REP(i, n) {
		...
		REP(j, n) {
			if(i == j) {
				chmin(result, distances[i][j]);
			} else {
				chmin(result, distances[i][j] + distances[j][i]);
			}
		}
