...
(defun main ()
  (input* ((n fixnum)
           ...
           (r fixnum)
           (s string))
    (let* ((alphabet-count (count-alphabet))
           (characters (aprog1 (make-array (list alphabet-count n) :initial-element 0)
                         (dotimes (i alphabet-count)
                           (dotimes (j n)
                             (when (= (char-to-index (aref s j)) i)
                               (setf (aref it i j) 1))))))
           (cumulates (aprog1 (make-array (list alphabet-count (1+ n)) :initial-element 0)
                        (dotimes (i alphabet-count)
                          (dotimes (j n)
                            (setf (aref it i (1+ j))
                                  (+ (aref it i j) (aref characters i j))))))))
      (loop for i from 1 to n
            for c = (char-to-index (aref s (1- i)))
            when (> (- i l) 0)
              sum (- (aref cumulates c (- i l)) (aref cumulates c (max (- i r 1) 0)))
            into result
            finally (println result)))))
