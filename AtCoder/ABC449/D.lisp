...
(defun count-horizontal (l r d u y x)
  (unless (<= d y u)
    (return-from count-horizontal 0))
  (1+ (- (min r x) (max l (- x)))))

(defun count-vertical (l r d u y x)
  (unless (<= l x r)
    (return-from count-vertical 0))
  (1+ (- (min u (1- y)) (max d (1+ (- y))))))
...
(defun main ()
  (input* ((l fixnum)
           ...
           (d fixnum)
           (u fixnum))
    (loop for x from 0 to (max (abs l) (abs r) (abs d) (abs u)) by 2
          when (intersectp l r d u x)
            sum (if (zerop x)
                    1
                    (+ (count-horizontal l r d u x x)
                       (count-horizontal l r d u (- x) x)
                       (count-vertical l r d u x x)
                       (count-vertical l r d u x (- x))))
              into result
