...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&mut self) -> usize {
			let mut start = 0;
			let mut modulo = 1;
			let mut now = self.a - 1;
			let mut visited = vec![None; self.n];
			visited[now] = Some(0usize);
			loop {
				let next = self.b[now] - 1;
				if let Some(x) = visited[next] {
					start = x;
					modulo = visited[now].unwrap() + 1 - x;
					break;
				}
				visited[next] = visited[now].map(|x| x + 1);
				now = next;
			}
			...
			let mov = {
				if self.k.len() > 6 {
					...
					let mov = self.k.iter().collect::<String>().parse().unwrap();
					if mov > start {
						(mov - start) % modulo + start
					} else {
						mov
					}
				}
