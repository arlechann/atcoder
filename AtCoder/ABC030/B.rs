...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> String {
			let short_degree = (self.n % 12 * 30) as f64 + self.m as f64 / 2.0;
			let long_degree = (self.m * 6) as f64;

			let result = f64_min(
				(short_degree - long_degree).abs(),
				(360.0 - f64_max(short_degree, long_degree)) + f64_min(short_degree, long_degree),
			);
			format!("{:.15}", result)
		}
	}
