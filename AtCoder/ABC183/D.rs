...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> String {
			const T_MAX: usize = 200000;
			let mut imos = Imos::with_capacity(T_MAX + 1);
			for &(s, t, p) in self.stp.iter() {
				imos.query(s, t, p as i64);
			}
			if imos.apply().iter().find(|&&x| x > self.w as i64).is_some() {
				"No".to_string()
			} else {
				"Yes".to_string()
			}
