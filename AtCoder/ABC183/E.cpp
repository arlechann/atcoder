...
int main() {
	...
	vector<vector<vector<mint>>> dp(
		h, vector<vector<mint>>(w, vector<mint>(3, 0)));
	// 横0, 縦1, 斜め2
	REP(i, h) {
		REP(j, w) {
			if(s[i][j] == '#') {
			...
			if(i == h - 1 && j == w - 1) {
				...
				mint sum = dp[i][j][0] + dp[i][j][1] + dp[i][j][2];
				dp[i][j + 1][0] = sum + dp[i][j][0];
			} else if(j == w - 1) {
				mint sum = dp[i][j][0] + dp[i][j][1] + dp[i][j][2];
	...
	cout << dp[h - 1][w - 1][0] + dp[h - 1][w - 1][1] + dp[h - 1][w - 1][2]
		 << endl;
	return 0;
}
