...
int main() {
	...
	RANGE(z, 2, n) {
		ll target_q = cumsum[z] - q;
		if(target_q < 0) {
			continue;
		}
		auto iter_lb_q = lower_bound(ALL(cumsum), target_q);
		if(target_q != *iter_lb_q) {
			continue;
		}
		int y = distance(cumsum.begin(), iter_lb_q);

		ll target_p = cumsum[y] - p;
		if(target_p < 0) {
			continue;
		}
		auto iter_lb_p = lower_bound(ALL(cumsum), target_p);
		if(target_p != *iter_lb_p) {
			continue;
		}

		ll target_r = cumsum[z] + r;
		if(target_r > cumsum[n]) {
			continue;
		}
		auto iter_lb_r = lower_bound(ALL(cumsum), target_r);
		if(target_r != *iter_lb_r) {
			continue;
		}
