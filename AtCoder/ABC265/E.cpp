...
unordered_map<ll, unordered_set<ll>> disabled;
int used[N_MAX + 1][N_MAX + 1][N_MAX + 1];
mint memo[N_MAX + 1][N_MAX + 1][N_MAX + 1];

mint dp(int ab, int cd, int ef) {
	int& is_used = used[ab][cd][ef];
	mint& ret = memo[ab][cd][ef];
	if(is_used != 0) {
		return ret;
	}
	is_used = 1;

	ll xx = a * ab + c * cd + e * ef;
	ll yy = b * ab + d * cd + f * ef;

	if(disabled[xx].count(yy) != 0) {
		return 0;
	}
...
int main() {
	...
	REP(i, m) {
		disabled[x[i]].insert(y[i]);
	}

	mint result = dp(0, 0, 0);
