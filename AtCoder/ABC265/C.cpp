...
pair<int, int> solve(const vector<string>& g, VI2D& used, int r, int c) {
	if(used[r][c] != 0) {
		return {-1, -1};
	}
	used[r][c] = 1;

	if((g[r][c] == 'U' && r == 0) || (g[r][c] == 'D' && r == h - 1) ||
	   (g[r][c] == 'L' && c == 0) || (g[r][c] == 'R' && c == w - 1)) {
		return {r, c};
	}
	int nr = g[r][c] == 'U' ? r - 1 : g[r][c] == 'D' ? r + 1 : r;
	int nc = g[r][c] == 'L' ? c - 1 : g[r][c] == 'R' ? c + 1 : c;
	return solve(g, used, nr, nc);
}
