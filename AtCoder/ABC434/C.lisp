...
(defun solve (n h tt l u)
  (loop with prev = 0
        and upper = h
        and lower = h
        for i from 0 below n
        for time = (aref tt i)
        and section-upper = (aref u i)
        and section-lower = (aref l i)
        for dt = (- time prev)
        for upper-limit = (+ upper dt)
        and lower-limit = (max (- lower dt) 0)
        ;do #?(list i prev upper lower) #?(list upper-limit lower-limit) #?(list time section-upper section-lower)
        if (> section-lower upper-limit)
          do (return-from solve nil)
        else if (> lower-limit section-upper)
          do (return-from solve nil)
        do (setf prev time)
           (setf upper (min upper-limit section-upper))
           (setf lower (max lower-limit section-lower))
        finally (return t)))
...
(defun main ()
  (input* ((tt fixnum))
    (dotimes (i tt)
      (input* ((n fixnum)
               (h fixnum)
               (tlu (vector (cons* fixnum fixnum fixnum null) n)))
        (let* ((time (map 'vector #'(lambda (tlu) (first tlu)) tlu))
               (l (map 'vector #'(lambda (tlu) (second tlu)) tlu))
               (u (map 'vector #'(lambda (tlu) (third tlu)) tlu)))
          (print-boolean (solve n h time l u)))))))
