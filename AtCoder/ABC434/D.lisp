...
(defun make-imos2d (h w &optional initial-contents)
  (list :imos2d nil (or initial-contents (make-array (list h w) :initial-element 0))))

(defun imos2d-applied-p (imos2d)
  (second imos2d))

(defun imos2d-array (imos2d)
  (third imos2d))

(defun imos2d-ref (imos2d r c)
  (aref (imos2d-array imos2d) r c))

(defun imos2d-height (imos2d)
  (array-dimension (imos2d-array imos2d) 0))

(defun imos2d-width (imos2d)
  (array-dimension (imos2d-array imos2d) 1))

(defun imos2d-add (imos2d u l d r value)
  (let ((a (imos2d-array imos2d))
        (h (imos2d-height imos2d))
        (w (imos2d-width imos2d)))
    (incf (aref a u l) value)
    (when (< d h)
      (decf (aref a d l) value))
    (when (< r w)
      (decf (aref a u r) value))
    (when (and (< d h) (< r w))
      (incf (aref a d r) value)))
  (setf (second imos2d) nil))

(defun imos2d-apply (imos2d)
  (unless (imos2d-applied-p imos2d)
    (let ((a (imos2d-array imos2d))
          (h (imos2d-height imos2d))
          (w (imos2d-width imos2d)))
      (dotimes (i h)
        (dotimes (j w)
          (unless (zerop j)
            (incf (aref a i j) (aref a i (1- j))))))
      (dotimes (i h)
        (dotimes (j w)
          (unless (zerop i)
            (incf (aref a i j) (aref a (1- i) j)))))
      (setf (second imos2d) t)))
  imos2d)

(defun imos2d-query (imos2d r c)
  (unless (imos2d-applied-p imos2d)
    (imos2d-apply imos2d))
  (imos2d-ref imos2d r c))
...
(defun main ()
  (input* ((n fixnum)
           (clouds (vector (cons* fixnum1 fixnum fixnum1 fixnum null) n)))
    (let* ((sky (aprog1 (make-imos2d *h* *w*)
                  (dotimes (i n)
                    (let* ((cloud (aref clouds i))
                           (u (first cloud))
                           (d (second cloud))
                           (l (third cloud))
                           (r (fourth cloud)))
                      (imos2d-add it u l d r 1)))
                  (imos2d-apply it)))
           (blue-count (let ((c 0))
                         (dotimes (i *h* c)
                           (dotimes (j *w*)
                             (incf c (if (zerop (imos2d-query sky i j))
                                         1 0))))))
           (covered-one (progn (let ((a (make-array (list *h* *w*) :initial-element 0)))
                                 (dotimes (i *h*)
                                   (dotimes (j *w*)
                                     (when (= (imos2d-query sky i j) 1)
                                       (setf (aref a i j) 1))))
                                 (make-imos2d *h* *w* a)))))
      ;#?sky #?covered-one #?blue-count
      (loop for k from 0 below n
            for cloud = (aref clouds k)
            for u = (first cloud)
            and d = (second cloud)
            and l = (third cloud)
            and r = (fourth cloud)
            for removed-count = (- (+ (imos2d-query covered-one (1- d) (1- r))
                                      (if (or (zerop u) (zerop l)) 0 (imos2d-query covered-one (1- u) (1- l))))
                                   (if (zerop u) 0 (imos2d-query covered-one (1- u) (1- r)))
                                   (if (zerop l) 0 (imos2d-query covered-one (1- d) (1- l))))
            do (println (+ blue-count removed-count))))))
