...
(defun main ()
  (input* ((n fixnum)
    ...
    (let ((s-num 6)
          ...
          (l-num 12))
      (let ((result 
              (nlet rec ((s-count 0) (m-count 0) (l-count 0))
                (if (>= (+ (* s-num s-count)
                           (* m-num m-count)
                           (* l-num l-count))
                        n)
                    ...
                    (let ((ret 10000000))
                      ...
                      (when (zerop l-count)
                        (setf ret
                              (min ret
                                   (rec s-count (1+ m-count) 0))))
                      (setf ret
                            (min ret
                                 (rec s-count m-count (1+ l-count))))
                      ret)))))
        (format t "~A~%" result)))))
...
(labels ((test-case (input expect)
  ...
  (test-case "16 120 150 200" "300")
  (test-case "10 100 50 10" "10")
  (test-case "99 600 800 1200" "10000")
  )
