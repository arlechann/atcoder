...
(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (p (vector string n)))
    (let ((cumsum2d (make-array (list (1+ n) (1+ n))
                                :element-type 'fixnum
                                :initial-element 0)))
      (nlet rec ((i 0) (j 0))
        (cond ((= i n) nil)
              ((= j n) (rec (1+ i) 0))
              (t (setf (aref cumsum2d (1+ i) (1+ j))
                       (- (+ (aref cumsum2d (1+ i) j)
                             (aref cumsum2d i (1+ j))
                             (c->n (aref (aref p i) j)))
                          (aref cumsum2d i j)))
      ...
      (nlet rec ((i 0))
        (unless (= i q)
          (format t "~S~%" (solve-query i n q p cumsum2d))
          (rec (1+ i)))))))

(defun solve-query (i n q p cumsum2d)
  (input* ((top fixnum)
           (left fixnum)
           (bottom fixnum)
           (right fixnum))
    (incf bottom)
    (incf right)
    (let ((hd (* n (floor top n)))
          (wd (* n (floor left n))))
      (decf top hd)
      (decf left wd)
      (decf bottom hd)
      (decf right hd))
    (cond ((and (= (floor top n) (floor (1- bottom) n))
                (= (floor left n) (floor (1- right) n)))
           (+ (- (aref cumsum2d bottom right)
                 (aref cumsum2d bottom left)
                 (aref cumsum2d top right))
              (aref cumsum2d top left)))
          ((= (floor top n) (floor (1- bottom) n))
           (let* ((full-width (- (floor right n)
                                 (ceiling left n))))
             (+ (* full-width
                   (- (aref cumsum2d bottom n)
                      (aref cumsum2d top n)))
                (+ (- (aref cumsum2d bottom n)
                      (aref cumsum2d bottom left)
                      (aref cumsum2d top n))
                   (aref cumsum2d top left))
                (- (aref cumsum2d bottom (rem right n))
                   (aref cumsum2d top (rem right n))))))
          ((= (floor left n) (floor (1- right) n))
           (let* ((full-height (- (floor bottom n)
                                  (ceiling top n))))
             (+ (* full-height
                   (- (aref cumsum2d n right)
                      (aref cumsum2d n left)))
                (+ (- (aref cumsum2d n right)
                      (aref cumsum2d n left)
                      (aref cumsum2d top right))
                   (aref cumsum2d top left))
                (- (aref cumsum2d (rem bottom n) right)
                   (aref cumsum2d (rem bottom n) left)))))
          (t (let* ((full-height (- (floor bottom n)
                                    (ceiling top n)))
                    (full-width (- (floor right n)
                                   (ceiling left n))))
               (+ (* full-width
                     full-height
                     (aref cumsum2d n n))
                  (* full-width
                     (+ (- (aref cumsum2d n n)
                           (aref cumsum2d top n))
                        (aref cumsum2d (rem bottom n) n)))
                  (* full-height
                     (+ (- (aref cumsum2d n n)
                           (aref cumsum2d n left))
                        (aref cumsum2d n (rem right n))))
                  (+ (- (aref cumsum2d n n)
                        (aref cumsum2d top n)
                        (aref cumsum2d n left))
                     (aref cumsum2d top left))
                  ...
                  (- (aref cumsum2d (rem bottom n) n)
                     (aref cumsum2d (rem bottom n) left))
                  (aref cumsum2d (rem bottom n) (rem right n))))))))
