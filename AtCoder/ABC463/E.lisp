  ...
  (input* ((n fixnum)
    ...
    (let* ((extended-size (1+ (* n 2)))
           (graph (aprog1 (make-adlist-single-edge-graph extended-size)
                    ...
                    (dotimes (i n)
                      (let ((extended-node (+ i n))
                            (extra-node (* n 2)))
                        (graph-add-edge it i extended-node :cost (aref x i))
                        (graph-add-edge it extended-node i :cost (aref x i))
                        (graph-add-edge it extended-node extra-node :cost y)
                        (graph-add-edge it extra-node extended-node :cost 0)))))
