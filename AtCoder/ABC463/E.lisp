...
(define-compiler-macro call-with-graph-neighbors (&whole form graph node fn)
  (let ((graph-var (gensym "GRAPH-"))
        (node-var (gensym "NODE-"))
        (fn-var (gensym "FN-"))
        (edge-var (gensym "EDGE-"))
        (to-node-var (gensym "TO-NODE-"))
        (matrix-var (gensym "MATRIX-")))
    `(let ((,graph-var ,graph)
           (,node-var ,node)
           (,fn-var ,fn))
       (typecase ,graph-var
         (<adlist-graph>
          (assert-graph-node-index ,graph-var ,node-var "CALL-WITH-GRAPH-NEIGHBORS")
          (dolist (,edge-var (aref (graph-adlist ,graph-var) ,node-var))
            (funcall ,fn-var ,edge-var)))
         (<matrix-graph>
          (assert-graph-node-index ,graph-var ,node-var "CALL-WITH-GRAPH-NEIGHBORS")
          (let ((,matrix-var (graph-matrix ,graph-var)))
            (dotimes (,to-node-var (graph-size ,graph-var))
              (when-let ((,edge-var (aref ,matrix-var ,node-var ,to-node-var)))
                (funcall ,fn-var ,edge-var)))))
         (t
          (funcall (symbol-function 'call-with-graph-neighbors)
                   ,graph-var ,node-var ,fn-var))))))
...
(defun dinic (graph source sink &key (capacity #'edge-cost))
  "Compute max-flow from SOURCE to SINK with Dinic algorithm.
EDGE-COST is treated as capacity by default."
  (assert-graph-node-index graph source "DINIC")
  (assert-graph-node-index graph sink "DINIC")
  (let* ((size (graph-size graph))
         (residual (make-adlist-multigraph size))
         (reverse-edge (make-hash-table :test #'eq)))
    ;; Build residual graph from GRAPH using graph package API only.
    (dotimes (from size)
      (do-graph-neighbors (edge graph from)
        (let* ((to (edge-to edge))
               (cap (funcall capacity edge)))
          #-atcoder
          (unless (and (realp cap) (>= cap 0))
            (error "DINIC: capacity must be non-negative real, got ~S." cap))
          (when (> cap 0)
            (let ((forward (make-edge from to :cost cap))
                  (backward (make-edge to from :cost 0)))
              (push forward (graph-multi-edges-ref residual from to))
              (push backward (graph-multi-edges-ref residual to from))
              (setf (gethash forward reverse-edge) backward
                    (gethash backward reverse-edge) forward))))))
    (let ((level (make-array size :initial-element -1))
          (it (make-array size :initial-element 0))
          (adj (make-array size :initial-element #())))
      (dotimes (node size)
        (setf (aref adj node) (coerce (graph-neighbors residual node) 'simple-vector)))
      (labels ((build-level ()
                 (fill level -1)
                 (let ((queue (make-deque)))
                   (setf (aref level source) 0)
                   (deque-push-back queue source)
                   (loop until (deque-empty-p queue)
                         do (let ((node (deque-peek-front queue)))
                              (deque-pop-front queue)
                              (do-graph-neighbors (edge residual node)
                                (let ((next (edge-to edge)))
                                  (when (and (> (edge-cost edge) 0)
                                             (= (aref level next) -1))
                                    (setf (aref level next) (1+ (aref level node)))
                                    (deque-push-back queue next)))))))
                 (/= (aref level sink) -1))
               (send-flow (node f)
                 (when (= node sink)
                   (return-from send-flow f))
                 (loop while (< (aref it node) (length (aref adj node)))
                       for i = (aref it node)
                       for edge = (svref (aref adj node) i)
                       do (let ((next (edge-to edge)))
                            (if (or (<= (edge-cost edge) 0)
                                    (/= (aref level next) (1+ (aref level node))))
                                (incf (aref it node))
                                (let ((pushed (send-flow next (min f (edge-cost edge)))))
                                  (if (> pushed 0)
                                      (progn
                                        (decf (edge-cost edge) pushed)
                                        (incf (edge-cost (gethash edge reverse-edge)) pushed)
                                        (return pushed))
                                      (incf (aref it node))))))
                       finally (return 0))))
        (loop with max-flow = 0
              while (build-level)
              do (fill it 0)
                 (loop for pushed = (send-flow source *graph-cost-infinity*)
                       while (> pushed 0)
                       do (incf max-flow pushed))
...
(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (y fixnum)
           (uvt (vector (cons* fixnum1 fixnum1 fixnum null) m))
           (x (vector fixnum n)))
    (let* ((extended-size (+ n 2))
           (graph (aprog1 (make-adlist-single-edge-graph extended-size)
                    (loop for (u v time) across uvt
                          do (graph-add-edge it u v :cost time)
                             (graph-add-edge it v u :cost time))
                    (let ((ext-from n)
                          (ext-to (1+ n)))
                      (graph-add-edge it ext-from ext-to :cost y)
                      (dotimes (i n)
                        (graph-add-edge it i ext-from :cost (aref x i))
                        (graph-add-edge it ext-to i :cost (aref x i))))))
           (distances (dijkstra graph)))
      (loop for i from 1 below n
            do (format t "~A " (aref distances i)))
      (terpri))))
