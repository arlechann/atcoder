...
(defun main ()
  (input* ((n fixnum)
           (hl (vector (cons* fixnum fixnum) n))
           (q fixnum)
           (tt (vector fixnum q)))
    (let* ((h (map 'vector #'car hl))
           (l (map 'vector #'cdr hl))
           (times-count (+ n q))
           (times-ht (aprog1 (make-hash-table)
                       (map-with-index 'vector
                                       #'(lambda (i time)
                                           (setf (gethash time it) i))
                                       (sort (concatenate 'vector
                                                          (map 'vector
                                                               #'(lambda (x) (* x 2))
                                                               l)
                                                          (map 'vector
           ...
           (l2 (map 'vector
                    #'(lambda (leave)
                        (gethash (* leave 2) times-ht))
                    l))
           (t2 (map 'vector
                    #'(lambda (time)
                        (gethash (1+ (* time 2)) times-ht))
                    tt))
           (cumsum (aprog1 (make-array (1+ times-count) :initial-element 0)
                     (dotimes (i n)
                       (let ((height (aref h i))
                             (leave (aref l2 i)))
                         (maxf (aref it leave) height)))
                     (loop for i from (1- times-count) downto 0
                           do (maxf (aref it i) (aref it (1+ i)))))))
      (dotimes (i q)
        (println (aref cumsum (aref t2 i)))))))
