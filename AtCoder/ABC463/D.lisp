...
(defun main ()
  (input* ((n fixnum)
           (k fixnum)
           (lr (vector (cons* fixnum fixnum) n)))
    (let ((sorted (sort (copy-seq lr) #'< :key #'cdr)))
      (flet ((validp (target)
               (loop with last = (cdr (aref sorted 0))
                     and count = 1
                     for i from 1 below n
                     for cloth = (aref sorted i)
                     for left = (car cloth)
                     and right = (cdr cloth)
                     if (<= (+ last target) left)
                       do (setf last right)
        ...
        (let ((result (meguru-method 0 1000000001 #'validp)))
          (println (if (zerop result) -1 result)))))))
