	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			let places = (0..self.s.len())
				...
				.collect_vec();
			let s = self.s.chars().rev().collect_vec();
			...
			for i in 0..s.len() {
				for j in 0..13 {
					if s[i] == '?' {
						for k in 0..10 {
							let next = (j + k * places[i]) % 13;
							dp[i + 1][next] += dp[i][j];
							dp[i + 1][next] %= MOD as usize;
						}
					} else {
						let c = s[i].to_digit(10).unwrap() as usize;
						let next = (j + c * places[i]) % 13;
						dp[i + 1][next] += dp[i][j];
						dp[i + 1][next] %= MOD as usize;
					}
				}
