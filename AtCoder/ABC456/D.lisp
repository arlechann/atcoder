...
(defun main ()
  (input* ((s string))
    (let* ((n (length s))
           (dp (aprog1 (make-array (list n 3) :initial-element (mint:make-mint 0))
                 (setf (aref it 0 (char-to-index (aref s 0))) (mint:make-mint 1)))))
      (loop for i from 1 below n
            for now = (char-to-index (aref s i))
            do (loop for j from 0 below 3
                     if (= j (char-to-index (aref s i)))
                       do (setf (aref dp i j)
                                (mint:mint+ 1
                                            (aref dp (1- i) j)
                                            (if (zerop (mint:mint-value (aref dp (1- i) (mod (1+ j) 3))))
                                                0 (aref dp (1- i) (mod (1+ j) 3)))
                                            (if (zerop (mint:mint-value (aref dp (1- i) (mod (+ j 2) 3))))
                                                0 (aref dp (1- i) (mod (+ j 2) 3)))))
                     else
                       do (setf (aref dp i j) (aref dp (1- i) j)))
            finally (println (mint:mint-value (mint:mint+ (aref dp (1- n) 0)
                                                          (aref dp (1- n) 1)
                                                          (aref dp (1- n) 2))))
                    ;; #?dp
            ))))
