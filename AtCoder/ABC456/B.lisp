...
(defun main ()
  (input* ((a (vector (vector fixnum 6) 3)))
    (let ((expect (vector 4 5 6))
          (result 0))
      (do-permutations (x expect)
        (let ((p 1))
          (dotimes (i 3)
            (setf p (* p (/ (count (aref x i) (aref a i)) 6))))
          (incf result p)))
      (print-double (coerce result 'double-float)))))
