...
(defun main ()
  (input* ((s string))
    (let* ((n (length s))
           (count (loop with count = 1
                        and result = nil
                        for i from 1 below n
                        for now-char = (aref s i)
                        and prev-char = (aref s (1- i))
                        if (char= now-char prev-char)
                          do (setf result (cons count result)
                                   count 1)
                        else
                          do (incf count)
                        finally (return (nreverse (cons count result))))))
      (println (mint:mint-value (reduce #'mint:mint+
                                        (nmap #'(lambda (c)
                                                  (mint:make-mint (triangular-number c)))
                                              count)
                                        :initial-value 0))))))
