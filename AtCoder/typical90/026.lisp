...
(defun make-graph (size edges &key (bidirectional t))
...
(defun solve (graph node prev redp collect)
  (when redp
    (funcall collect node))
  (mapc (lambda (next)
          (solve graph next node (not redp) collect))
        (remove-if (lambda (next) (= next prev))
                   (graph-neighbors graph node))))

(defun main ()
  (input* ((n fixnum)
           (ab (vector (list fixnum1 fixnum1) (1- n))))
    (let ((graph (make-graph n ab)))
      (format t "~{~A ~}~%"
              (mapcar #'1+
                      (take (longer (let ((result nil))
                                      (solve graph 0 -1 t
                                             (lambda (node) (push node result)))
                                      result)
                                    (let ((result nil))
                                      (solve graph 0 -1 nil
                                             (lambda (node) (push node result)))
