...
(defun solve-odd (n k a)
  (declare (ignore n))
  (let ((diffs-left (make-array (/2 k)
                                :initial-contents
                                (loop for i from 0 below (1- k) by 2
                                      collect (- (aref a (1+ i))
                                                 (aref a i)))))
        (diffs-right (make-array (/2 k)
                                 :initial-contents
                                 (loop for i from 1 below k by 2
                                       collect (- (aref a (1+ i))
                                                  (aref a i))))))
    (let ((cumsum-left (cumsum diffs-left))
          (cumsum-right (cumsum diffs-right)))
      (labels ((range-left (b)
                 (aref cumsum-left (/2 b)))
               (range-right (a)
                 (- (aref cumsum-right (/2 k))
                    (aref cumsum-right (/2 a)))))
        (loop for i from 0 below k
              minimize (+ (range-left i)
                          (range-right i)))))))

(defun solve-even (n k a)
  (declare (ignore n))
  (loop for i from 0 below k by 2
        sum (- (aref a (1+ i))
               (aref a i))))
...
(defun main ()
  (input* ((n fixnum)
           (k fixnum)
           (a (vector fixnum k)))
    (let ((result (funcall (if (oddp k) #'solve-odd #' solve-even)
                           n k a)))
      (format t "~A~%" result))))
