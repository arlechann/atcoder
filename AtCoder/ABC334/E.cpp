...
int pack_index(int row, int col) {
	return row * w + col;
}

int unpack_index_row(int index) {
	return index / w;
}

int unpack_index_col(int index) {
	return index % w;
}
...
int main() {
	...
	REP(i, h) {
		REP(j, w) {
			...
			if(i != h - 1 && s[i + 1][j] == '#') {
				uf.merge(pack_index(i, j), pack_index(i + 1, j));
			}
			if(j != w - 1 && s[i][j + 1] == '#') {
				uf.merge(pack_index(i, j), pack_index(i, j + 1));
			}
		}
	}

	int green_cc_count = uf.count_connected_components() - red_count;
	mint result = 0;
	REP(i, h) {
		REP(j, w) {
			...
			REP(k, 4) {
				...
				groups.insert(uf.root(pack_index(ny, nx)));
			}
			int groups_size = groups.size();
			if(groups_size == 0) {
				result += green_cc_count + 1;
			} else if(groups_size == 1) {
				result += green_cc_count;
			} else if(groups_size == 2) {
				result += green_cc_count - 1;
			} else if(groups_size == 3) {
				result += green_cc_count - 2;
			} else {
				result += green_cc_count - 3;
			}
		}
	}
	result /= red_count;
