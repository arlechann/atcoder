...
mint sum(ll y, ll x) {
	// cout << "sum(" << y << ", " << x << ")" << endl;
	mint x_even_num = (x + 1) / 2;
	mint x_odd_num = x / 2;
	mint x_sum_even = square(x_even_num);
	mint x_sum_odd = (x_odd_num + 1) * x_odd_num;
	mint n_sum = x_sum_even * ((y + 1) / 2) + x_sum_odd * (y / 2);
	// cout << "x_even_num: " << x_even_num << endl;
	// cout << "x_odd_num: " << x_odd_num << endl;
	// cout << "x_sum_even: " << x_sum_even << endl;
	// cout << "x_sum_odd: " << x_sum_odd << endl;
	// cout << "n_sum: " << n_sum << endl;

	mint y_even_num = (y + 1) / 2;
	mint y_odd_num = y / 2;
	mint y_sum_even = (y_even_num - 1) * y_even_num;
	mint y_sum_odd = square(y_odd_num);
	mint m_sum = y_sum_even * m * ((x + 1) / 2) + y_sum_odd * m * (x / 2);
	// cout << "y_even_num: " << y_even_num << endl;
	// cout << "y_odd_num: " << y_odd_num << endl;
	// cout << "y_sum_even: " << y_sum_even << endl;
	// cout << "y_sum_odd: " << y_sum_odd << endl;
...
mint imos(ll y1, ll x1, ll y2, ll x2) {
	mint ret = sum(y1, x1);
	ret -= sum(y1, x2 - 1);
	ret -= sum(y2 - 1, x1);
	ret += sum(y2 - 1, x2 - 1);
...
int main() {
	...
	vector<mint> result(q);
	REP(i, q) {
		result[i] = imos(b[i], d[i], a[i], c[i]);
	}
