...
int y_pred(int left, int middle) {
	int t = question(left + 1, middle, 1, n);
	if(t == -1) {
		return -1;
	}
	if(t == middle - left) {
...
int x_pred(int left, int middle) {
	int t = question(1, n, left + 1, middle);
	if(t == -1) {
		return -1;
	}
	if(t == middle - left) {
...
int main() {
	...
	int y;
	int y_left = 0, y_right = n;
	while((y_right - y_left) != 1) {
		int middle = (y_left + y_right) / 2;
		if(y_pred(y_left, middle)) {
			y_right = middle;
		} else {
			y_left = middle;
		}
	...
	int x;
	int x_left = 0, x_right = n;
	while((x_right - x_left) != 1) {
		int middle = (x_left + x_right) / 2;
		if(x_pred(x_left, middle)) {
			x_right = middle;
		} else {
			x_left = middle;
		}
