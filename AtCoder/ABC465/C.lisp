...
(defun main ()
  (input* ((n fixnum)
           (s string))
    (loop with result = (make-array n :initial-element 0)
          and front = 0
          and back = (1- n)
          and dir = nil
          for i from (1- n) downto 0
          for is-flip = (char= (aref s i) #\o)
          if is-flip
            if dir
              do (setf (aref result back) (1+ i))
                 (decf back)
                 (setf dir (not dir))
            else
              do (setf (aref result front) (1+ i))
                 (incf front)
                 (setf dir (not dir))
          else
            if dir
