...
(defun main ()
  (input* ((n fixnum)
           ...
           (s string)
           (x (vector fixnum n)))
    (let* ((left-ants-count (count #\0 s))
           (right-ants-count (count #\1 s))
           (left-ants (let ((vec (make-array left-ants-count :initial-element 0))
                            (vec-index 0))
                        (dotimes (i n (sort vec #'<))
                          (when (char= (aref s i) #\0)
                            (setf (aref vec vec-index) (aref x i))
                            (incf vec-index)))))
           (right-ants (let ((vec (make-array right-ants-count :initial-element 0))
                             (vec-index 0))
                         (dotimes (i n (sort vec #'<))
                           (when (char= (aref s i) #\1)
                             (setf (aref vec vec-index) (aref x i))
                             (incf vec-index))))))
      (let ((result (loop for start across right-ants
                          sum (let ((a (meguru-method left-ants-count -1
                                                      (lambda (index)
                                                        (if (= index left-ants-count) t
                                                            (> (aref left-ants index)
                                                               (+ start (2* tt)))))))
                                    (b (meguru-method left-ants-count -1
                                                      (lambda (index)
                                                        (if (= index left-ants-count) t
                                                            (> (aref left-ants index)
                                                               start))))))
