    ...
    (let* ((splitted (split-string st))
      ...
      (dotimes (w (1- s-len))
        (let ((w (1+ w)))
          (dotimes (c w)
            (let ((result (dvector)))
              (loop for i from c by w
                    while (< i s-len)
                    do (vector-push-extend (aref s i) result))
              ;(format t "result: ~A~%" (coerce result 'string))
              (when (string= (coerce result 'string) tt)
                (format t "Yes~%")
                (return-from main))))))
      (format t "No~%"))))

(defun test ()
  (test-case* "atcoder toe" "Yes" "beginner r" "No" "verticalreading agh" "No"))
