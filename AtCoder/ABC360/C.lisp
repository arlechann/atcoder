...
(defconstant +inf+ 10000000001)

(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum1 n))
           (w (vector fixnum n)))
    (let ((sum-weight (sum w))
          (max-weights (make-array n :initial-element 0)))
      (loop for box across a
            for weight across w
            when (maxp weight (aref max-weights box))
              do (setf (aref max-weights box) weight))
      (let ((result (- sum-weight (sum max-weights))))
        (format t "~A~%" result)))))
