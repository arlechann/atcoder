...
(defun lower-bound (vector element &rest args &key (compare #'<=) (start 0) end)
  (declare (ignore end))
  (apply #'find-bound vector element :compare compare :start start args))
...
(defconstant +d-max+ 200000000)

(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (a (vector fixnum n))
           (bk (vector (cons* fixnum fixnum) q)))
    (sortf a #'<)
    (let* ((result (make-array q)))
      (dotimes (i q)
        (let* ((bk (aref bk i))
               (b (car bk))
               (k (cdr bk)))
          (setf (aref result i)
                (meguru-method +d-max+ -1
                               (lambda (d)
                                 (>= (- (upper-bound a (+ b d))
                                        (lower-bound a (- b d)))
                                     k))))))
