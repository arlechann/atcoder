...
(defun main ()
  (input* ((n fixnum)
    ...
    (let ((sorted (stable-sort (sort lr #'> :key #'car) #'< :key #'cdr))
          (result 0))
      (let ((max-left 0)
            (prev-right1 0)
            (prev-right2 0))
        (do-seq (range sorted)
          (let ((left (car range))
                (right (cdr range)))
            (incf result (choice2 (- right (max left prev-right1))))
            (incf result (* (max (- (max left prev-right1) (max max-left prev-right2)) 0)
                            (- right (max left prev-right1))))
            (maxf max-left left)
            (setf prev-right2 prev-right1 prev-right1 right)))
        (incf result (choice2 (- m prev-right1))))
      (println result))))
