...
(defun main ()
  (input* ((n fixnum)
    ...
    (labels ((in-board-p (row col)
               (and (<= 0 row (1- n))
                    (<= 0 col (1- n))))
             (each-move (row col fn)
               (let ((dy '(2 1 -1 -2 -2 -1 1 2))
                     (dx '(1 2 2 1 -1 -2 -2 -1)))
                 (do-seq* ((dy dx) (dy dx))
                   (let ((ny (+ row dy))
                         (nx (+ col dx)))
                   (when (in-board-p ny nx)
                     (funcall fn ny nx)))))))
      (let ((not-movable-cells (make-hash-table :test #'equal)))
        (dotimes (i m)
          (let* ((piece (aref ab i))
                 (row (car piece))
                 (col (cdr piece)))
            (setf (gethash piece not-movable-cells) t)
            (each-move row col
                       (lambda (row col)
                         (setf (gethash (cons row col) not-movable-cells) t)))))
        (println (- (square n) (hash-table-count not-movable-cells)))))))
