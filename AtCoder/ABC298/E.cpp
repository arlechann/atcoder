...
int n, p, q;
mint memo[N_MAX][N_MAX][2];
int used[N_MAX][N_MAX][2];

mint dp(int a, int b, int is_takahashi) {
	if(a >= n - 1) {
		return 1;
	}
	if(b >= n - 1) {
	...
	mint& ret = memo[a][b][is_takahashi];
	if(used[a][b][is_takahashi] != 0) {
		return ret;
	}
	used[a][b][is_takahashi] = 1;
