...
int main() {
	...
	// cout << endl;
	REP(i, m) {
		ll ball = st.find(0, b[i] + 1);
		st.update(b[i], -ball, add);
		if(b[i] != n - 1) {
			st.update(b[i] + 1, ball, add);
		}
		ll loop = ball / n, rest = ball % n;
		st.update(0, loop, add);
		if(rest != 0) {
			if(b[i] == n - 1) {
				st.update(0, 1, add);
				st.update(rest, -1, add);
			} else if(b[i] + rest == n - 1) {
				st.update(b[i] + 1, 1, add);
			} else if(b[i] + rest < n - 1) {
				st.update(b[i] + 1, 1, add);
				st.update(b[i] + rest + 1, -1, add);
			} else {
				int x = n - (b[i] + 1);
				st.update(b[i] + 1, 1, add);
				st.update(0, 1, add);
				st.update(rest - x, -1, add);
			}
		}
		// cout << "i: " << i << " => ";
		// REP(i, n) {
