...
VLL dijkstra(const vector<vector<pair<int, ll>>>& edges, int s, int g) {
	int n = edges.size();
	VI used(n, 0);
...
int main() {
	...
	REP(i, n - 1) {
		edges[i].push_back(make_pair(i + 1, a[i]));
		if(x[i] != i) {
			edges[i].push_back(make_pair(x[i], b[i]));
		}
	}
	auto distances = dijkstra(edges, 0, n - 1);
