...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> String {
			if self.xy.iter().any(|&(x, y)| {
				calc((self.txa, self.tya), (self.txb, self.tyb), (x, y)) <= (self.t * self.v) as f64
			}) {
				"YES".to_string()
			} else {
				"NO".to_string()
	...
	fn distance((a, b): (isize, isize), (x, y): (isize, isize)) -> f64 {
		(((a - x).abs() * (a - x).abs() + (b - y).abs() * (b - y).abs()) as f64).sqrt()
	}

	fn calc((txa, tya): (isize, isize), (txb, tyb): (isize, isize), (x, y): (isize, isize)) -> f64 {
		distance((txa, tya), (x, y)) + distance((txb, tyb), (x, y))
	}
}
