...
(defun main ()
  (input* ((n fixnum)
           ...
           (a (vector fixnum1 m))
           (b (vector fixnum1 m)))
    (let ((uf (make-union-find (* n 2))))
      (dotimes (i m)
        (let ((ai (aref a i))
              (bi (aref b i)))
          (setf uf (union-find-merge uf ai (+ bi n)))
          (setf uf (union-find-merge uf (+ ai n) bi))))
      (nlet rec ((i 0) (result t))
        (if (or (not result) (= i n))
            (format t "~A~%" (if result "Yes" "No"))
            (rec (1+ i) (not (union-find-unite-p uf i (+ i n)))))))))
