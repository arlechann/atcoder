...
bool dfs(const VI2D& edges,
		 ...
		 int prev_value) {
	if(used[node]) {
		return prev_value != node_values[node];
	}

	used[node] = 1;
	node_values[node] = -prev_value;
	...
	EACH(next, edges[node]) {
		...
		bool ret = dfs(edges, node_values, used, next, node, node_values[node]);
		if(!ret) {
			return false;
		}
	}
...
int main() {
	...
	REP(i, n) {
		...
		bool result = dfs(edges, node_values, used, i, -1, -1);
		if(!result) {
			cout << "No" << endl;
			return 0;
		}
