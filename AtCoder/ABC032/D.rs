...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> u64 {
			if self.n <= 30 {
				meet_in_middle(self.n, self.k, &self.vw)
			} else if self.vw.iter().all(|&(_, w)| w <= 1000) {
				dp_value(self.n, self.k, &self.vw)
			} else {
				dp_weight(self.n, self.k, &self.vw)
			}
		}
	}
	...
	fn meet_in_middle(n: usize, k: u64, vw: &[(u64, u64)]) -> u64 {
		let harf = (n + 1) / 2;
		let mut wv = vw.iter().map(|&(v, w)| (w, v)).collect_vec();
		wv.sort();
		let head = {
			let h = &wv[..harf];
			let h_len = h.len();
			let mut head = vec![];
			for set in 0..(1 << h_len) {
				let (mut w, mut v) = (0, 0);
				for i in 0..h_len {
					if set & (1 << i) != 0 {
						w += h[i].0;
		...
		let mut result = 0;
		for &(hw, hv) in head.iter() {
			if hw > k {
				continue;
			}
			let index = bin_search(0isize, tail_len as isize, |i| {
				let (tw, tv) = tail[i as usize];
				hw + tw <= k
			}) as usize;
			for i in 0..=index {
				result = max(result, hv + tail[i].1);
			}
		}
		result
	}

	fn dp_value(n: usize, k: u64, vw: &[(u64, u64)]) -> u64 {
		let k = k as usize;
		let mut dp = vec![vec![0; k + 1]; n + 1];
		for i in 0..n {
			for j in 0..k {
				let v = vw[i].0;
				let w = vw[i].1 as usize;
				if j + 1 >= w {
	...
	fn dp_weight(n: usize, k: u64, vw: &[(u64, u64)]) -> u64 {
		const INF: u64 = 1_000_000_000_000_000_000;
		let v_max = vw.iter().map(|&(v, _)| v as usize).sum::<usize>();
		let mut dp = vec![vec![INF; v_max + 1]; n + 1];
		dp[0][0] = 0;
		for i in 0..n {
			dp[i + 1][0] = 0;
			for j in 0..v_max {
				let v = vw[i].0 as usize;
				let w = vw[i].1;
				if j + 1 >= v {
					dp[i + 1][j + 1] = min(dp[i][j + 1], dp[i][j + 1 - v] + w);
				} else {
					dp[i + 1][j + 1] = dp[i][j + 1];
				}
			}
