...
VI c;
vector<pair<int, int>> costs(C_NUM);
int memo[C_NUM][N_MAX + 1];
int dp(int c, int rest) {
	int& ret = memo[c][rest];
	if(rest <= 0) {
		return ret = 0;
	}
	...
	RRANGE(i, c, costs.size()) {
		int x = dp(i, rest - costs[c]);
		chmax(ret, x);
	}
	return ret += 1;
}

void rev(string& result, int c, int rest) {
	if(rest <= 0) {
		return;
	}
	RANGE(i, c, costs.size()) {
		if(i == costs.size() - 1) {
			return result += '0' + c[costs[c].second]
		}
	}
}
...
int main() {
	...
	costs = vector<pair<int, int>>(C_NUM);
	REP(i, c) {
		costs[i] = make_pair(c[i], -i);
	}
	sort(ALL(costs));
	costs.erase(unique(ALL(costs)), costs.end());
	REP(i, c) {
		costs[i].second = -costs[i].second;
	}
