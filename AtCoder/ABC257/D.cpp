...
int visit(VI& used, ll s, int node) {
	used[node] = 1;
	int ret = 1;
	REP(i, edges[node].size()) {
		ll d = edges[node][i].first;
		...
		ret += visit(used, s, next);
	}
	return ret;
}

bool is_valid(int root, ll s) {
	VI used(n, 0);
	return visit(used, s, root) == n;
}
...
int main() {
	...
	REP(i, n) {
		REP(j, n) {
			...
			ll d = diff(x[i], x[j]) + diff(y[i], y[j]);
			edges[i].push_back(make_pair(d, j));
		}
		sort(ALL(edges[i]));
	}

	ll result = bin_search<ll>(4000000100, 0, [&](ll s) {
		REP(root, n) {
			if(is_valid(root, s)) {
				return true;
			}
