...
int main() {
	int n,m;ll p;cin>>n>>m>>p;VLL a(n), b(m);EACH(e,a){cin>>e;}EACH(e,b){cin>>e;}

	sort(RALL(b));
	VLL cumsum(m + 1, 0);
	REP(i, m) {cumsum[i + 1] += cumsum[i] + b[i];}

	ll result = 0;
	REP(i, n) {
		ll a_p = a[i];
		// ap + bp > p;
		int index_b_bt_p = bin_search<int>(-1, m, [&](int index) {
			if(index == -1) {return true;}
			return a_p + b[index] > p;
		});
		int num_b_bt_p = index_b_bt_p + 1;
		result += p * num_b_bt_p + a_p * (m - num_b_bt_p) + cumsum[m] - cumsum[num_b_bt_p];
	}
