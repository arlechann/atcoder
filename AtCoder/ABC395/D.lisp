...
(defun main ()
  (input* ((n fixnum)
           (q fixnum))
    (let ((pigeons (aprog1 (make-array n :initial-element 0)
                     (dotimes (i n)
                       (setf (aref it i) i))))
          (positions (aprog1 (make-array n :initial-element 0)
                       (dotimes (i n)
                         (setf (aref it i) i))))
          (now (aprog1 (make-array n :initial-element 0)
                 (dotimes (i n)
                   (setf (aref it i) i)))))
      (dotimes (k q)
        (let ((type (read)))
          ;;#?type #?pigeons #?positions
          (case type
            (1 (let ((a (1- (read)))
                     (b (1- (read))))
                 (setf (aref pigeons a) (aref positions b))))
            (2 (let* ((a (1- (read)))
                      (b (1- (read)))
                      (pa (aref positions a))
                      (pb (aref positions b)))
                 (rotatef (aref positions a) (aref positions b))
                 (rotatef (aref now pa) (aref now pb))))
            (3 (let* ((a (1- (read)))
                      (p (aref pigeons a))
                      (result (aref now p)))
                 (println (1+ result))))))))))
