...
(defparameter *a-max* 1000000)

(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum1 n)))
    (let ((last-indexes (make-array (1+ *a-max*) :initial-element nil))
          (result nil))
      (dotimes (i n)
        (let* ((ai (aref a i))
               (last-index (aref last-indexes ai)))
          (when last-index
            (if result
                (minf result (1+ (- i last-index)))
                (setf result (1+ (- i last-index)))))
          (setf (aref last-indexes ai) i)))
      (println (or result -1)))))
