...
(defun solve (n level board)
  (let ((c (if (evenp level) #\# #\.))
        (end (- n level)))
    (when (<= end level)
      (return-from solve))
    (loop for i from level below end
          do (setf (aref board level i) c
                   (aref board i level) c
                   (aref board (1- end) i) c
                   (aref board i (1- end)) c))
    (solve n (1+ level) board)))

(defun main ()
  (input* ((n fixnum))
    (let ((board (make-array (list n n) :initial-element #\?)))
      (solve n 0 board)
      (dotimes (i n)
        (dotimes (j n)
          (write-char (aref board i j)))
        (terpri)))))
