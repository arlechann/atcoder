...
using Cost = pair<int, ll>;
std::vector<Cost> dijkstra(const VI2D& edges, const VLL& values, int start) {
	int n = edges.size();
	VI used(n, 0);
	vector<Cost> distances(n, make_pair(INF / 2, 0));

	distances[start] = make_pair(0, -values[start]);
	priority_queue<pair<Cost, int>,
				   vector<pair<Cost, int>>,
				   greater<pair<Cost, int>>>
		pq;
	pq.push(make_pair(make_pair(0, -values[start]), start));
	while(!pq.empty()) {
		...
		for(auto&& next : edges[node]) {
			Cost alt = make_pair(cost.first + 1, cost.second - values[next]);
			if(alt < distances[next]) {
				distances[next] = alt;
				pq.push(make_pair(alt, next));
			}
		}
	}
	return distances;
...
int main() {
	...
	vector<vector<Cost>> distances(n);
	REP(i, n) {
		distances[i] = dijkstra(edges, a, i);
	}

	REP(i, q) {
		auto cost = distances[u[i]][v[i]];
		int flight = cost.first;
		ll value = -cost.second;
		if(flight == INF / 2) {
			cout << "Impossible" << endl;
		} else {
			cout << flight << " " << value << endl;
		}
	}
	return 0;
}
