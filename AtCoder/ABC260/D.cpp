...
int main() {
	...
	SegmentTree<int> mex(
		VI(n, INF), [&](int a, int b) { return min(a, b); }, INF);
	VI under(n, -1);
	VI stack_size(n, 0);
	VI result(n, -1);
	...
	REP(i, n) {
		int card = p[i];
		int mex_num = mex.find(card, n);
		mex.update(card, card);
		if(mex_num != INF) {
			mex.update(mex_num, INF);
			under[card] = mex_num;
			stack_size[card] = stack_size[mex_num] + 1;
		} else {
			stack_size[card] = 1;
		}

		if(stack_size[card] == k) {
			mex.update(card, INF);
			int now = card;
			while(now != -1) {
				result[now] = i + 1;
