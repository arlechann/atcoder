...
int main() {
	...
	REP(i, n) {
		students[i] = make_pair(i, make_pair(a[i], b[i]));
	}

	VI result(n, 0);

	stable_sort(ALL(students),
				[](auto a, auto b) { return a.first < b.first; });
	stable_sort(RALL(students),
				[](auto a, auto b) { return a.second.first < b.second.first; });
	REP(i, n) {
		if(x <= 0) {
			break;
		}
		result[students[i].first] = 1;
		x--;
	}

	stable_sort(ALL(students),
				[](auto a, auto b) { return a.first < b.first; });
	stable_sort(RALL(students), [](auto a, auto b) {
