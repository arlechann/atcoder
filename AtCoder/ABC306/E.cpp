...
int main() {
	...
	REP(i, q) {
		ll prev = a[x[i]];
		a[x[i]] = y[i];
		ll min_higher = *higher.begin();
		ll max_lower = k == n ? -1 : *lower.rbegin();
		if(prev >= min_higher && y[i] >= max_lower) {
			higher.erase(higher.find(prev));
			higher.insert(y[i]);
			result -= prev;
			result += y[i];
		} else if(prev >= min_higher && y[i] < max_lower) {
			higher.erase(higher.find(prev));
			higher.insert(max_lower);
			lower.erase(lower.find(max_lower));
			lower.insert(y[i]);
			result -= prev;
			result += max_lower;
		} else if(prev <= max_lower && y[i] > min_higher) {
			lower.erase(lower.find(prev));
			lower.insert(min_higher);
			higher.erase(higher.find(min_higher));
			higher.insert(y[i]);
			result -= min_higher;
			result += y[i];
		} else {
