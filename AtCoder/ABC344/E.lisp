...
(defun main ()
  (let* ((n (read))
         (a (loop for i from 0 below n
                  collect (read)))
         (q (read)))
    (let ((ht (make-hash-table)))
      (nlet rec ((a a) (prev nil))
        (unless (null a)
          (setf (gethash (car a) ht) (cons a prev))
          (rec (cdr a) a)))
      (labels ((insert (x y)
                 (let* ((pair (gethash x ht))
                        (cell (car pair))
                        (new-cell (cons y (cdr cell))))
                   (setf (cdr cell) new-cell)
                   (setf (gethash y ht) (cons new-cell cell))
                   (unless (null (cdr new-cell))
                     (setf (gethash (cadr new-cell) ht)
                           (cons (cdr new-cell) new-cell)))))
               (remove-x (x)
                 (let* ((pair (gethash x ht))
                        (cell (car pair))
                        (prev (cdr pair)))
