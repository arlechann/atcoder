...
(defun main ()
  (input* ((tstr string)
    ...
    (let ((a (make-array n :element-type 'fixnum))
          (s (make-array n :element-type 'vector)))
      (dotimes (i n)
        (let ((len (read)))
          (setf (aref a i) len
                (aref s i)
      ...
      (format t "~A~%" (solve tstr n a s)))))

(defun solve (tstr n a s)
  (let* ((memo (make-hash-table :test 'equal))
         (result (dp memo tstr n a s 0 0 0)))
    (if (= result +inf+)
        -1
        result)))

(defun tstr-eq (tstr end str)
  (loop for i from 0 below (length str)
        do (unless (and (< (+ end i) (length tstr))
                        (char= (aref tstr (+ end i))
                               (aref str i)))
             (return-from tstr-eq nil)))
  t)

(defun dp (memo tstr n a s index pick end)
  ;(format t "index: ~A, pick: ~A, end: ~A~%" index pick end)
  (let ((val (gethash (list index pick end) memo nil)))
    (if val
        val
        (setf (gethash (list index pick end)
                       memo)
              (cond ((= end (length tstr)) 0)
                    ((> end (length tstr)) +inf+)
                    ((>= index n) +inf+)
                    ((>= pick (length (aref s index)))
                     (dp memo tstr n a s (1+ index) 0 end))
                    (t (let ((str (aref (aref s index) pick)))
                         (min (if (tstr-eq tstr end str)
                                  (1+ (dp memo tstr n a s (1+ index) 0 (+ end (length str))))
                                  +inf+)
                              (dp memo tstr n a s index (1+ pick) end)))))))))
(defun test ()
  (test-case "abcde
3
3 ab abc abcd
