...
int main() {
	...
	unordered_map<int, unordered_map<int, ll>> edges;
	ll sum_cost = 0;
	REP(i, n) {
		edges[0][i + 1] = a[i];
		sum_cost += a[i];
	...
	REP(i, n) {
		int next = (i + 1) % n;
		edges[min(i, next) + 1][max(i, next) + 1] += b[i];
		sum_cost += b[i];
	}

	auto dp = make_vector({n + 1, 2, 2}, 0LL);
	// dp[i][iの色][1の色]
	dp[0][0][0] = dp[0][0][1] = -INFLL;
	dp[0][1][0] = dp[0][1][1] = -INFLL;
