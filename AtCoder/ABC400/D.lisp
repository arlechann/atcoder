...
(defun bfs01 (maze starty startx goaly goalx)
  (loop with h = (array-dimension maze 0)
        and w = (array-dimension maze 1)
        with distances = (make-array (list h w) :element-type 'fixnum :initial-element *inf*)
        and used = (make-array (list h w) :element-type 'boolean :initial-element nil)
        and deque = (make-deque)
        initially (setf (aref distances starty startx) 0)
                        ;(aref used starty startx) t)
                  (deque-push-front deque (cons starty startx))
        until (deque-empty-p deque)
        do ; #?(deque::deque-buffer deque)
           (let* ((node (prog1 (deque-peak-front deque)
                          (deque-pop-front deque)))
                  (nodey (car node))
                  (nodex (cdr node)))
             ; #?nodey #?nodex #?(aref distances nodey nodex)
             (when (and (= nodey goaly)
                        (= nodex goalx))
               (return-from bfs01 distances))
             (unless (aref used nodey nodex)
               (setf (aref used nodey nodex) t)
               (loop for dy in '(0 1 0 -1)
                     for dx in '(1 0 -1 0)
                     do (let ((nexty (+ nodey dy))
                              (nextx (+ nodex dx)))
                          (when (and (< -1 nexty h)
                                     (< -1 nextx w)
                                     (not (aref maze nexty nextx))
                                     (< (aref distances nodey nodex) (aref distances nexty nextx)))
                            (setf (aref distances nexty nextx) (aref distances nodey nodex))
                            (deque-push-front deque (cons nexty nextx)))))
               (loop for dy in '(0 1 0 -1 0 2 0 -2)
                     for dx in '(1 0 -1 0 2 0 -2 0)
                     do (let ((nexty (+ nodey dy))
                              (nextx (+ nodex dx)))
                          (when (and (< -1 nexty h)
                                     (< -1 nextx w)
                                     (not (aref used nexty nextx))
                                     (< (1+ (aref distances nodey nodex)) (aref distances nexty nextx)))
                            (setf (aref distances nexty nextx) (1+ (aref distances nodey nodex)))
                            (deque-push-back deque (cons nexty nextx)))))))
        finally (return distances)))
...
(defun main ()
  (input* ((h fixnum)
           ...
           (c fixnum1)
           (d fixnum1))
    (let* ((maze (aprog1 (make-array (list h w) :element-type 'boolean :initial-element nil)
                   (dotimes (i h)
                     (dotimes (j w)
                       (setf (aref it i j)
                             (char= (aref (aref s i) j) #\#))))))
           (distances (bfs01 maze a b c d)))
      (println (aref distances c d)))))
