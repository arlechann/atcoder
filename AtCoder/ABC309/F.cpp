...
struct Box {
	...
	Box() {}
	Box(int low, int mid, int hi) : low(low), mid(mid), hi(hi) {}
...
int main() {
	...
	VLL low(n), mid(n), hi(n);
	REP(i, n) {
		low[i] = min({h[i], w[i], d[i]});
		hi[i] = max({h[i], w[i], d[i]});
		mid[i] = h[i] + w[i] + d[i] - low[i] - hi[i];
	}

	VLL sorted_low = low;
	sort(ALL(sorted_low));
	unordered_map<ll, int> cmp_low;
	REP(i, n) {
		comp_low[sorted_low[i]] = i;
	}

	VLL sorted_mid = mid;
	sort(ALL(sorted_mid));
	unordered_map<ll, int> cmp_mid;
	REP(i, n) {
		comp_mid[sorted_mid[i]] = i;
	}

	VLL sorted_hi = hi;
	sort(ALL(sorted_hi));
	unordered_map<ll, int> cmp_hi;
	REP(i, n) {
		comp_hi[sorted_hi[i]] = i;
	}

	vector<Box> boxes(n);
	REP(i, n) {
		boxes[i].low = cmp_low[low[i]];
		boxes[i].mid = cmp_mid[mid[i]];
		boxes[i].hi = cmp_hi[hi[i]];
	}
	sort(ALL(boxes));
	return 0;
}
