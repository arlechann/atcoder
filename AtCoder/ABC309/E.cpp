...
void solve(const VI2D& children, VI& ins_gens, VI& has_ins, int person) {
	if(ins_gens[person] > 0) {
		has_ins[person] = 1;
	}
	EACH(child, children[person]) {
		chmax(ins_gens[child], ins_gens[person] - 1);
		if(ins_gens[person] > 0) {
			has_ins[child] = 1;
		}
		solve(children, ins_gens, has_ins, child);
	}
}
...
int main() {
	...
	VI has_ins(n, 0);
	REP(i, m) {
		chmax(ins_gens[x[i]], y[i]);
		has_ins[x[i]] = 1;
	}
	solve(children, ins_gens, has_ins, 0);
