...
(defgeneric graph-has-neighbor-p (graph node))
...
(defmethod graph-has-neighbor-p ((graph <adlist-graph>) node)
  (not (null (graph-neighbors graph node))))
...
(defun make-bar (i rcl) (cons i rcl))
(defun bar-long (bar) (fourth bar))
(defun bar-row (bar) (second bar))
(defun bar-col (bar) (third bar))
(defun bar-id (bar) (first bar))
(defun bar-left (bar) (bar-col bar))
(defun bar-right (bar) (+ (bar-col bar) (bar-long bar)))

(defun fall-bars (root-nodes graph bottom)
  (let ((result (make-array (graph-size graph) :initial-element -1)))
    (labels ((rec (node)
               (let ((val (aref result node)))
                 (when (/= val -1)
                   (return-from rec val)))
               (let ((neighbors (graph-neighbors graph node)))
                 (setf (aref result node)
                       (if (null neighbors)
                           bottom
                           (1- (loop for neighbor in neighbors
                                     minimize (rec (edge-to neighbor)))))))))
      (dolist (node root-nodes result)
        (rec node)))))
...
(defun main ()
  (input* ((h fixnum)
           (w fixnum)
           (n fixnum)
           (rcl (vector (cons* fixnum1 fixnum1 fixnum nil) n)))
    (declare (ignorable h w))
    (let* ((bars (aprog1 (make-array n)
                   (dotimes (i n)
                     (setf (aref it i)
                           (make-bar i (aref rcl i))))))
           (bars-sorted-left (sort (copy-seq bars)
                                  #'(lambda (a b)
                                      (< (bar-left a) (bar-left b)))))
           (graph (make-adlist-graph n))
           (indegrees (make-array n :initial-element 0)))
      (loop with last = 0
            for i below n
            for bar = (aref bars-sorted-left i)
            do (let ((end (meguru-method n i
                                         #'(lambda (k)
                                             (or (= k n) 
                                                 (>= (bar-left (aref bars-sorted-left k))
                                                     (bar-right bar)))))))
                 (loop for k from (1+ i) below end
                       for bar2 = (aref bars-sorted-left k)
                       if (> (bar-row bar) (bar-row bar2))
                         do (graph-add-edge graph (bar-id bar2) (bar-id bar))
                            (incf (aref indegrees (bar-id bar)))
                       else 
                         do (graph-add-edge graph (bar-id bar) (bar-id bar2))
                            (incf (aref indegrees (bar-id bar2))))))
      (let* ((root-nodes (loop for i below n
                               when (zerop (aref indegrees i))
                                 collect i))
             (result (fall-bars root-nodes graph h)))
        (print-sequence result *standard-output* :spacer #\lf)))))
