...
(defun solve (n m cont)
  (labels ((rec (i lst)
             (when (= i n)
               (funcall cont (reverse lst))
               (return-from rec))
             (let ((prev (car lst)))
               (if (null prev)
                   (loop for k from 1 to (- m (* 10 (1- n)))
                         do (rec (1+ i) (cons k lst)))
                   (loop for k from (+ prev 10) to (- m (* 10 (- n i 1)))
                         do (rec (1+ i) (cons k lst)))))))
    (rec 0 nil)))
    ...
    (let ((count 0)
      ...
      (solve n m
             #'(lambda (lst)
                 ...
                 (push lst result)))
      (println count)
      (loop for lst in (nreverse result)
            do (print-sequence lst)))))
