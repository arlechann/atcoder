...
int main() {
	...
	int xtop = INF, xbottom = 0, xleft = INF, xright = 0;
	REP(i, hx) {
		REP(j, wx) {
			if(x[i][j] == '#') {
				chmin(xtop, i);
	...
	int hy = 30, wy = 30;
	vector<string> temp(hy, string(wy, '.'));
	REP(ai, hy - ha + 1) {
		REP(aj, wy - wa + 1) {
			REP(bi, hy - hb + 1) {
				REP(bj, hx - wb + 1) {
					vector<string> y = temp;
					REP(i, ha) {
						REP(j, wa) {
							chmin(y[ai + i][aj + j], a[i][j]);
						}
					}
					REP(i, hb) {
						REP(j, wb) {
							chmin(y[bi + i][bj + j], b[i][j]);
						}
					}
					int ytop = INF, ybottom = 0, yleft = INF, yright = 0;
					REP(i, hy) {
						REP(j, wy) {
							if(y[i][j] == '#') {
								chmin(ytop, i);
					...
					bool is_equal = true;
					REP(i, xbottom - xtop + 1) {
						REP(j, xright - xleft + 1) {
							if(y[ytop + i][yleft + j] !=
							   x[xtop + i][xleft + j]) {
								is_equal = false;
								break;
