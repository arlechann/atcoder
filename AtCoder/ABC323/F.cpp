...
int main() {
	...
	ll result = INFLL;
	REP(i, 4) {
		ll xp = dx[i] + xb, yp = dy[i] + yb;

		ll start_to_cargo = diff(xa, xb) + diff(ya, yb);
		ll start_to_push = diff(xa, xp) + diff(ya, yp);

		ll cargo_to_goal = diff(xb, xc) + diff(yb, yc);
		ll push_to_goal = diff(xp, xc) + diff(yp, yc);

		if(cargo_to_goal > push_to_goal) {
			continue;
		}
		ll dist = start_to_push + push_to_goal - 1;
		if((xp == xa && start_to_push > start_to_cargo) ||
		   (yp == ya && start_to_push > start_to_cargo)) {
			dist += 2;
		} else {
		}
		if((xp == xb && xp == xc) || (yp == yb && yp == yc)) {
		} else {
			dist += 2;
		}
		chmin(result, dist);
