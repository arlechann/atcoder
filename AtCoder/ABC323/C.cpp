...
int main() {
	...
	VI max_left(n + 1, -1);
	REP(i, n) {
		max_left[i + 1] = max(max_left[i], scores[i]);
	}
	VI max_right(n + 1, -1);
	RREP(i, n) {
		max_right[i] = max(max_right[i + 1], scores[i]);
	}

	REP(i, n) {
		int target = max(max_left[i], max_right[i + 1] + 1);

		auto dp = make_vector({m + 1, target + 1}, INF);
		dp[0][0] = 0;
		REP(j, m) {
			REP(k, target + 1) {
				if(s[i][j] == 'x') {
					chmin(dp[j + 1][min(k + a[j], target)], dp[j][k] + 1);
					chmin(dp[j + 1][k], dp[j][k]);
				} else {
					chmin(dp[j + 1][min(k + a[j], target)], dp[j][k]);
				}
			}
		}
