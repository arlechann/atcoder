...
(defun dfs (n k char-counts index rest-chars used unused)
  (when (= index (length char-counts))
    (return-from dfs
      (if (zerop rest-chars)
          (* (/ (fact (floor k 2))
                (reduce (lambda (acc x) (* acc (fact (floor x 2)))) used :initial-value 1))
             (/ (fact (1+ (- n k)))
                (reduce (lambda (acc x) (* acc (fact x))) unused :initial-value 1)))
          0)))
  (let ((count (aref char-counts index))
        (ret 0))
    (if (oddp rest-chars)
        (dotimes (i (1+ (min count rest-chars)))
          (incf ret (dfs n k char-counts
                         (1+ index)
                         (- rest-chars i)
                         (cons i used)
                         (cons (- count i) unused))))
        (dotimes (i (1+ (floor (min count rest-chars) 2)))
          (incf ret (dfs n k char-counts
                         (1+ index)
                         (- rest-chars (* i 2))
                         (cons (* i 2) used)
                         (cons (- count (* i 2)) unused)))))
    ret))
...
(defun main ()
  (input* ((n fixnum)
           (k fixnum)
           (s string))
    (let ((char-counts (make-array (count-alphabet) :initial-element 0)))
      (do-seq (c s)
        (incf (aref char-counts (char-to-index c))))
      (format t "~A~%" (- (/ (fact n)
                             (reduce (lambda (acc x) (* acc (fact x)))
                                     char-counts
                                     :initial-value 1))
                          (dfs n k char-counts 0 k nil nil))))))
