...
(defun bfs (maze h w sy sx gy gx)
  (let* ((inf 1000000000)
         (dydxs #((0 . 1) (1 . 0) (0 . -1) (-1 . 0)))
         (distances (make-array (list h w)
                                :initial-element inf
                                :element-type 'fixnum)))
    (let ((q (make-deque)))
      (deque-push-back q (cons sy sx))
      (loop until (deque-empty-p q)
            do (let* ((point (deque-peak-front q))
                      (now-y (car point))
                      (now-x (cdr point))
                      (d (aref distances now-y now-x)))
                 (deque-pop-front q)
                 (when (and (= gy now-y)
                            (= gx now-x))
                   (return-from bfs d))
                 (loop for i from 0 below 4
                       do (let* ((dydx (aref dydxs i))
                                 (next-y (+ now-y (car dydx)))
                                 (next-x (+ now-x (cdr dydx))))
                            (when (and (< -1 next-y h)
                                       (< -1 next-x w)
                                       (= (aref distances next-y next-x) inf)
                                       (= (aref maze next-y next-x) 0))
                              (setf (aref distances next-y next-x)
                                    (1+ d))
                              (deque-push-back q (cons next-y next-x)))))))))
  nil)

(defun main ()
  (input* ((n fixnum)
           (s (vector string n)))
    (let ((maze (make-array (list n n)
                            :initial-element 0))
          (py nil)
          (px nil))
      (dotimes (i n)
        (dotimes (j n)
          (cond ((char= (aref (aref s i) j) #\#)
                 (setf (aref maze i j) 1))
                ((char= (aref (aref s i) j) #\p)
                 (push i py)
                 (push j px))))))))
