...
(defun main ()
  (input* ((s string))
    (let ((rev-s (reverse s)))
      (let ((dot-index (find-index rev-s
                                   (lambda (c) (char= c #\.)))))
        (let ((tld (reverse (subseq rev-s 0 dot-index))))
          (format t "~A~%" tld))))))

(defun test ()
  (test-case "atcoder.jp" "jp")
  (test-case "translate.google.com" "com")
  (test-case ".z" "z")
  (test-case "..........txt" "txt")
  )
