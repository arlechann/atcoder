...
int sim(int a, int b, int c, int turn) {
	if((turn == 0 && a == a_size) || (turn == 1 && b == b_size) ||
	   (turn == 2 && c == c_size)) {
		return turn;
	}

	if(turn == 0) {
		return sim(a + 1, b, c, s[0][a] - 'a');
	} else if(turn == 1) {
		return sim(a, b + 1, c, s[1][b] - 'a');
	} else {
		return sim(a, b, c + 1, s[2][c] - 'a');
	}
}
...
int main() {
	...
	char members[] = {'A', 'B', 'C'};
	int winner = sim(0, 0, 0, 0);
	cout << members[winner] << endl;
	return 0;
}
