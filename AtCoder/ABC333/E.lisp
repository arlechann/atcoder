...
(defun main ()
  (input* ((n fixnum)
           (tx (vector (list fixnum fixnum1) n)))
    (let ((need-potions (make-array n :initial-element 0))
          (result (make-array n :initial-element 0)))
      (let ((k (loop with k = 0
                     for i from (1- n) downto 0
                     maximize (let ((query-type (car (aref tx i)))
                                    (thing-type (cadr (aref tx i))))
                                (cond ((= query-type 1)
                                       (cond ((zerop (aref need-potions thing-type))
                                              (setf (aref result i) 0)
                                              k)
                                             (t (decf k)
                                                (decf (aref need-potions thing-type))
                                                (setf (aref result i) 1)
                                                k)))
                                      (t (incf k)
                                         (incf (aref need-potions thing-type))
                                         k))))))
        (if (some (lambda (need) (not (zerop need))) need-potions)
            (format t "-1~%")
            (progn (format t "~A~%" k)
                   (loop for i below n
                         do (when (= (car (aref tx i)) 1)
