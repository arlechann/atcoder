...
(defun dfs (graph node prev)
  (reduce #'+
          (mapcar (lambda (next)
                    (dfs graph next node))
                  (remove prev (graph-neighbors graph node)))
          :initial-value 1))

(defun main ()
  (input* ((n fixnum)
           (uv (vector (list fixnum1 fixnum1) (1- n))))
    (let ((graph (make-graph-from-edges n uv)))
      (format t "~A~%"
              (let ((subtree-costs (mapcar (lambda (next)
                                             (dfs graph next 0))
                                           (graph-neighbors graph 0))))
                (1+ (- (reduce #'+ subtree-costs :initial-value 0)
                       (reduce #'max subtree-costs :initial-value 0))))))))
