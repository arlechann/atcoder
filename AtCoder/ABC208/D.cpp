...
// 最短経路探索(非負閉路)
Weight
dijkstra(const Graph& graph, const size_t s, const size_t g, const size_t k) {
	size_t n = graph.node;
...
int main() {
	...
	VLL2D distances(n, VLL(n, INFLL));
	ll result = 0;
	REP(i, m) { distances[a[i]][b[i]] = c[i]; }
