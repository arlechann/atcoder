...
void make_list(VLL& list, VLL& primes, int n, ll x) {
	if(primes.size() == n) {
		if(x != 1) {
			list.push_back(x);
		}
	...
	make_list(list, primes, n + 1, x);
	make_list(list, primes, n + 1, x * primes[n]);
}
...
int main() {
	...
	auto primes = prime_list(51LL);
	ll result = 1;
	VLL list;
	make_list(list, primes, 0, 1);
	sort(ALL(list));
	EACH(t, list) {
		bool is_end = true;
		REP(i, n) {
