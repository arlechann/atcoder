...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> isize {
			(-300 - self.g..=300)
				.map(|g_left| {
					let g_right = g_left + self.g - 1;
					let r_right = -100 + (self.r - 1) / 2;
					let r_count = if r_right < g_left {
						let rr = (self.r - 1) / 2;
						let rl = self.r / 2;
						(1..=rr).sum::<isize>() + (1..=rl).sum::<isize>()
					} else if g_left <= -100 {
						let rr = -(g_left - 1) - 100;
						let rl = rr + self.r - 1;
						(rr..=rl).sum::<isize>()
					...
					let b_left = 100 - (self.b - 1) / 2;
					let b_count = if g_right < b_left {
						let bl = (self.b - 1) / 2;
						let br = self.b / 2;
						(1..=bl).sum::<isize>() + (1..=br).sum::<isize>()
					} else if 100 <= g_right {
						let bl = g_right + 1 - 100;
						let br = bl + self.b - 1;
						(bl..=br).sum::<isize>()
					} else {
						let bl = 100 - g_right - 1;
						let br = self.b - bl - 1;
						(1..=bl).sum::<isize>() + (1..=br).sum::<isize>()
					};

					let g_count = if g_right < 0 {
						(-g_right..=-g_left).sum::<isize>()
					} else if 0 < g_left {
						(g_left..=g_right).sum::<isize>()
					} else {
						(1..=(-g_left)).sum::<isize>() + (1..=g_right).sum::<isize>()
					};

					// println!(
					// 	"rr:{}, gl:{}, gr:{}, bl:{}",
					// 	r_right, g_left, g_right, b_left
					// );
					// println!(
					// 	"{} + {} + {} = {}",
					// 	r_count,
