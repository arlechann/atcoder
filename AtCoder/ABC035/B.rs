...
mod solve {
	...
	use iter_utils::counter::CounterExt;
	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			let mut m = self.s.chars().counter();
			let u = m.remove(&'U').unwrap_or(0);
			let d = m.remove(&'D').unwrap_or(0);
			let l = m.remove(&'L').unwrap_or(0);
			let r = m.remove(&'R').unwrap_or(0);
			let q = m.remove(&'?').unwrap_or(0);
			let y = max(u, d) - min(u, d);
			let x = max(l, r) - min(l, r);
			if self.t == 1 {
				x + y + q
			} else if x + y > q {
				x + y - q
