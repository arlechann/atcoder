...
class SegmentTree {
	...
	// 全区間取得
	T find() { return this->find(0, this->size); }
...
int main() {
	...
	SegmentTree<int> is_isolated(VI(n, 1));
	...
	REP(i, q) {
		if(t[i] == 1) {
			...
			is_isolated.update(u[i], 0);
			is_isolated.update(v[i], 0);
		} else {
			EACH(neigh, edges[v[i]]) {
				edges[neigh].erase(v[i]);
				if(edges[neigh].empty()) {
					is_isolated.update(neigh, 1);
				}
			}
			edges[v[i]].clear();
			is_isolated.update(v[i], 1);
		}
		result[i] = is_isolated.find();
	}
