...
int main() {
	...
	REP(i, n) {
		...
		int snuke_lower_index = bin_search(0, m, [&](int i) {
			ll snuke_value = b[i];
			return snuke_value - aoki_value <= d;
		});
		ll snuke_lower_value = b[snuke_lower_index];
		if(abs(snuke_lower_value - aoki_value) <= d) {
			chmax(result, aoki_value + snuke_lower_value);
		}

		int snuke_higher_index = bin_search(m - 1, -1, [&](int i) {
			ll snuke_value = b[i];
			return aoki_value - snuke_value <= d;
		});
		ll snuke_higher_value = b[snuke_higher_index];
		if(abs(aoki_value - snuke_higher_value) <= d) {
			chmax(result, aoki_value + snuke_higher_value);
		}
	}
