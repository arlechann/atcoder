...
(defun solve ()
  (input* ((n fixnum)
           ...
           (c (vector fixnum n)))
    (let ((cc (cumulate c)))
      (println (loop for x from 0 below (* w 2)
                     minimize (loop for i from x to (+ n w) by (* w 2)
                                    ;; do (format *error-output* "(x, i) = (~A, ~A): [~A, ~A)~%" x i (- i w) i)
                                    sum (- (aref cc (min i n)) (aref cc (max (- i w) 0)))))))))
