...
void solve(Graph& g1, Graph& g2, int s, int t) {}
...
int main() {
	...
	REP(i, m) {
		graph.link(u[i] + n * 0, v[i] + n * 1);
		graph.link(u[i] + n * 1, v[i] + n * 2);
		graph.link(u[i] + n * 2, v[i] + n * 0);
	}
	auto distances = bfs(graph, s);

	cout << (distances[t] == INF ? -1 : distances[t] / 3) << endl;
	return 0;
}
