...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			let mut count: Vec<Vec<usize>> = (0..3).map(|_| vec![0; self.m]).collect();
			for i in 0..self.n {
				for j in 0..self.n {
					count[(i + j) % 3][self.c[i][j] - 1] += 1;
				}
			}
			...
			let wrongness = count
				...
				.map(|v| {
					(0..self.m)
						.map(|i| v.iter().enumerate().map(|(j, x)| self.d[j][i] * x).sum())
						.collect::<Vec<usize>>()
				})
				.collect::<Vec<_>>();
			...
			for i in 0..self.m {
				for j in 0..self.m {
					...
					for k in 0..self.m {
						...
						result = min(result, wrongness[0][i] + wrongness[1][j] + wrongness[2][k]);
					}
				}
			}
			result
