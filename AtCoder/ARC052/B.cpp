...
int main() {
	...
	vector<double> vol(A_MAX + 1, 0);
	REP(i, n) {
		REP(k, h[i]) {
			int dx = x[i] + h[i] - k - 1;
			vol[dx] += PI *
					   static_cast<double>(square<ll>(r[i]) *
										   (3 * square<ll>(k) + 3 * k + 1)) /
					   static_cast<double>(3 * square<ll>(h[i]));
		}
	}

	RREP(i, A_MAX) { vol[i] += vol[i + 1]; }

	REP(i, q) { cout << PRECISION(15) << vol[a[i]] - vol[b[i]] << endl; }
	return 0;
}
