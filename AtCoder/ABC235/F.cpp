...
int main() {
	...
	int size = n.size();
	auto dp = make_vector({size + 1, 2, 1 << 10}, mint(0));
	dp[0][1][0] = 1;
	REP(i, size) {
		REP(k, 1 << 10) {
			REP(j, m) {
				...
				if((k & add_bit) == 0) {
					dp[i + 1][0][k] += dp[i][0][k ^ add_bit];
					if(j < n[i] - '0') {
						dp[i + 1][0][k] += dp[i][1][k ^ add_bit];
					}
				}
			}
		}
	...
	cout << (dp[size][1][(1 << 10) - 1] + dp[size][0][(1 << 10) - 1]) << endl;
	return 0;
}
