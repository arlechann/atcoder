...
int main() {
	...
	unordered_map<ll, unordered_map<int, int>> queries;
	unordered_map<ll, unordered_map<int, int>> results;
	REP(i, q) {
		queries[x[i]][k[i]] = 1;
		results[x[i]][k[i]] = -1;
	}
	...
	REP(i, n) {
		count[a[i]]++;
		if(queries[a[i]][count[a[i]]] != 0) {
			results[a[i]][count[a[i]]] = i + 1;
		}
	}

	REP(i, q) { cout << results[x[i]][k[i]] << endl; }
	return 0;
}
