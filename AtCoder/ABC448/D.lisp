...
(defun print-ht (ht)
  (maphash #'(lambda (k v) (format *error-output* "(k . v) => (~S . ~S)~%" k v)) ht))

(defun dfs (edges values node prev ht upper-t collector)
  (let* ((value (aref values node))
         (count (gethash value ht 0))
         (check (/= count 0)))
    ;; #?node (print-ht ht)
    (funcall collector node (or upper-t check))
    (incf (gethash value ht 0))
    (dolist (next (aref edges node))
      (unless (= next prev)
        (dfs edges values next node ht (or upper-t check) collector)))
    (decf (gethash value ht))))

(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum n))
           (uv (vector (cons* fixnum1 fixnum1) (1- n))))
    (let* ((edges (aprog1 (make-array n :initial-element nil)
                    (dotimes (i (1- n))
                      (destructuring-bind (u . v) (aref uv i)
                        (push u (aref it v))
                        (push v (aref it u))))))
           (result (make-array n :initial-element nil)))
      (dfs edges a 0 0 (make-hash-table) nil #'(lambda (node has-value) (setf (aref result node) has-value)))
      (dotimes (i n)
        (print-boolean (aref result i))))))
