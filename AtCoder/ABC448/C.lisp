...
(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (a (vector fixnum n)))
    (let ((st (make-segment-tree n #'min *inf* :initial-contents (coerce a 'list))))
      (dotimes (i q)
        (input* ((k fixnum)
                 (b (vector fixnum1 k)))
          (let ((b-values (map 'vector #'(lambda (index) (aref a index)) b)))
            (dotimes (j k)
              (setf (segment-tree-ref st (aref b j)) *inf*))
            (println (segment-tree-fold st 0 n))
            (loop for index across b
                  for value across b-values
                  do (setf (segment-tree-ref st index) value))))))))
