...
(defun div-longhand (numer-iter denom &key (mod 100000007))
  (nlet rec ((result 0) (numer (funcall numer-iter)))
    ;; #?(list result numer denom)
    (cond ((null numer) result)
          ((zerop (floor numer denom))
           (let ((n (funcall numer-iter)))
             (if (null n)
                 (values result numer)
                 (rec (mod (* result 10) mod)
                      (+ (* numer 10)
                         n)))))
          (t (rec (mod (+ result
                          (floor numer denom))
...
(defun main ()
  (input* ((k fixnum)
           (m fixnum)
           (cl (vector (cons* fixnum fixnum) k)))
    (let* ((iter (let ((index 0))
                   #'(lambda ()
                       (when (< index k)
                         (let* ((cli (aref cl index))
                                (ci (car cli))
                                (li (cdr cli)))
                           (if (= li 1)
                               (incf index)
                               (decf (cdr (aref cl index))))
                           ci))))))
      (println (div-longhand iter m :mod 10007)))))
