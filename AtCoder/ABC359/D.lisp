...
(defun xor (a b) (not (eq (not a) (not b))))

(defun log-palindrome-p (n length)
  (loop for i from 0
        while (< i (- length i 1))
        when (xor (logbitp i n)
                  (logbitp (- length i 1) n))
          do (return-from log-palindrome-p nil))
  t)

(defun eq-bitset-string (bitset s length)
  (loop for i from 0 below length
        unless (or (and (not (logbitp (- length i 1) bitset))
                        (or (char= (aref s i) #\A)
                            (char= (aref s i) #\?)))
                   (and (logbitp (- length i 1) bitset)
                        (or (char= (aref s i) #\B)
                            (char= (aref s i) #\?))))
          do (return-from eq-bitset-string nil))
  t)
...
(defun main ()
  (input* ((n fixnum)
           (k fixnum)
           (s string))
    (let ((dp (make-array (list (1+ n) (ash 1 k))
                          :element-type 'fixnum
                          :initial-element 0)))
      (loop for bitset from 0 below (ash 1 k)
            do (when (and (not (log-palindrome-p bitset k))
                          (eq-bitset-string bitset s k))
                 (setf (aref dp k bitset) 1)))
      (loop for i from k below n
            do (loop for bitset from 0 below (ash 1 k)
                     when (and (or (and (zerop (logand bitset 1))
                                        (or (char= (aref s i) #\A)
                                            (char= (aref s i) #\?)))
                                   (and (not (zerop (logand bitset 1)))
                                        (or (char= (aref s i) #\B)
                                            (char= (aref s i) #\?))))
                               (not (log-palindrome-p bitset k)))
                       do (setf (aref dp (1+ i) bitset)
                                (+ (aref dp i (ash bitset -1))
                                   (aref dp i (logior (ash bitset -1) (ash 1 (1- k))))))
                          (setf (aref dp (1+ i) bitset)
                                (mod (aref dp (1+ i) bitset) +mod+))))
      (let ((result 0))
        (loop for bitset from 0 below (ash 1 k)
              do (incf result (aref dp n bitset))
                 (setf result (mod result +mod+)))
        (format t "~A~%" result)))))
