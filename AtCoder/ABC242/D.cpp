...
int main() {
	...
	unordered_map<char, vector<char>> move_to;
	move_to['A'] = {'B', 'C'};
	move_to['B'] = {'C', 'A'};
	move_to['C'] = {'A', 'B'};
	...
	REP(i, q) {
		...
		// cout << "move:  " << std::bitset<64>(move) << endl;
		char now = s[first];
		if(t[i] > 64) {
			REP(j, (t[i] - 64) % 3) { now = move_to[now][0]; }
		}
		chmin<ull>(t[i], 64);
		REP(j, t[i]) {
			ull is_right = ((move >> (t[i] - 1 - j)) & 1);

			// cout << "now: " << now << endl;
			// cout << "is_right: " << is_right << endl;
			now = move_to[now][is_right];
		}
