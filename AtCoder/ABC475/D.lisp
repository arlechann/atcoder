...
(defun alphameticp (x s)
  (let ((x (princ-to-string x)))
    (unless (= (length x) (length s))
      (return-from alphameticp nil))
    (let ((xa (make-array 10 :initial-element -1 :element-type 'fixnum))
          (sa (make-array (count-alphabet) :initial-element -1 :element-type 'fixnum)))
      (loop for xc across x
            and sc across s
            for xci = (char-digit xc)
            and sci = (char-to-index sc)
            ;; do #?(list xc sc xci sci)
            if (and (= (aref xa xci) -1)
                    (= (aref sa sci) -1))
              do (setf (aref xa xci) sci
                       (aref sa sci) xci)
            else if (or (/= (aref xa xci) sci)
                        (/= (aref sa sci) xci))
              do (return-from alphameticp nil)
            finally (return t)))))

(defun main ()
  (input* ((s string))
    (let* ((primes (primes 10000000))
           (result (find-if (pa #'alphameticp :$0 s) primes)))
      (println (or result -1)))))
