...
(defun main ()
  (input* ((n fixnum)
           ...
           (l fixnum)
           (a (vector fixnum (1- n))))
    (let* ((cumsum (cumulate a))
           (result 0))
      (loop for left from 0 to s
            for ld = (* (diff (aref cumsum s)
                              (aref cumsum left))
                        2)
            maximize (loop for right from s below n
                           for rd = (diff (aref cumsum s)
                                          (aref cumsum right))
                           when (<= (+ ld rd) l)
                             maximize (1+ (- right left)))
              into r
            finally (maxf result r))
      (loop for left from 0 to s
            for ld = (diff (aref cumsum s)
                           (aref cumsum left))
            maximize (loop for right from s below n
