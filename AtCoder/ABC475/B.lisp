...
(defun main ()
  (input* ((n fixnum)
    ...
    (loop with coin1 = 0
          ...
          and coin100 = 0
          for ai across a
          for rest = (mod ai 1000)
          do (let* ((d1 (mod rest 10))
                    (d10 (mod (ceiling rest 10) 10))
                    (d100 (mod (ceiling rest 100) 10)))
               (unless (zerop d1)
                 (incf coin1 (- 10 d1)))
               (unless (zerop d10)
                 (incf coin10 (- 10 d10)))
               (unless (zerop d100)
                 (incf coin100 (- 10 d100))))
          finally (format t "~A ~A ~A~%" coin1 coin10 coin100))))
