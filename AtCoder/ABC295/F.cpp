...
ll test_case(int t) {
	...
	ll result = 0;
	{
		int n = r.size();
		int m = s.size();
		auto dp1 = make_vector({n + 1, m + 1, 2}, 0LL);
		dp1[0][0][1] = 1;
		REP(i, n) {
			dp1[i + 1][0][0] += dp1[i][0][0] * 9 + dp1[i][0][1] * (r[i] - '0');
			dp1[i + 1][0][1] += (s[0] == r[i] ? 0 : dp1[i][0][1]);
			REP(j, m) {
				dp1[i + 1][0][0] +=
					dp1[i][j + 1][0] * (s[0] == s[j] ? 9 : 8) +
					dp1[i][j + 1][1] * ((r[i] - '0') - (s[j] < r[i] ? 1 : 0));
				dp1[i + 1][1][0] +=
					dp1[i][j + 1][0] + (s[0] < r[i] ? dp1[i][j + 1][1] : 0);
				dp1[i + 1][1][1] += (s[0] == r[i] ? dp1[i][j + 1][1] : 0);
				dp1[i + 1][j + 1][0] +=
					dp1[i][j][0] + (s[j] < r[i] ? 0 : dp1[i][j][1]);
				dp1[i + 1][j + 1][1] += (s[j] == r[i] ? dp1[i][j][1] : 0);
			}
		}
	...
	l = to_string(lx);
	{
		int n = l.size();
		int m = s.size();
		auto dp2 = make_vector({n + 1, m + 1, 2}, 0LL);
		dp2[0][0][1] = 1;
		REP(i, n) {
			dp2[i + 1][0][0] += dp2[i][0][0] * 9 + dp2[i][0][1] * (r[i] - '0');
			dp2[i + 1][0][1] += (s[0] == r[i] ? 0 : dp2[i][0][1]);
			REP(j, m) {
				dp2[i + 1][0][0] +=
					dp2[i][j + 1][0] * 9 +
					dp2[i][j + 1][1] * ((r[i] - '0') - (s[j] < r[i] ? 1 : 0));
				dp2[i + 1][1][0] +=
					dp2[i][j + 1][0] + (s[0] < r[i] ? dp2[i][j + 1][1] : 0);
				dp2[i + 1][1][1] += (s[0] == r[i] ? dp2[i][j + 1][1] : 0);
				dp2[i + 1][j + 1][0] +=
					dp2[i][j][0] + (s[j] < r[i] ? 0 : dp2[i][j][1]);
				dp2[i + 1][j + 1][1] += (s[j] == r[i] ? dp2[i][j][1] : 0);
			}
		}
