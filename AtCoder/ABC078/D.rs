...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&mut self) -> i64 {
			let a = self.a.iter().copied().rev().collect_vec();
			self.memo = vec![vec![None; 2]; self.n + 1];
			self.dp(&a, self.n, self.n, 0)
		}

		fn dp(&mut self, a: &[i64], n: usize, prev: usize, player: usize) -> i64 {
			if let Some(ret) = self.memo[n][player] {
				return ret;
			}

			if n == 0 {
				return if prev == self.n {
					(self.w - a[0]).abs()
				} else {
					(a[0] - a[prev]).abs()
				};
			...
			let iter = (0..n).map(|i| self.dp(a, i, n, player ^ 1));
			let ret = if player == 0 {
				iter.max().unwrap()
			} else {
				iter.min().unwrap()
			};
			self.memo[n][player] = Some(ret);
			ret
		}
	}
}
