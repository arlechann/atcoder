...
pair<ll, ll> solve(const VI2D& edges, int node, int color) {
	used[node] = 1;
	colors[node] = color;
	...
	EACH(next, edges[node]) {
		if(used[next] && color != colors[next]) {
			continue;
		}
		if(used[next] && color == colors[next]) {
			// cout << "color == colors[next]" << endl;
			return {-1, -1};
		}

		auto [b, w] = solve(edges, next, (color + 1) % 2);
...
int main() {
	...
	ll result =
		static_cast<ll>(n) * static_cast<ll>(n - 1) / 2LL - static_cast<ll>(m);
	REP(i, n) {
		colors[i] = -1;
	}
	REP(i, n) {
		...
		auto [black, white] = solve(edges, i, 0);
		// cout << "i: " << i << ", black: " << black << ", white: " << white
		// 	 << endl;
		if(black == -1) {
			cout << 0 << endl;
			return 0;
