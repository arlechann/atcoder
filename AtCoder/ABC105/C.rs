...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> String {
			let mut pow_neg2: Vec<i128> = Vec::with_capacity(11);
			pow_neg2.push(1);
			for i in 0..120 {
				pow_neg2.push(pow_neg2[i] * -2);
			}

			if self.n == 0 {
				"0".to_string()
			} else {
				self.rec(&pow_neg2, pow_neg2.len(), self.n)
					.map(|s| {
						s.chars()
							.rev()
							.skip_while(|&c| c == '0')
		...
		fn rec(&self, pn2: &[i128], i: usize, n: i128) -> Option<String> {
			if n == 0 {
				return Some("0".repeat(i));
			}
			if i == 0 {
				return None;
			}
			if pn2[i - 1].abs() * 2 < n.abs() {
				return None;
			}
			let mut a = self.rec(pn2, i - 1, n - pn2[i - 1]);
			a.iter_mut().for_each(|a| a.push('1'));
			let mut b = self.rec(pn2, i - 1, n);
			b.iter_mut().for_each(|b| b.push('0'));
			a.or(b)
		}
	}
}
