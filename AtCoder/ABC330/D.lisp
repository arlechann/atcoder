...
(defun main ()
  (input* ((n fixnum)
           (s (vector string n)))
    (let ((row-count (make-array n :initial-element 0))
          (col-count (make-array n :initial-element 0)))
      (loop for i from 0 below n
            do (loop for j from 0 below n
                     do (incf (aref row-count i)
                              (o->n (aref (aref s i) j)))
                        (incf (aref col-count j)
                              (o->n (aref (aref s i) j)))))
      (format t "~A~%"
              (loop for i from 0 below n
                    sum (loop for j from 0 below n
                              sum (if (char-o-p (aref (aref s i) j))
                                      (* (1- (aref row-count i))
                                         (1- (aref col-count j)))
                                      0)))))))
