...
class UnionFind {
	...
	UnionFind(size_t size)
		: parents(size), rank(size, 0), tree_size(size, 1), used(size, 0) {
		std::iota(this->parents.begin(), this->parents.end(), 0);
	}

	// 併合
	bool merge(size_t a, size_t b) {
		this->used[a] = this->used[b] = 1;
	...
	void use(size_t i) { this->used[i] = 1; }
	...
	size_t solve() {
		...
		REP(i, this->parents.size()) {
			if(this->used[i] != 0 && i == this->parents[i]) {
				ret++;
			}
		}
		return ret;
	...
	REP(i, h) {
		REP(j, w) {
			...
			REP(k, 8) {
				...
				if(s[yy][xx] == '#') {
					uf.merge(encode(i, j), encode(yy, xx));
				}
			}
		}
	}
