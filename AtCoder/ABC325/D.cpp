...
int ins(
	...
	priority_queue<pair<ll, ll>, vector<pair<ll, ll>>, greater<pair<ll, ll>>>&
		pq,
	int result) {
	// cout << "ins(" << index << ", " << now << ", pq, " << result << ");"
	// 	 << endl;
	auto [in, out] = intervals[index];
	chmax(now, in);
	pq.push(make_pair(out, in));
	return write(index + 1, now, pq, result);
}
int write(
	int index,
	ll now,
	priority_queue<pair<ll, ll>, vector<pair<ll, ll>>, greater<pair<ll, ll>>>&
		pq,
	int result) {
	// cout << "write(" << index << ", " << now << ", pq, " << result << ");"
	// 	 << endl;
	if(pq.empty() && index == n) {
		return result;
	}
	...
	if(pq.empty()) {
		return ins(index, now, pq, result);
	}
	...
	if(index == n) {
		...
		pq.pop();
		if(out <= now) {
			return write(index, now, pq, result);
		}
		// cout << "now: " << now << ", in: " << in << ", out: " << out << endl;
		return write(index, now + 1, pq, result + 1);
	}

	auto [next_in, next_out] = intervals[index];
	if(now >= next_in) {
		return ins(index, now, pq, result);
	}
	auto [out, in] = pq.top();
...
int main() {
	...
	REP(i, n) {
		intervals[i] = make_pair(t[i], t[i] + d[i] + 1);
	}
	sort(ALL(intervals));
