...
VLL dijkstra(const vector<vector<pair<ll, int>>>& edges,
			 int s,
			 int g1,
			 int g2) {
	int n = edges.size();
	...
	while(!pq.empty()) {
		...
		used[node] = 1;
		if(used[g1] != 0 && used[g2] != 0) {
			break;
		}
		for(auto&& e : edges[node]) {
			ll alt = e.first + dist;
			if(alt < distances[e.second]) {
				distances[e.second] = alt;
				pq.push(make_pair(alt, e.second));
			}
		}
	}
	return distances;
...
int main() {
	...
	vector<vector<pair<ll, int>>> edges(n * 2);
	REP(i, n) {
		edges[i].push_back(make_pair(0, i + n));
		REP(j, n) {
			if(i != j) {
				edges[i].push_back(make_pair(d[i][j] * a, j));
				edges[i + n].push_back(make_pair(d[i][j] * b + c, j + n));
			}
		}
	}

	auto distances = dijkstra(edges, 0, n - 1, n * 2 - 1);
	ll result = min(distances[n - 1], distances[n * 2 - 1]);
	cout << result << endl;
	return 0;
}
