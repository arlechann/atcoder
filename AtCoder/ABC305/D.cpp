...
int main() {
	...
	times.push_back(make_pair(a[0], a[0]));
	ll last_sleep_time = a[0];
	REP(i, n / 2) {
		times.push_back(make_pair(a[i * 2 + 1], last_sleep_time));
		ll new_last_sleep_time =
			(a[i * 2 + 2] - a[i * 2 + 1]) + last_sleep_time;
		times.push_back(make_pair(a[i * 2 + 2], new_last_sleep_time));
		last_sleep_time = new_last_sleep_time;
	}

	VLL result(q);
	int times_size = times.size();
	REP(i, q) {
		int left_index = bin_search<int>(0, times_size, [&](int index) {
			return times[index].first < l[i];
		});
		ll left_time =
			times[left_index].second +
			(left_index % 2 != 0 ? 0 : (l[i] - times[left_index].first));

		int right_index = bin_search<int>(0, times_size, [&](int index) {
			return times[index].first < r[i];
		});
		ll right_time =
			times[right_index].second +
			(right_index % 2 != 0 ? 0 : (r[i] - times[right_index].first));
		result[i] = right_time - left_time;
	}
