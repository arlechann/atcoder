...
set<int> dijkstra(const VI2D& edges, const VI& p, const VI& h) {
	int n = edges.size();
	VI used(n, 0);
	VI rest_healths(n, -1);
	...
	while(!pq.empty()) {
		...
		int node = x.second;
		pq.pop();
		if(rest_healths[node] > health) {
			continue;
		}
		rest_healths[node] = health;
		ret.insert(node);
		for(auto&& e : edges[node]) {
			int alt = health - 1;
			if(alt <= rest_healths[e]) {
				continue;
			}
			rest_healths[e] = alt;
			pq.push(make_pair(alt, e));
		}
	}
