...
mint solve(const Graph& g, const VI& distances, int node) {
	mint& ret = memo[node];
	if(used[node] != 0) {
		return ret;
	}
	...
	used[node] = 1;
	int here = distances[node];
	if(here == 0) {
		return ret = 1;
	}
	EACH(next, g.edges[node]) {
		if(distances[next.to] == here - 1) {
			ret += solve(g, distances, next.to);
		}
	}
	return ret;
}
...
int main() {
	...
	cout << solve(g, distances, n - 1) << endl;
	return 0;
}
