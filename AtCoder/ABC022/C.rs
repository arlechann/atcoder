...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> isize {
			let mut zero_connections = vec![];
			let mut distances_without_zero = vec![vec![INF; self.n]; self.n];
			for &(u, v, l) in self.uvl.iter() {
				if u == 1 {
					zero_connections.push((v - 1, l));
				} else {
					distances_without_zero[u - 1][v - 1] = l;
					distances_without_zero[v - 1][u - 1] = l;
				}
			}
			for i in 0..self.n {
				distances_without_zero[i][i] = 0;
			}
			for k in 0..self.n {
				for i in 0..self.n {
					for j in 0..self.n {
						if distances_without_zero[i][j]
							> distances_without_zero[i][k] + distances_without_zero[k][j]
						{
							distances_without_zero[i][j] =
								distances_without_zero[i][k] + distances_without_zero[k][j];
						}
					}
				}
			...
			let zc_len = zero_connections.len();
			let mut result = INF;
			for i in 0..zc_len {
				for j in i + 1..zc_len {
					let (i_node, i_len) = zero_connections[i];
					let (j_node, j_len) = zero_connections[j];
					result = min(
						result,
						i_len + j_len + distances_without_zero[i_node][j_node],
					);
				}
			}
