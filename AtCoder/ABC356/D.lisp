  ...
  (input* ((n fixnum)
    ...
    (let ((msb (logmsb n))
      ...
      (dotimes-rev (i (1+ msb))
        ...
        (unless (zerop (logand (ash 1 i)
                               m))
          (unless (zerop (logand (ash 1 i)
                                 n))
            (incf result (1+ (logand n (1- (ash 1 i))))))
          (incf result (floor (logand n (lognot (1- (ash 1 (1+ i))))) 2))
          (setf result (mod result +mod+))))
      (format t "~A~%" result))))

(defun test ()
  (test-case* "4 3" "4" "0 0" "0" "1152921504606846975 1152921504606846975" "499791890"))
