...
(defun check (k keys ai ri)
  (let ((used-right-key-count 
          (loop for key across ai
                sum (if (logbitp key keys) 1 0))))
    (if (eq ri 'o)
        (>= used-right-key-count k)
        (< used-right-key-count k))))
...
(defun main ()
  (input* ((n fixnum)
    ...
    (let ((c (make-array m))
      ...
      (dotimes (i m)
        (let* ((ci (read))
               (ai (let ((ai (make-array ci)))
                     (dotimes (j ci ai)
                       (setf (aref ai j) (1- (read)))))))
          (setf (aref c i) ci)
          (setf (aref a i) ai)
          (setf (aref r i) (read))))
      (let ((result
              (loop for keys from 0 below (ash 1 n)
                    count (every (lambda (ai ri)
                                   (check k keys ai ri))
                                 a r))))
        (format t "~A~%" result)))))
