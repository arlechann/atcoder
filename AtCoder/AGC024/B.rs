...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			let mut dp = vec![1; self.n];
			let mut prev_index = vec![None; self.n];
			for i in 0..self.n {
				if let Some(prev) = prev_index[self.a[i] - 1] {
					dp[i] = dp[prev] + 1;
				}
				if self.a[i] != self.n {
					prev_index[self.a[i]] = Some(i);
				}
			}
			self.n - dp.into_iter().max().unwrap()
		}
	}
}
