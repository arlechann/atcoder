...
void dfs(const vector<vector<pair<int, pair<ll, ll>>>>& edges,
		 VLL& res_x,
		 VLL& res_y,
		 VI& used,
		 int now) {
	used[now] = 1;
	ll pos_x = res_x[now], pos_y = res_y[now];
	EACH(edge, edges[now]) {
		int next = edge.first;
		if(used[next] != 0) {
			continue;
		}
		res_x[next] = pos_x + edge.second.first;
		res_y[next] = pos_y + edge.second.second;
		dfs(edges, res_x, res_y, used, next);
	}
}
...
int main() {
	...
	REP(i, m) {
		edges[a[i]].push_back(make_pair(b[i], make_pair(x[i], y[i])));
		edges[b[i]].push_back(make_pair(a[i], make_pair(-x[i], -y[i])));
	}

	VLL res_x(n, 0), res_y(n, 0);
	VI used(n, 0);
	dfs(edges, res_x, res_y, used, 0);
