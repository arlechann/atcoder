...
struct Query {
	...
	ll w;
	ll s;
	Query(ll t, int type, int n, ll w, ll s)
		: t(t), type(type), n(n), w(w), s(s) {}

	bool operator<(const Query& rhs) const { return this->t > rhs.t; }
};
...
int main() {
	...
	priority_queue<Query> pq_query;
	priority_queue<Query> pq_return;
	REP(i, m) {
		pq_query.push(Query(t[i], 0, 0, w[i], s[i]));
	}
	...
	SegmentTree<int> stree(
		a, [](int a, int b) { return min(a, b); }, INF);

	VLL result(n, 0);
	while(!pq_query.empty()) {
		auto q = pq_query.top();
		pq_query.pop();
		while(!pq_return.empty()) {
			auto r = pq_return.top();
			if(r.t > q.t) {
				break;
			}
			pq_return.pop();
			stree.update(r.n, r.n);
		}
		// cout << q.t << " " << q.w << " " << q.s << endl;
		int top = stree.find(0, n + 1);
		// cout << top << endl;
		if(top < n) {
			stree.update(top, INF);
			result[top] += q.w;
			pq_return.push(Query(q.t + q.s, 1, top, 0, 0));
		}
	}
