...
(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (p (vector fixnum1 n)))
    (let ((p2 (aprog1 (make-array n)
                (dotimes (i n)
                  (setf (aref it (aref p i)) i)))))
      ;; #?p #?p2
      (dotimes (i q)
        (let* ((type (read))
               (x (when (= type 1) (1- (read))))
               (y (when (= type 1) (1- (read)))))
          (cond ((= type 1)
                 (rotatef (aref p x) (aref p y))
                 (rotatef (aref p2 (aref p x)) (aref p2 (aref p y)))
                 ;; #?(cons p p2)
                 )
                (t
                 (rotatef p p2)
