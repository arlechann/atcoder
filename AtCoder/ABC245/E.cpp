...
struct Rect {
	...
	ll height;
	Rect() {}
	Rect(ll width, ll height) : width(width), height(height) {}
...
int main() {
	...
	vector<Rect> chocorates(n);
	REP(i, n) { chocorates[i] = Rect(b[i], a[i]); }
	sort(RALL(chocorates));
	vector<Rect> boxes(n);
	REP(i, m) { boxes[i] = Rect(d[i], c[i]); }
	sort(ALL(boxes));

	REP(i, n) {
		auto c = chocorates[i];
		auto lb = lower_bound(ALL(boxes), c);
		if(lb == boxes.end()) {
			cout << "No" << endl;
			return 0;
		}
