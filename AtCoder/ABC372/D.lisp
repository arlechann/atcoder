...
(defun main ()
  (input* ((n fixnum)
           (h (vector fixnum n)))
    (let ((st (make-segment-tree (1+ n) #'max 0))
          (result (make-array (1+ n) :initial-element 0)))
      (loop for i from 0
            for hi across h
            do (let ((ri (segment-tree-fold st hi (1+ n))))
                 (incf (aref result ri))
                 (decf (aref result i))
                 (setf (segment-tree-ref st hi) i)))
      (loop for i from 1 below n
            do (incf (aref result i) (aref result (1- i))))
      (dotimes (i n)
