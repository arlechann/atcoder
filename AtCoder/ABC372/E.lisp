...
(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (queries (vector (cons* fixnum fixnum1 fixnum1 nil) q)))
    (let ((roots (aprog1 (make-array n)
                   (dotimes (i n) (setf (aref it i) i))))
          (sizes (make-array n :initial-element 1))
          (groups (aprog1 (make-array n :initial-element nil)
                    (dotimes (i n) (push i (aref it i))))))
      (labels ((root (i)
                 (let ((r (aref roots i)))
                   (if (= r i)
                       i
                       (setf (aref roots i) (root r)))))
               ((setf root) (value i) (setf (aref roots (root i)) value))
               (size (i) (aref sizes i))
               ((setf size) (value i) (setf (aref sizes i) value))
               (group (i) (aref groups i))
               ((setf group) (value i) (setf (aref groups i) value))
               (unite (a b)
                 (setf a (root a)
                       b (root b))
                 (when (= a b)
                 ...
                 (let ((ag (group a))
                       (bg (group b)))
                   (setf (root b) a
                         (group a) (take (merge 'list ag bg #'>) 10))))
               (find-k (v k)
                 (nth k (group (root v)))))
        (loop for (ty a b) across queries
              ;do #?roots #?sizes #?groups
              if (= ty 1)
                do (unite a b)
              else
                do (format t "~A~%" (or (aand (find-k a b) (1+ it)) -1)))))))
