...
(def-input-reader char () `(read-char))

(defun count-substring (sub s)
  (let ((count -1)
        (pos -1))
    (loop 
      (incf count)
      (setf pos (search sub s :start2 (1+ pos)))
      (when (null pos)
        (return count)))))

(defun shared-subseq (str start end)
...
(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (s string)
           (xc (vector (cons* fixnum1 char nil) q)))
    (let ((count (count-substring "ABC" s)))
      (loop for i below q
            for (x c) across xc
            unless (char= c (aref s x))
              do (when (or (and (char= (aref s x) #\A)
                                (< (+ x 2) n)
                                (string= "ABC" (shared-subseq s x (+ x 3))))
                           (and (char= (aref s x) #\B)
                                (< -1 (1- x) (1+ x) n)
                                (string= "ABC" (shared-subseq s (1- x) (+ x 2))))
                           (and (char= (aref s x) #\C)
                                (< -1 (- x 2))
                                (string= "ABC" (shared-subseq s (- x 2) (1+ x)))))
                   (decf count))
                 (setf (aref s x) c)
                 (when (or (and (char= (aref s x) #\A)
                                (< (+ x 2) n)
                                (string= "ABC" (shared-subseq s x (+ x 3))))
                           (and (char= (aref s x) #\B)
                                (< -1 (1- x) (1+ x) n)
