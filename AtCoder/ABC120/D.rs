...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> String {
			let mut results: Vec<u64> = Vec::with_capacity(self.m);
			let mut state = (self.n * (self.n - 1)) as u64 / 2;
			results.push(state);

			let mut uf = UnionFind::new(self.n);
			for &(a, b) in self.ab.iter().skip(1).rev() {
				let size_a = uf.size(a - 1) as u64;
				let size_b = uf.size(b - 1) as u64;
