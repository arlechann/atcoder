...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> i64 {
			(0..8).map(|i| {
				let mut values = self.xyz.iter().copied().map(|(x, y, z)| if i & 1 == 0 { x } else { -x } + if i & 2 == 0 { y } else { -y } + if i & 4 == 0 { z } else { -z } ).collect_vec();
				values.sort_by_key(|&x| Reverse(x));
				values.iter().take(self.m).sum()
			}).max().unwrap()
		}
	}
}
