...
int main() {
	...
	REP(i, n) {
		if(used[i]) {
			count_x[i + 1] = count_x[i] + static_cast<int>(a[i] == x);
			count_y[i + 1] = count_y[i] + static_cast<int>(a[i] == y);
		}
	}

	vector<pair<int, int>> intervals;
	int left = 0;
	bool is_found_left = false;
	REP(i, n + 1) {
		if(!is_found_left && used[i]) {
			left = i;
			is_found_left = true;
		} else if(is_found_left && !used[i]) {
			intervals.push_back(make_pair(left, i));
			is_found_left = false;
		}
	}

	// REP(i, n + 1) { cout << count_x[i] << " "; }
	// cout << endl;

	// REP(i, n + 1) { cout << count_y[i] << " "; }
	// cout << endl;

	ll result = 0;
	EACH(interval, intervals) {
		// cout << interval.first << ", " << interval.second << endl;
		RANGE(i, interval.first, interval.second) {
			int left = bin_search<int>(interval.second, i, [&](int x) {
				int cx = count_x[x + 1] - count_x[i];
				int cy = count_y[x + 1] - count_y[i];
				return cx > 0 && cy > 0;
			});
			int range = interval.second - left;
			if(x == y) {
				range++;
			}
			result += range;

			// cout << i << " - " << left << ": " << range << endl;
		}
	}
