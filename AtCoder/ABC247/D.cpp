...
struct Cylinder {
	...
	void insert(ll x, ll c) { cylinder.push_back(make_pair(x, c)); }
	ll get(ll c) {
		ll sum = 0;
		while(c != 0) {
			auto [value, count] = cylinder.front();
			cylinder.pop_front();
			if(c < count) {
				sum += value * c;
				count -= c;
				c = 0;
				cylinder.push_front(make_pair(value, count));
			} else {
				sum += value * count;
				c -= count;
			}
...
int main() {
	...
	REP(i, q) {
		...
		if(t == 1) {
			...
			ll c;
			cin >> c;
			result.push_back(cylinder.get(c));
		}
	}
