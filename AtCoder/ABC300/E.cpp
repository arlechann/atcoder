...
const int POWER_MAX = 64;

inline mint get(const auto& dp, int i, int j, int k) {
	if(i < 0 || POWER_MAX < i) {
		return 0;
	}
	if(j < 0 || POWER_MAX < j) {
...
int main() {
	...
	auto dp =
		make_vector({POWER_MAX + 1, POWER_MAX + 1, POWER_MAX + 1}, mint(0));
	dp[0][0][0] = 1;
	REP(i, POWER_MAX + 1) {
		REP(j, POWER_MAX + 1) {
			REP(k, POWER_MAX + 1) {
				...
				// dp[i][j][k] = dp[i][j][k] / 6 +
				// 			  (get(dp, i - 1, j, k) + get(dp, i, j - 1, k) +
				// 			   get(dp, i - 2, j, k) + get(dp, i, j, k - 1) +
				// 			   get(dp, i - 1, j - 1, k)) /
				// 				  6;
				dp[i][j][k] = (get(dp, i - 1, j, k) + get(dp, i, j - 1, k) +
							   get(dp, i - 2, j, k) + get(dp, i, j, k - 1) +
							   get(dp, i - 1, j - 1, k)) /
							  5;
			}
	...
	cout << dp[factors[2]][factors[3]][factors[5]] << endl;
	return 0;
}
