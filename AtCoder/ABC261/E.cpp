...
int main() {
	...
	VI cumsum1(n + 1, (1 << BIT) - 1);
	REP(i, n) {
		if(t[i] == 1) {
			cumsum1[i + 1] = cumsum1[i] & a[i];
		} else if(t[i] == 2) {
			cumsum1[i + 1] = cumsum1[i] | a[i];
		} else {
			cumsum1[i + 1] = cumsum1[i] ^ a[i];
	...
	REP(i, n) {
		REP(j, BIT) {
			// cout << std::bitset<30>(result) << endl;
			int b = result & (1 << j);
			if(b == 0) {
				result |= (cumsum0[i + 1] & (1 << j));
			} else {
				result ^= ((cumsum1[i + 1] & (1 << j)) ^ (1 << j));
			}
		}
		cout << result << endl;
	}
