...
int mtl(pair<int, int> a) {
	return a.first * w + a.second;
}

pair<int, int> ltm(int a) {
	return make_pair(a / w, a % w);
}
...
int main() {
	...
	auto distances = dijkstra(graph, mtl(s));

	cout << (distances[mtl(g)] > 2 ? "NO" : "YES") << endl;
	return 0;
}
