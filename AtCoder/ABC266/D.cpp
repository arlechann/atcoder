...
int main() {
	...
	auto diffs = make_vector({T_MAX + 1, X_MAX + 1}, 0LL);
	REP(i, n) {
		diffs[t[i]][x[i]] = a[i];
	}

	auto dp = make_vector({T_MAX + 1, X_MAX + 1}, 0LL);
	REP(i, T_MAX) {
		REP(j, X_MAX + 1) {
			if(j > i + 1) {
				continue;
			}
			chmax(dp[i + 1][j], dp[i][j] + diffs[i + 1][j]);
			if(j != 0) {
				chmax(dp[i + 1][j], dp[i][j - 1] + diffs[i + 1][j]);
			}
			if(j != X_MAX) {
				chmax(dp[i + 1][j], dp[i][j + 1] + diffs[i + 1][j]);
			}
