...
int test_case(int t) {
	...
	REP(i, m) {
		if(c[u[i]] == c[v[i]]) {
			same_edges[u[i]].push_back(v[i]);
			same_edges[v[i]].push_back(u[i]);
		} else {
			change_edges[u[i]].push_back(v[i]);
			change_edges[v[i]].push_back(u[i]);
		}
	}

	VI2D distances(n, VI(n, INF));
	distances[0][n - 1] = 0;
	queue<pair<int, int>> que;
	que.push(make_pair(0, n - 1));
	while(!que.empty()) {
		auto [takahashi, aoki] = que.front();
		que.pop();
		int d = distances[takahashi][aoki];
		// cout << "t: " << takahashi << ", a: " << aoki << ", d: " << d <<
		// endl;
		if(takahashi == n - 1 && aoki == 0) {
			return d;
		}
		EACH(e1, same_edges[takahashi]) {
			EACH(e2, same_edges[aoki]) {
				if(distances[e1][e2] < INF) {
					continue;
				}
				distances[e1][e2] = d + 1;
				que.push(make_pair(e1, e2));
			}
		}
		EACH(e1, change_edges[takahashi]) {
			EACH(e2, change_edges[aoki]) {
				if(distances[e1][e2] < INF) {
					continue;
				}
				distances[e1][e2] = d + 1;
