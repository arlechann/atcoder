...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			let mut sum = 0;
			let mut imos = Imos::with_capacity(self.m);
			for &(l, r, s) in self.lrs.iter() {
				sum += s;
				imos.query(l - 1, r, s as i64);
			}
			sum - imos.apply().into_iter().min().unwrap() as usize
		}
	}
}
