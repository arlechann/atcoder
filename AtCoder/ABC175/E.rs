...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> u64 {
			let mut map = vec![vec![vec![]; self.c]; self.r];
			for &(r, c, v) in self.rcv.iter() {
				map[r - 1][c - 1].push(v);
			}
			for i in 0..self.r {
				for j in 0..self.c {
					let len = map[i][j].len();
					map[i][j].sort_by_key(|&x| std::cmp::Reverse(x));
			...
			let mut dp = vec![vec![vec![0; 4]; self.c + 1]; self.r + 1];
			for i in 0..self.r {
				for j in 0..self.c {
					dp[i + 1][j + 1][0] = max(dp[i + 1][j][0], *dp[i][j + 1].iter().max().unwrap());
					dp[i + 1][j + 1][1] = vec![
						map[i][j].get(0).unwrap_or(&0) + dp[i + 1][j][0],
						dp[i + 1][j][1],
						map[i][j].get(0).unwrap_or(&0) + *dp[i][j + 1].iter().max().unwrap(),
					]
					.into_iter()
					.max()
					.unwrap();
					dp[i + 1][j + 1][2] = vec![
						map[i][j].get(1).unwrap_or(&0) + dp[i + 1][j][0],
						map[i][j].get(0).unwrap_or(&0) + dp[i + 1][j][1],
						dp[i + 1][j][2],
						map[i][j].get(1).unwrap_or(&0) + *dp[i][j + 1].iter().max().unwrap(),
					]
					.into_iter()
					.max()
					.unwrap();
					dp[i + 1][j + 1][3] = vec![
						map[i][j].get(2).unwrap_or(&0) + dp[i + 1][j][0],
						map[i][j].get(1).unwrap_or(&0) + dp[i + 1][j][1],
						map[i][j].get(0).unwrap_or(&0) + dp[i + 1][j][2],
						dp[i + 1][j][3],
						map[i][j].get(2).unwrap_or(&0) + *dp[i][j + 1].iter().max().unwrap(),
					]
					.into_iter()
					.max()
					.unwrap();
			...
			*dp[self.r][self.c].iter().max().unwrap()
		}
	}
}
