...
(defconstant +mod+ 100000000)

;;   f(a0, a0) + f(a0, a1) + f(a0, a2) + f(a0, a3) + ... + f(a0, an-1)
;; + f(a1, a0) + f(a1, a1) + f(a1, a2) + f(a1, a3) + ... + f(a1, an-1)
;; + f(a2, a0) + f(a2, a1) + f(a2, a2) + f(a2, a3) + ... + f(a2, an-1)
;; + f(a3, a0) + f(a3, a1) + f(a3, a2) + f(a3, a3) + ... + f(an-2, an-1)


(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum n)))
    (let ((sorted-a (sort (copy-seq a) #'>))
          (cumsum (cumsum a)))
      (flet ((sum-right (index)
               (- (aref cumsum n) (aref cumsum (1+ index)))))
        (let ((count (floor (- (loop for i from 0 below n
                                     sum (1+ (meguru-method -1 n
                                                            (lambda (x)
                                                              (if (= x -1) t
                                                                  (>= (+ (aref sorted-a i)
                                                                         (aref sorted-a x))
                                                                      +mod+))))))
                               (count-if (lambda (x) (>= x (floor +mod+ 2))) a))
                            2))
              (result (loop for i from 0 below n
                            sum (+ (* (aref a i) (- n i 1))
                                   (sum-right i)))))
          (format t "~A~%" (- result (* +mod+ count))))))))
