...
(defun main ()
  (input* ((n fixnum)
           (s string))
    (declare (ignore n))
    (let* ((s (split-string s :separator '(#\space)))
           (trie (make-trie)))
      (dolist (s s)
        (trie-insert trie s))
      (let ((result 0))
        (dolist (s s)
          (trie-traverse trie s
                         (lambda (node str index)
                           (declare (ignore str))
                           (unless (zerop index)
                             ;(format *error-output* ":str ~S :index ~S :node ~S~%" str index node)
                             (incf result (1- (trie-node-prefix-count node)))))))
        (format t "~A~%" (floor result 2))))))
