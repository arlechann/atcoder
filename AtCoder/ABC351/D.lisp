...
#+swank (declaim (optimize (speed 1) (safety 3) (debug 3)))
...
(defun in-grid-p (h w i j)
  (and (< -1 i h)
       (< -1 j w)))

(defun moves (h w i j)
  (remove-if-not (lambda (pair)
                   (in-grid-p h w (car pair) (cdr pair)))
                 (mapcar (lambda (pair)
                           (cons (+ i (car pair)) (+ j (cdr pair))))
                         '((0 . 1)
                           (1 . 0)
                           (0 . -1)
                           (-1 . 0)))))
...
(defun solve (h w grid)
  (let ((used1 (make-array (list h w) :initial-element nil))
        (used2 (make-hash-table :test 'equal)))
    (labels ((used1-p (key)
               (aref used1 (car key) (cdr key)))
             (set-used1 (key)
               (setf (aref used1 (car key) (cdr key)) t))
             (used2-p (key)
               (gethash key used2 nil))
             (set-used2 (key)
               (setf (gethash key used2) t))
             (clear-used2 () (clrhash used2))
             (dfs (i j)
               (let ((key (cons i j)))
                 (when (eq (aref grid i j) 'magnet)
                   (return-from dfs 0))
                 (when (eq (aref grid i j) 'near)
                   (when (used2-p key)
                     (return-from dfs 0))
                   (set-used2 key)
                   (return-from dfs 1))
                 ...
                 (set-used1 key)
                 (let ((ret 1))
                   (do-neighbors ((ni nj) (i j) ret)
                     (when (in-grid-p h w ni nj)
                       (incf ret (dfs ni nj))))
                   ret))))
      (let ((result 0))
        (do-combination ((i j) (h w))
          (cond ((eq (aref grid i j) 'near)
                 (maxf result 1))
                ((and (eq (aref grid i j) 'none)
                      (not (used1-p (cons i j))))
                 (maxf result (dfs i j))
                 (clear-used2))))
        result))))
...
(defun main ()
  (input* ((h fixnum)
           (w fixnum)
           (s (vector string h)))
    (let ((grid (make-array (list h w)
                            :element-type 'symbol
                            :initial-element 'none)))
      (do-combination ((i j) (h w))
        (when (char= (aref (aref s i) j) #\#)
          (setf (aref grid i j) 'magnet)
          (do-neighbors ((ni nj) (i j))
            (when (and (in-grid-p h w ni nj)
                       (not (eq (aref grid ni nj) 'magnet)))
              (setf (aref grid ni nj) 'near)))))
      (format t "~A~%" (solve h w grid)))))
