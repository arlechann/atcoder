...
int main() {
	...
	sort(ALL(a));
	ll a_min = a[0], a_max = a[n - 1];
	VLL min_diff(n + 1, 0), max_diff(n + 1, 0);
	RREP(i, n) {
		min_diff[i] = min_diff[i + 1] + (a[i] - a_min);
	}
	REP(i, n) {
		max_diff[i + 1] = max_diff[i] + (a_max - a[i]);
	}

	VLL result(q, 0);
	REP(i, q) {
		int lower = distance(a.begin(), lower_bound(ALL(a), x[i]));
		int upper = distance(a.begin(), upper_bound(ALL(a), x[i]));

		ll l = max_diff[lower] - ((a_max - x[i]) * lower);
		ll u = min_diff[upper] - ((x[i] - a_min) * (n - upper));
		result[i] = l + u;
	}
