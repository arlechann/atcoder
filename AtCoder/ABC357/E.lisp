...
(defun detect-loop (node prev-nodes in-loop-p used-p)
  (labels ((find-loop (now)
             ))
    (find-loop node)))

(defun count-loop-reachable (node next-nodes reachable-count)
  (labels ((count-nodes (now)
             (when (= now node)
               (return-from count-nodes 0))
             (1+ (count-nodes (aref next-nodes now))))
           (set-count (now count)
             (when (= now node)
               (return-from set-count))
             (setf (aref reachable-count now) count)
             (set-count (aref next-nodes now) count)))
    (set-count node (count-nodes node))))

(defun count-reachable (node next-nodes in-loop-p reachable-count)
  (labels ((rec (node)
             (unless (zerop (aref reachable-count node))
               (return-from rec (aref reachable-count node)))
             (setf (aref reachable-count node)
                   (rec (aref next-nodes node)))
             (unless (aref in-loop-p node)
               (incf (aref reachable-count node)))
             (aref reachable-count node)))
    (rec node)))
...
(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum1 n)))
    (let ((next-nodes a)
          (prev-nodes (make-array n :initial-element nil))
          (in-loop-p (make-array n :initial-element nil))
          (used-p (make-array n :initial-element nil))
          (result (make-array n :initial-element 0)))
      (dotimes (node n)
        (push node (aref prev-nodes (aref next-nodes node))))
      (dotimes (node n)
        (detect-loop n node prev-nodes in-loop-p used-p))
      (dotimes (node n)
        (when (and (aref in-loop-p node)
                   (zerop (aref result node)))
          (count-loop-reachable node next-nodes result)))
      (dotimes (node n)
        (when (and (not (aref in-loop-p node))
                   (zerop (aref result node)))
          (count-reachable node next-nodes in-loop-p result)))
      (format t "~A~%" (sum result)))))
