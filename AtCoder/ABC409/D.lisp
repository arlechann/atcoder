...
(defun solve ()
  (input* ((n fixnum)
           (s string))
    ;#?n #?s
    (let* ((start (loop for i from 0 below (1- n)
                      when (char> (aref s i)
                                  (aref s (1+ i)))
                        return i
                      finally (return n)))
           (end (loop for i from (+ start 2) below n
                      when (char> (aref s i)
                                  (aref s start))
                        return i
                      finally (return n))))
      ...
      (when (= start n)
        (println s)
        (return-from solve))
      (write-string (subseq s 0 start))
      (write-string (subseq s (1+ start) end))
      (write-char (aref s start))
      (write-string (subseq s end))
      (terpri))))
