...
(defun main ()
  (input* ((n fixnum)
           (l fixnum)
           (d (vector fixnum (1- n))))
    (unless (zerop (mod l 3))
      (println 0)
      (return-from main))
    (let* ((edge (floor l 3))
           (points (aprog1 (make-array n :initial-element 0)
                     (let ((now 0))
                       (dotimes (i (1- n))
                         (incf now (aref d i))
                         (setf (aref it (1+ i)) (mod now l))))
                     (sortf it #'<)))
           (counts (aprog1 (make-array l :initial-element 0)
                     (dotimes (i n)
                       (incf (aref it (aref points i))))))
           (result 0))
      (dotimes (i edge)
        (incf result
              (* (aref counts i)
                 (aref counts (+ i edge))
                 (aref counts (+ i edge edge)))))
      (println result))))
