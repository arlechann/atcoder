...
(defun main ()
  (input* ((n fixnum)
           (x (vector fixnum n))
           (uvw (vector (cons* fixnum1 fixnum1 fixnum null) (1- n))))
    (let* ((graph (aprog1 (make-array n :initial-element nil)
                    (dotimes (i (1- n))
                      (let* ((edge (aref uvw i))
                             ...
                             (to (second edge))
                             (weight (third edge)))
                        (push (cons weight to) (aref it from))
                        (push (cons weight from) (aref it to))))))
           (result (solve n x graph 0 -1)))
      (println result))))

(defun solve (n x graph node parent)
  (let ((edges (aref graph node))
        (cost 0)
        (rest (aref x node)))
    (dolist (edge edges)
      (let ((weight (car edge))
