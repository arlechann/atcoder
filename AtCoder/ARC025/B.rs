...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			...
			let mut cumsum = vec![vec![0; self.w + 1]; self.h + 1];
			for i in 0..self.h {
				for j in 0..self.w {
					cumsum[i + 1][j + 1] =
						c[i][j] + cumsum[i][j + 1] + cumsum[i + 1][j] - cumsum[i][j];
				}
			}
			...
			for i1 in 0..self.h {
				for j1 in 0..self.w {
					for i2 in i1 + 1..self.h + 1 {
						for j2 in j1 + 1..self.w + 1 {
							let count =
								cumsum[i2][j2] - cumsum[i2][j1] - cumsum[i1][j2] + cumsum[i1][j1];
							if count == 0 {
								result = max(result, (i2 - i1) * (j2 - j1));
							}
						}
					}
				}
