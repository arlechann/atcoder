...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			let mut row = vec![0; self.r];
			let mut column = vec![0; self.c];
			let mut has_drop = vec![HashMap::new(); self.r];

			for &(y, x) in self.yx.iter() {
				row[y - 1] += 1;
				column[x - 1] += 1;
			}
			...
			for i in 0..self.r {
				...
				let lb = column.lower_bound(self.k - row[i]);
				let ub = column.upper_bound(self.k - row[i]);
				let dup1 = has_drop[i].get(&(self.k - row[i])).unwrap_or(&0);
				let dup2 = has_drop[i].get(&(self.k - row[i] + 1)).unwrap_or(&0);
				let d = (ub - lb) as usize - dup1 + dup2;
				result += d;
			}
			result
		}
