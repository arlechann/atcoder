...
class UnionFind {
	...
	UnionFind(size_t size)
		: parents(size), rank(size, 0), tree_size(size, 1), is_circle(size, 0) {
	...
	int count_circle() {
		int ret = 0;
		REP(i, this->parents.size()) {
			if(this->parents[i] == i && this->is_circle[i] != 0) {
				ret++;
			}
		}
		return ret;
...
int main() {
	...
	int xy = uf.count_union(), x = uf.count_circle();
