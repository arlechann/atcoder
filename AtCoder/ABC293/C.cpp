...
int dfs(const VI2D& map, int row, int column, VI& is_used) {
	int h = map.size(), w = map[0].size();
	if(is_used[map[row][column]]) {
		return 0;
	}
	if(row == h - 1 && column == w - 1) {
		return 1;
	}
	is_used[map[row][column]] = 1;

	int ret = 0;
	if(row != h - 1) {
		ret += dfs(map, row + 1, column, is_used);
	}
	if(column != w - 1) {
		ret += dfs(map, row, column + 1, is_used);
	}
	is_used[map[row][column]] = 0;
...
int main() {
	...
	VI is_used(numbers.size(), 0);
	int result = dfs(b, 0, 0, is_used);
