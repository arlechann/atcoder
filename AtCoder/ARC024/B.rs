...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> isize {
			...
			let (head_color, head_count) = compressed[0];
			let (tail_color, tail_count) = compressed[len - 1];
			if head_color == tail_color {
				compressed.pop();
				compressed[0].1 += tail_count;
			}
			let max_count = compressed
				.iter()
