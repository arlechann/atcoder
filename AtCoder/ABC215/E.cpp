...
int cap_alpha_to_int(char c) {
	return static_cast<int>(c) - 'A';
}

int bitset_insert(int s, int x) {
	return (s | (1 << x));
}

bool bitset_contains(int s, int x) {
	return (s & (1 << x)) != 0;
}

const int N_MAX = 1000, CHAR_SET = 10;

int n;
string s;

int used[N_MAX + 1][1 << CHAR_SET][CHAR_SET + 1];
mint memo[N_MAX + 1][1 << CHAR_SET][CHAR_SET + 1];

mint dp(int x, int set, int last) {
	mint& m = memo[x][set][last];
	if(used[x][set][last] != 0) {
		return m;
	}
	used[x][set][last] = 1;

	int sx = cap_alpha_to_int(s[x]);
	int ns = last == CHAR_SET ? set : bitset_insert(set, last);
	if(x == n) {
		return m = ns == 0 ? mint(0) : mint(1);
	}

	m = 0;
	if(!bitset_contains(set, sx) || last == sx) {
		m += dp(x + 1, ns, sx);
	}
	m += dp(x + 1, ns, last);
	return m;
