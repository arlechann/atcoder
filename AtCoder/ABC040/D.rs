...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> String {
			let mut aby = self.aby.clone();
			aby.sort_by_key(|&(_, _, y)| std::cmp::Reverse(y));
			let mut vw = self.vw.clone();
			vw.sort_by_key(|&(_, w)| std::cmp::Reverse(w));
			let mut uf = UnionFind::new(self.n);
			let mut m = HashMap::new();

			let mut i = 0;
			for &(v, w) in vw.iter() {
				while i < self.m && aby[i].2 > w {
					uf.merge(aby[i].0 - 1, aby[i].1 - 1);
					i += 1;
				}
				let size = uf.size(v - 1);
				m.entry((v, w)).or_insert(size);
			}

			self.vw
				.iter()
				.map(|vw| m[vw].to_string())
				.collect_vec()
				.join("\n")
		}
	}
