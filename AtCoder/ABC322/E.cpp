...
const int N_MAX = 100;
const int PARAM_MAX = 1<<(3*5+1);
ll memo[N_MAX + 1][PARAM_MAX + 1];
int used[N_MAX + 1][PARAM_MAX + 1];

VI de_params(int params) {
	VI ret(k);
	REP(i, k) {
		ret[i]=params%8;
		params/=8;
...
int con_params(const VI& params) {
	int ret = 0;
	REP(i, k) {
		ret *= 8;
		ret += min(7,params[k-i-1]);
	}
	return ret;
}

ll dp(int x, int params) {
	if(used[x][params] != 0) {
		return memo[x][params];
	}
	used[x][params] = 1;

	VI dep = de_params(params);
	// REP(i,k) {cout << dep[i] << " ";}
	// cout << endl;

	if(x == n) {
		REP(i,k){
			if(dep[i]<p){return memo[x][params]=INFLL;}
		}
		return memo[x][params]=0;
	}

	ll ret = INFLL;

	ll nopick = dp(x + 1, params);
	chmin(ret,nopick);

	REP(i,k){
		dep[i] += a[x][i];
	}
	ll pick = dp(x + 1, con_params(dep))+c[x];
	chmin(ret,pick);

	return memo[x][params] = ret;
}
...
int main() {
	...
	ll result = dp(0, 0);
	cout << (result == INFLL ? -1 : result) << endl;
	return 0;
}
