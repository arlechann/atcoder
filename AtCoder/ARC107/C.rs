...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> u64 {
			const N_MAX: u64 = 50;
			let mut fact: Vec<u64> = vec![1; N_MAX as usize + 1];
			for i in 0..N_MAX as usize {
				fact[i + 1] = fact[i] * (i + 1) as u64 % MOD;
			}

			let mut uf_y = UnionFind::new(self.n);
			for i in 0..self.n {
				for j in i..self.n {
					if self.a[i]
						.iter()
			...
			let res_y: u64 = uf_y
				.groups()
				.iter()
				.map(|group| fact[group.len()])
				.fold(1, |acc, x| (acc * x) % MOD);

			let mut uf_x = UnionFind::new(self.n);
			for i in 0..self.n {
				for j in i..self.n {
					if (0..self.n)
						.map(|y| self.a[y][i])
						.zip((0..self.n).map(|y| self.a[y][j]))
						.all(|(a, b)| a + b <= self.k)
					{
						uf_x.merge(i, j);
					}
