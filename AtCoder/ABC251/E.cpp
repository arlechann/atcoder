...
int main() {
	...
	auto dp = make_vector({n, 2, 2}, INFLL);
	dp[0][0][0] = 0;
	dp[0][1][1] = a[0];

	RANGE(i, 1, n - 1) {
		REP(k, 2) {
			dp[i][1][k] = min(dp[i - 1][1][k], dp[i - 1][0][k]) + a[i];
			dp[i][0][k] = dp[i - 1][1][k];
		}
	}

	dp[n - 1][1][0] = min(dp[n - 2][0][0], dp[n - 2][1][0]) + a[n - 1];
	dp[n - 1][0][0] = INFLL;
	dp[n - 1][1][1] = min(dp[n - 2][0][1], dp[n - 2][1][1]) + a[n - 1];
	dp[n - 1][0][1] = dp[n - 2][1][1];

	ll result = INFLL;
	REP(j, 2) {
		REP(k, 2) {
			chmin(result, dp[n - 1][j][k]);
		}
	}
