...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> u64 {
			...
			let mut dp1 = vec![vec![0u64; 2]; n + 1];
			dp1[0][0] = 1;
			for i in 0..n {
				dp1[i + 1][0] = dp1[i][0] & if vec![4, 9].contains(&a[i]) { 0 } else { 1 };
				dp1[i + 1][1] = dp1[i][0] * (a[i] - a[i] / 5) + dp1[i][1] * 8;
			}
			let a_count = dp1[n][0] + dp1[n][1];
			...
			let mut dp2 = vec![vec![0u64; 2]; m + 1];
			dp2[0][0] = 1;
			for i in 0..m {
				dp2[i + 1][0] = dp2[i][0] & if vec![4, 9].contains(&b[i]) { 0 } else { 1 };
				dp2[i + 1][1] = dp2[i][0] * (b[i] - b[i] / 5) + dp2[i][1] * 8;
			}
			let b_count = dp2[m][0] + dp2[m][1];
