...
int main() {
	...
	auto used = make_vector({n + 1, A_MAX + 1}, mint(0));
	auto unused = make_vector({n + 1, A_MAX + 1}, mint(0));
	used[0][0] = 1;
	REP(i, n) {
		used[i + 1][0] = 1;
		REP(j, A_MAX) {
			if(j + 1 - ab[i].second >= 0) {
				used[i + 1][j + 1] = used[i][j + 1 - ab[i].second] +
									 unused[i][j + 1 - ab[i].second];
			}
			unused[i + 1][j + 1] = used[i][j + 1] + unused[i][j + 1];
		}
	}

	mint result = 0;
	REP(i, n) {
		REP(j, ab[i].first) { result += used[i + 1][j + 1]; }
	}

	cout << result << endl;

	// REP(i, *max_element(ALL(a)) + 1) {
	// 	REP(j, n + 1) { cout << used[j][i] << " "; }
	// 	cout << endl;
	// }
	// cout << endl;
	// REP(i, *max_element(ALL(a)) + 1) {
	// 	REP(j, n + 1) { cout << unused[j][i] << " "; }
	// 	cout << endl;
	// }
	return 0;
}
