...
(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (s (vector string n)))
    (let ((grid (aprog1 (make-array (list n n) :element-type 'character :initial-element #\.)
                  (dotimes (i n)
                    (dotimes (j n)
                      (setf (aref it i j) (aref (aref s i) j))))))
          (hashset (make-hash-table :test #'equal)))
      (dotimes (i1 (1+ (- n m)))
        (dotimes (j1 (1+ (- n m)))
          (let ((str (make-array (* m m) :element-type 'character :initial-element #\.)))
            (dotimes (i2 m)
              (dotimes (j2 m)
                (setf (aref str (+ (* i2 m) j2)) (aref grid (+ i1 i2) (+ j1 j2)))))
            (setf (gethash str hashset) t))))
      (println (hash-table-count hashset)))))
