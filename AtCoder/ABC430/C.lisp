...
(defun main ()
  (input* ((n fixnum)
           ...
           (b fixnum)
           (s string))
    (let* ((sa (map 'vector #'(lambda (c) (if (char= c #\a) 1 0)) s))
           (sb (map 'vector #'(lambda (c) (if (char= c #\b) 1 0)) s))
           (a-cumsum (cumulate sa))
           (b-cumsum (cumulate sb))
           (result 0))
      ;; #?sa #?sb #?a-cumsum #?b-cumsum
      (dotimes (l n)
        (let* ((a-count (- n (meguru-method l (1+ n) #'(lambda (index) (< (- (aref a-cumsum index) (aref a-cumsum l)) a)))))
               (b-count (- (meguru-method l (1+ n) #'(lambda (index) (< (- (aref b-cumsum index) (aref b-cumsum l)) b))) l))
               (d (- (+ a-count b-count) (- n l))))
          ;; #?l #?a-count #?b-count #?d
          (incf result (max d 0))))
      (println result))))
