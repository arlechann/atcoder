...
(defun main ()
  (input* ((n fixnum)
           (x (vector fixnum n)))
    (let* ((line (coerce (sort (cons 0 (coerce x 'list)) #'<) 'vector))
           (order (aprog1 (make-hash-table)
                    (dotimes (i (1+ n))
                      (setf (gethash (aref line i) it) i))))
           (next (aprog1 (make-array (1+ n) :initial-element -1)
                   (dotimes (i n)
                     (setf (aref it i) (1+ i)))))
           (prev (aprog1 (make-array (1+ n) :initial-element -1)
                   (dotimes (i n)
                     (setf (aref it (1+ i)) i))))
           (ds (aprog1 (make-array (1+ n) :initial-element 0)
                 (dotimes (i (1+ n))
                   (setf (aref it i)
                         (cond ((zerop i) (aref line 1))
                               ((= i n) (- (aref line n) (aref line (1- n))))
                               (t (min (- (aref line (1+ i)) (aref line i))
                                       (- (aref line i) (aref line (1- i))))))))))
           (d (sum ds))
           (result nil))
      (flet ((update-d (ord)
               (when (< ord 0)
                 (return-from update-d))
               (let* ((prev-ord (aref prev ord))
                      (next-ord (aref next ord))
                      (pos (aref line ord))
                      (prev-pos (if (>= prev-ord 0) (aref line prev-ord) (- *inf*)))
                      (next-pos (if (>= next-ord 0) (aref line next-ord) *inf*))
                      (now-d (aref ds ord))
                      (new-d (min (- next-pos pos) (- pos prev-pos))))
                 (decf d now-d)
                 (incf d new-d)
                 (decf (aref ds ord) now-d)
                 (incf (aref ds ord) new-d))))
        (loop for i from (1- n) downto 0
              for pos = (aref x i)
              for ord = (gethash pos order)
              for prev-ord = (aref prev ord)
              for next-ord = (aref next ord)
              ;; do #?i #?pos #?ord #?prev-ord #?next-ord
              do (push d result)
              do (decf d (aref ds ord))
                 (setf (aref ds ord) 0)
                 (setf (aref next prev-ord) next-ord)
                 (when (>= next-ord 0) (setf (aref prev next-ord) prev-ord))
              do (update-d prev-ord)
                 (update-d next-ord))
