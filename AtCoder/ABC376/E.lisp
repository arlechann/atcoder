...
(defconstant +inf+ 100000000000000000)
  ...
  (input* ((n fixnum)
    ...
    (when (= k 1)
      (let ((ret +inf+))
        (return-from solve
          (dotimes (i n ret)
            (minf ret (* (aref a i) (aref b i)))))))
    (let* ((ab (aprog1 (make-array n)
                 (dotimes (i n (sort it #'< :key #'car))
                   (setf (aref it i)
                         (cons (aref a i) (aref b i))))))
           (b-sum 0)
           (b-top (make-binary-heap #'>))
           (result +inf+))
      (loop for i below k
            for bi = (cdr (aref ab i))
            do (incf b-sum bi)
               (binary-heap-push bi b-top)
            finally (setf result (* (car (aref ab (1- k))) b-sum)))
      (loop for i from k below n
            for ai = (car (aref ab i))
            for bi = (cdr (aref ab i))
            for b-max = (aprog1 (binary-heap-top b-top)
                          (decf b-sum it)
                          (binary-heap-pop b-top))
            do (minf result
                     (* ai (+ b-sum bi)))
               (incf b-sum (min b-max bi))
               (binary-heap-push (min b-max bi) b-top)
            finally (return result)))))
