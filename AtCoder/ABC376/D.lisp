...
(defun main ()
  (input* ((n fixnum)
    ...
    (let ((graph (make-adlist-graph n)))
      (dotimes (i m)
        (let* ((edge (aref ab i))
               (from (car edge))
               (to (cdr edge)))
          (graph-add-edge graph from to)))
      (let* ((distances (bfs graph :start 0 :end 0))
             (result (aref distances 0)))
        (format t "~A~%" (if (zerop result) -1 result))))))
