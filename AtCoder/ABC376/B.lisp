...
(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (h (vector (cons symbol fixnum1) q)))
    (let ((result 0)
          (left 0)
          (right 1))
      (labels ((up-dist (from to)
                 (if (<= from to)
                     (- to from)
                     (- (+ to n) from)))
               (down-dist (from to)
                 (up-dist to from))
               (up-movable-p (from to other)
                 (< (up-dist from to)
                    (up-dist from other)))
               (down-movable-p (from to other)
                 (< (down-dist from to)
                    (down-dist from other)))
               (move (from to other)
                 (min (if (up-movable-p from to other)
                          (up-dist from to)
                          +inf+)
                      (if (down-movable-p from to other)
                          (down-dist from to)
                          +inf+))))
        (dotimes (i q)
          (let* ((ope (aref h i))
            ...
            (incf result
                  (if (eq hand 'l)
                      (move left move-to right)
                      (move right move-to left)))
            (if (eq hand 'l)
                (setf left move-to)
                (setf right move-to))))
        (format t "~A~%" result)))))
