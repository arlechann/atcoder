...
(eval-when (:compile-toplevel :execute)
  (def-delimiter-macro #\[ #\] (arr &rest args)
    `(aref ,arr ,@args)))
...
(defun union-find-all-group-size (uf)
  (let ((ret nil))
    (dotimes (i (length (union-find-parents uf)) ret)
      (when (= (aref (union-find-parents uf) i) i)
        (push (union-find-group-size uf i) ret)))))
...
(defun permutations (lst)
  (let ((ret nil))
    (nlet rec ((lst lst) (acc nil))
      (if (null lst)
          (push acc ret)
          (dolist (x lst)
            (rec (remove x lst) (cons x acc)))))
    ret))

(defun swap-count (lst)
  (let* ((arr (map 'vector #'identity lst))
         (len (length arr))
         (ret 0))
    (dotimes (i len)
      (dotimes (j (1- len))
        (when (> #[arr j] #[arr (1+ j)])
          (rotatef #[arr j] #[arr (1+ j)])
          (incf ret))))
    ret))
...
(defun swap-array (h w row-perm col-perm a)
  (let ((ret (make-array (list h w))))
    (nlet rec-row ((row-perm row-perm) (i 0))
      (unless (null row-perm)
        (nlet rec-col ((col-perm col-perm) (j 0))
          (unless (null col-perm)
            (setf #[ret i j]
                  #[a (car row-perm) (car col-perm)])
            (rec-col (cdr col-perm) (1+ j))))
        (rec-row (cdr row-perm) (1+ i))))
    ret))

(defun solve (h w a b)
  (let ((row-perms (permutations (iota h)))
        (col-perms (permutations (iota w))))
    (nlet rec-row ((row-perms row-perms) (result *inf*))
      (if (null row-perms)
          result
          (rec-row (cdr row-perms)
                   (min result
                        (nlet rec-col ((col-perms col-perms) (result result))
                          (if (null col-perms)
                              result
                              (if (equalp (swap-array h w (car row-perms) (car col-perms) a)
                                          b)
                                  (rec-col (cdr col-perms) (min result
                                                                (+ (swap-count (car row-perms))
                                                                   (swap-count (car col-perms)))))
                                  (rec-col (cdr col-perms) result))))))))))
