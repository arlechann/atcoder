...
(defun main ()
  (input* ((n fixnum)
           ...
           (s string))
    (let ((days (length s)))
      (let ((result (nlet rec ((day 0) (logo 0) (muji m) (result 0))
                      (if (= day days)
                          (- result)
                          (let ((schedule (elt s day)))
                            (cond ((char= schedule #\0)
                                   (rec (1+ day) 0 m result))
                                  ((char= schedule #\1)
                                   (if (> muji 0)
                                       (rec (1+ day) logo (1- muji) result)
                                       (rec (1+ day) (1- logo) 0 (min result
                                                                      (1- logo)))))
                                  (t (rec (1+ day) (1- logo) muji (min result
                                                                       (1- logo))))))))))
        (format t "~A~%" result)))))
