...
(defun main ()
  (input* ((k fixnum)
           (g fixnum)
           (m fixnum))
    (let ((result (nlet rec ((k k) (grass 0) (mag 0))
                    (cond ((zerop k) (list grass mag))
                          ((= grass g) (rec (1- k) 0 mag))
                          ((zerop mag) (rec (1- k) grass m))
                          (t (let ((grass-rest (- g grass)))
                               (if (<= grass-rest mag)
                                   (rec (1- k) g (- mag grass-rest))
                                   (rec (1- k) (+ grass mag) 0))))))))
      (format t "~S ~S~%" (car result) (cadr result)))))
...
(labels ((test-case (input expect)
  ...
  (test-case "5 300 500" "200 500")
  (test-case "5 100 200" "0 0")
  )
