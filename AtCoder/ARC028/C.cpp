...
int dp(const VI2D& edges, int node, int parent) {
	int n = edges.size();
	int& ret = memo[node][parent];
	if(ret != 0) {
		return ret;
	}
	ret = 1;
	EACH(next, edges[node]) {
		...
		if(memo[node][next] != 0) {
			ret += n - memo[node][next];
		} else {
			ret += dp(edges, next, node);
		}
	}
	return ret;
}

int solve(const VI2D& edges, int node) {
	int n = edges.size();
	int ret = 0;
	EACH(next, edges[node]) {
		if(memo[node][next] != 0) {
			chmax(ret, n - memo[node][next]);
		} else {
			chmax(ret, dp(edges, next, node));
		}
	}
	return ret;
}
...
int main() {
	...
	REP(i, n) { cout << solve(edges, i) << endl; }

	// REP(i, n) {
	// 	cout << "node: " << i << endl;
	// 	EACH(next, edges[i]) {
	// 		cout << "\tfrom: " << next << ", value: " << memo[i][next] << endl;
	// 	}
	// }
	// cout << endl;
