...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> isize {
			let max_weight = self.n * 10;
			let mut dp = vec![vec![vec![INF; max_weight + 1]; max_weight + 1]; self.n + 1];
			dp[0][0][0] = 0;
			...
			for i in 0..self.n {
				...
				for j in 0..max_weight {
					for k in 0..max_weight {
						let (a, b, c) = self.abc[i];
						if j + 1 >= a && k + 1 >= b {
							dp[i + 1][j + 1][k + 1] =
								min(c + dp[i][j + 1 - a][k + 1 - b], dp[i][j + 1][k + 1]);
						} else {
			...
			let (ma, mb) = self.m;
			let result = (1..)
				.take_while(|i| ma * i <= max_weight && mb * i <= max_weight)
				.map(|i| dp[self.n][ma * i][mb * i])
				.min()
				.unwrap();
