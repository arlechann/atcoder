...
(defun sortp (sequence &optional (compare #'<))
  (let ((prev nil))
    (map nil
         (lambda (x)
           (unless (null prev)
             (unless (funcall compare prev x)
               (return-from sortp nil)))
           (setf prev x))
         sequence)
    t))

(defun main ()
  (input* ((a (vector fixnum 5)))
    (loop for i below 4
          do (rotatef (aref a i) (aref a (1+ i)))
          when (sortp a)
            do (println "Yes")
               (return-from main)
          do (rotatef (aref a i) (aref a (1+ i))))
...
(defun test ()
  (test-case "1 2 4 3 5" "Yes")
  (test-case "5 3 2 4 1" "No")
  (test-case "1 2 3 4 5" "No")
  (test-case "2 1 3 4 5" "Yes")
  )
