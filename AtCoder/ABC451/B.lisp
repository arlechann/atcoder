...
(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (ab (vector (cons* fixnum1 fixnum1) n)))
    (loop with now = (make-array m :initial-element 0)
          and next = (make-array m :initial-element 0)
          for (a . b) across ab
          do (incf (aref now a))
             (incf (aref next b))
          finally (loop for i from 0 below m
                        do (println (- (aref next i) (aref now i)))))))
