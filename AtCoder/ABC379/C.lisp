...
(defun sum-of-one-diff-seq (length)
  (floor (* length (1- length)) 2))
...
(defun main ()
  (input* ((n fixnum)
           ...
           (x (vector fixnum1 m))
           (a (vector fixnum m)))
    (let ((stones (map 'vector #'cons x a)))
      (sortf stones #'< :key #'car)
      (let ((result 0)
            (fill-right 0))
        (loop for (index . nums) across stones
              when (< fill-right index)
                do (println "-1")
                   (return-from main)
              do (incf result (+ (* nums (- fill-right index))
                                 (sum-of-one-diff-seq nums)))
                 (incf fill-right nums))
        (when (/= fill-right n)
          (println -1)
          (return-from main))
        (format t "~A~%" result)))))
