...
(defun main ()
  (input* ((q fixnum))
    (let ((query-types (make-array q :initial-element 0))
          (query-params (make-array q :initial-element 0)))
      (labels ((get-type (i) (aref query-types i))
               ((setf get-type) (value i) (setf (aref query-types i) value))
               (get-param (i) (aref query-params i))
               ((setf get-param) (value i) (setf (aref query-params i) value)))
        (dotimes (i q)
          (setf (get-type i) (read))
          (unless (= (get-type i) 1)
            (setf (get-param i) (read))))
        (let* ((plant-count (count 1 query-types))
               (plants (aprog1 (make-array plant-count :initial-element 0)
                         (let ((last-plant 0))
                           (dotimes (i q)
                             (cond ((= (get-type i) 1)
                                    (incf last-plant))
                                   ((and (= (get-type i) 2) (/= last-plant 0))
                                    (incf (aref it (1- last-plant))
                                          (get-param i))))))
                         (loop for i from (1- plant-count) downto 1
                               do (incf (aref it (1- i))
                                        (aref it i)))))
               (harvests nil))
          (loop with grow = 0
                for i from (1- q) downto 0
                do (cond ((= (get-type i) 2)
                          (incf grow (get-param i)))
                         ((= (get-type i) 3)
                          (push (1+ (meguru-method -1 plant-count
                                                   #'(lambda (x)
                                                       (cond ((< x 0) t)
                                                             ((>= x plant-count) nil)
                                                             (t (>= (- (aref plants x) grow)
                                                                    (get-param i)))))))
                                harvests))))
          ;#?plants #?harvests
          (loop with harvested = 0
                for harvest in harvests
                do (format t "~A~%" (- harvest harvested))
                   (setf harvested harvest)))))))
