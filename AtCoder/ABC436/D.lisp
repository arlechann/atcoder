...
(defun main ()
  (input* ((h fixnum)
           (w fixnum)
           (s (vector string h)))
    (let* ((warps (make-array (1+ (count-alphabet)) :initial-element nil))
           (warp-used (make-array (1+ (count-alphabet)) :initial-element nil))
           (maze (aprog1 (make-array (list h w) :initial-element 0)
                   (dotimes (i h)
                     (dotimes (j w)
                       (let ((c (aref (aref s i) j)))
                         (case c
                           (#\. nil)
                           (#\# (setf (aref it i j) -1))
                           (otherwise (progn (setf (aref it i j) (1+ (char-to-index c)))
                                             (push (cons i j) (aref warps (1+ (char-to-index c))))))))))))
           (distances (aprog1 (make-array (list h w) :initial-element *inf*)
                        (setf (aref it 0 0) 0))))
      (loop with queue = (make-deque)
            initially (deque-push-back queue (cons 0 0))
            until (deque-empty-p queue)
            for now = (prog1 (deque-peak-front queue)
                        (deque-pop-front queue))
            for y = (car now)
            and x = (cdr now)
            for d = (aref distances y x)
            do (do-4neighbors ((ny nx) (y x))
                 (when (and (< -1 ny h)
                            (< -1 nx w)
                            (>= (aref maze ny nx) 0)
                            (= (aref distances ny nx) *inf*))
                   (setf (aref distances ny nx) (1+ d))
                   (deque-push-back queue (cons ny nx))))
            when (and (> (aref maze y x) 0)
                      (not (aref warp-used (aref maze y x))))
              do (setf (aref warp-used (aref maze y x))
                       t)
                 (dolist (dist (aref warps (aref maze y x)))
                   (let ((ny (car dist))
                         (nx (cdr dist)))
                     (unless (and (= ny y) (= nx x))
                       (setf (aref distances ny nx) (1+ d))
                       (deque-push-back queue dist))))
            finally (let ((result (aref distances (1- h) (1- w))))
                      ;; #?warps #?maze #?distances
                      (println (if (= result *inf*) -1 result)))))))
