...
(defun main ()
  (input* ((n fixnum))
    (loop with cells = (make-array (list n n) :initial-element nil)
          and count = 0
          and row = 0
          and column = (floor (1- n) 2)
          for i from 0 below (* n n)
          do (incf count)
             (setf (aref cells row column) count)
             (cond ((null (aref cells (mod (1- row) n) (mod (1+ column) n)))
                    (setf row (mod (1- row) n)
                          column (mod (1+ column) n)))
                   (t (setf row (mod (1+ row) n))))
          finally (dotimes (i n)
                    (dotimes (j n)
                      (format t "~A " (aref cells i j)))
                    (terpri)))))
