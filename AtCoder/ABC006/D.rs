...
mod solve {
	...
	use slice_utils::bin_search::LowerBoundExt;

	fn lis(v: &[usize]) -> Vec<usize> {
		let mut dp = vec![];
		for &e in v.iter() {
			if dp.is_empty() || e > *dp.last().unwrap() {
				dp.push(e);
			} else {
				let i = dp.lower_bound(e);
				dp[i] = e;
			}
		}
		dp
...
mod slice_utils {
	pub mod bin_search {
		...
		impl<T: Eq + Ord> LowerBoundExt<T> for [T] {
			fn lower_bound(&self, x: T) -> usize {
				let mut ng: isize = -1;
				let mut ok: isize = self.len() as isize - 1;
				while (ok - ng).abs() > 1 {
					let middle = (ok + ng) / 2;
