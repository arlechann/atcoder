...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> String {
			for b in 0..self.n {
				let c = bin_search(0, self.n + 1, move |c| b + c * 2 <= self.m - self.n * 2);
				let a = self.n - b - c;
				if a >= 0 && b >= 0 && c >= 0 && a * 2 + b * 3 + c * 4 == self.m {
					return format!("{} {} {}", a, b, c);
				}
			}
			"-1 -1 -1".to_string()
	...
	fn bin_search<F: Fn(isize) -> bool>(mut ok: isize, mut ng: isize, pred: F) -> isize {
		while (ok - ng).abs() > 1 {
			let middle = (ok + ng) / 2;
			if pred(middle) {
				ok = middle;
