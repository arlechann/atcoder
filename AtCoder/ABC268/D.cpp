...
std::string join(const std::vector<std::string>& v, char delim = 0) {
	std::string s;
	if(!v.empty()) {
		s += v[0];
...
pair<int, string>
solve2(const vector<string>& s, vector<int>& used, int count, string x);

pair<int, string>
solve1(const vector<string>& s, vector<int>& used, int count, string x) {
	int n = s.size();
	int len = x.size();
	if(len > 16) {
		return {0, ""};
	...
	REP(i, n) {
		...
		used[i] = 1;
		p += s[i];
		auto [status, result] = solve2(s, used, count + 1, p);
		used[i] = 0;
		if(status == 1) {
			return {1, result};
		}
...
solve2(const vector<string>& s, vector<int>& used, int count, string x) {
	...
	for(;;) {
		...
		auto [status, result] = solve1(s, used, count, x);
		if(status == 1) {
			return {1, result};
		} else if(status == 0) {
			return {-1, ""};
...
int main() {
	...
	VI used(n, 0);
	auto [status, result] = solve1(s, used, 0, "");
