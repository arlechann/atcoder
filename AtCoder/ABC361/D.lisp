...
(defun next-states (s)
  (let* ((len (length s))
         (dot-index (position #\. s))
         (ret nil))
    (dotimes (i (1- len) ret)
      (unless (<= (1- dot-index) i (1+ dot-index))
        (let ((copy-s (copy-seq s)))
          (rotatef (aref copy-s i)
                   (aref copy-s dot-index))
          (rotatef (aref copy-s (1+ i))
                   (aref copy-s (1+ dot-index)))
          (push copy-s ret))))))

(defun solve (n s1 tt)
  (let ((s (make-array (+ n 2) :element-type 'character :initial-element #\.))
        (moves (make-hash-table :test #'equal))
        (deque (make-deque)))
    (dotimes (i n)
      (setf (aref s i) (aref s1 i)))
