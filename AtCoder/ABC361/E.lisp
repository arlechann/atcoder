...
(defun make-edge (from to cost) (list from to cost))
(defun edge-from (edge) (first edge))
(defun edge-to (edge) (second edge))
(defun edge-cost (edge) (third edge))

(defun edge-reverse (edge)
  (make-edge (edge-to edge)
             (edge-from edge)
             (edge-cost edge)))

(defun farthest-node-and-distance (edges node)
  (let ((farthest 0)
        (result node))
    (labels ((rec (node parent distance)
               (let ((next-edges (aref edges node)))
                 (when (and (length1p next-edges)
                            (/= node parent))
                   (when (> distance farthest)
                     (setf farthest distance
                           result node))
                   (return-from rec))
                 (dolist (edge next-edges)
                   (let* ((to (edge-to edge))
                          (cost (edge-cost edge)))
                     (unless (= to parent)
                       (rec to node (+ distance cost))))))))
      (rec node node 0))
    (values result farthest)))

(defun solve (edges)
  (let ((start (farthest-node-and-distance edges 0))
        (sum-cost (reduce (lambda (acc edges)
                            (+ acc
                               (reduce (lambda (acc edge)
                                         (+ acc (edge-cost edge)))
                                       edges
                                       :initial-value 0)))
                          edges
                          :initial-value 0)))
    (multiple-value-bind (end diameter)
        (farthest-node-and-distance edges start)
      (declare (ignore end))
        (- sum-cost diameter))))

(defun main ()
  (input* ((n fixnum)
           (abc (vector (cons* fixnum1 fixnum1 fixnum nil) (1- n))))
    (let ((edges (make-array n :initial-element nil)))
      (dotimes (i (1- n))
        (let ((edge (aref abc i)))
          (push edge (aref edges (edge-from edge)))
          (push (edge-reverse edge) (aref edges (edge-to edge)))))
