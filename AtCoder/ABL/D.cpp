...
class SegmentTree {
	...
	void debug_print() const {
		int offset = this->size - 1;
		RANGE(i, offset, offset + 16) { cout << this->tree[i] << " "; }
		cout << endl;
	}
...
int main() {
	...
	VI last(300010, 0);
	SegmentTree<int> st(
		last, [](int a, int b) { return max(a, b); }, 0);
	REP(i, n) {
		int m = st.find(max(a[i] - k, 0), min(a[i] + k + 1, 300010));
		st.update(a[i], m + 1, [](int a, int b) { return max(a, b); });
	}

	cout << st.find(0, 300010) << endl;
	return 0;
}
