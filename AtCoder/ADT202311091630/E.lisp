...
(defun main ()
  (input* ((n fixnum)
           (fs (vector (list fixnum1 fixnum) n)))
    (let ((f (map 'vector #'car fs))
          (s (map 'vector #'cadr fs)))
      (let ((result 0)
            (max2 (list 0 0))
            (tastes (make-array n :element-type 'list :initial-element nil)))
        (dotimes (i n)
          (push (aref s i) (aref tastes (aref f i))))
        (dotimes (i n)
          (let ((ss (aref tastes i)))
            (setf ss (sort ss #'>))
            (unless (null (car ss))
              (setf max2 (cdr (sort (cons (car ss) max2) #'<))))
            (unless (null (cadr ss))
              (setf result (max result (+ (car ss) (floor (cadr ss) 2)))))))
        (setf s (sort s #'>))
        (setf result (max result (apply #'+ max2)))
        (format t "~A~%" result)))))
