...
(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (s (vector string n)))
    (let ((pairs (nlet rec ((i 0) (j 1) (acc nil))
                   (if (>= i n)
                       (nreverse acc)
                       (if (>= j n)
                           (rec (1+ i) (+ i 2) acc)
                           (rec i (1+ j) (cons (cons i j) acc)))))))
      (format t "~A~%"
              (apply #'+ (mapcar (lambda (p)
                                   (let ((i (car p))
                                         (j (cdr p)))
                                     (nlet rec ((k 0) (solvable t))
                                       (if (= k m)
                                           (if solvable 1 0)
                                           (rec (1+ k) (and solvable
                                                            (or (char= (aref (aref s i) k) #\o)
                                                                (char= (aref (aref s j) k) #\o))))))))
                                 pairs))))))
