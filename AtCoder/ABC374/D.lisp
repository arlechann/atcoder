...
(defun main ()
  (input* ((n fixnum)
           (s double)
           (tt double)
           (lines (list (cons* (cons* fixnum fixnum) (cons* fixnum fixnum)) n)))
    (let ((*read-default-float-format* 'double-float)
          (result +inf+))
      (dolist (lines (permutations lines))
        (dotimes (k (ash 1 n))
          (let ((time 0.0d0)
                (pos-y 0)
                (pos-x 0))
            ...
            (loop for i from 0
                  ...
                  when (logbitp i k)
                    do (rotatef from to)
                  do (let ((dist-to-start (sqrt (+ (square (diff pos-y (car from)))
                                                   (square (diff pos-x (cdr from))))))
                           (line-length (sqrt (+ (square (diff (car from) (car to)))
                                                 (square (diff (cdr from) (cdr to)))))))
                       (incf time (/ dist-to-start s))
                       (incf time (/ line-length tt))
                       (setf pos-y (car to)
                             pos-x (cdr to))))
            (minf result time))))
      (format t "~A~%" result))))
