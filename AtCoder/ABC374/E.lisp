...
(defun process-a (p) (first p))
(defun process-p (p) (second p))
(defun process-b (p) (third p))
(defun process-q (p) (fourth p))

(defconstant +inf+ 1000000000000)

(defun main ()
  (input* ((n fixnum)
           (x fixnum)
           (processes (vector (cons* fixnum fixnum fixnum fixnum nil) n)))
    (labels ((process (i) (aref processes i))
             (min-cost (i norma)
               (let* ((process (process i))
                      ...
                      (b (process-b process))
                      (q (process-q process))
                      (s-max (ceiling norma a))
                      (t-max (ceiling norma b)))
                 +inf+)))
      (let ((result (meguru-method 0 +inf+
                                   (lambda (w)
                                     (>= x (loop for i below n
                                                 sum (min-cost i w)))))))
        (format t "~A~%" result)))))
