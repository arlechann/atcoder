	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			...
			let even = self
				...
				.map(|(_, &x)| x)
				.collect_vec();
			let mut odd_count = vec![0; 11];
			let mut even_count = vec![0; 11];
			for &x in odd.iter() {
				odd_count[x] += 1;
			}
			for &x in even.iter() {
			...
			let odd_max_color = (1..=10).max_by_key(|&i| odd_count[i]).unwrap();
			let even_max_color = (1..=10).max_by_key(|&i| even_count[i]).unwrap();
			let odd_max2_color = (1..=10)
				.filter(|&i| i != odd_max_color)
				.max_by_key(|&i| odd_count[i])
				.unwrap();
			let even_max2_color: usize = (1..=10)
				.filter(|&i| i != even_max_color)
				.max_by_key(|&i| even_count[i])
				.unwrap();

			if odd_max_color == even_max_color {
				(self.n
					- max(
						odd_count[odd_max_color] + even_count[even_max2_color],
						odd_count[odd_max2_color] + even_count[even_max_color],
					)) * self.c
			} else {
				(self.n - (odd_count[odd_max_color] + even_count[even_max_color])) * self.c
			}
		}
	}
}
