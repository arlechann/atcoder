...
(defun bfs (graph &key (start 0) end)
  (dolist (s (ensure-list start))
    (assert-graph-node-index graph s "BFS"))
  (when end
  ...
  (loop with size = (graph-size graph)
        ...
        initially (dolist (s (ensure-list start))
                    (setf (aref distances s) 0
                          (aref usedp s) t)
  ...
  (let* ((lines (split-string text :separator '(#\Newline)))
         (filtered (nmap #'(lambda (line)
                         ...
                         (delete-if #'(lambda (line)
                                        (string-prefix-p "```" (trim-whitespace line)))
                                    lines))))
    (trim-whitespace (atcoder-html-unescape (strjoin filtered)))))

(defun detect-sample-kind (heading)
  (let ((h (string-downcase (trim-whitespace heading))))
...
(defun main ()
  (input* ((h fixnum)
           ...
           (k fixnum)
           (s (vector string h)))
    (let* ((grid (make-grid-graph h w s))
           (danger-row (aprog1 (make-array h :initial-element nil)
                         (dotimes (i h)
                           (dotimes (j w)
                             (when (char= (aref (aref s i) j) #\#)
                               (setf (aref it i) t))))))
           (danger-col (aprog1 (make-array w :initial-element nil)
                         (dotimes (i h)
                           (dotimes (j w)
                             (when (char= (aref (aref s i) j) #\#)
                               (setf (aref it j) t))))))
           (safe-cells (loop for i from 0 below h
                             unless (aref danger-row i)
                               nconc
                               (loop for j from 0 below w
                                     unless (aref danger-col j)
                                       collect (grid-point-to-index grid i j))))
           (distances (bfs grid :start safe-cells))
           (result (count-if #'(lambda (d) (<= d k)) distances)))
      ;; #?danger-row #?danger-col #?safe-cells #?distances
      (println result))))
