...
int main() {
	...
	REP(i, q) {
		...
		if(is_box_empty[a] && is_box_empty[b]) {
			// Nothing
		} else if(is_box_empty[a]) {
			// Nothing
		} else if(is_box_empty[b]) {
			colors[a].swap(colors[b]);
			is_box_empty[a] = 1;
			is_box_empty[b] = 0;
		} else {
			int a_color_size = colors[a].size(),
				b_color_size = colors[b].size();
			if(a_color_size > b_color_size) {
				colors[a].swap(colors[b]);
			}
		...
		result[i] = is_box_empty[b] ? 0 : colors[b].size();
	}
