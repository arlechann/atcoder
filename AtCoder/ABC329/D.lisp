...
(defun main ()
  (input* ((n fixnum)
	   ...
	   (a (vector fixnum1 m)))
    (declare (ignorable n))
    (let ((result (nlet rec ((i 0) (votes (make-rbtree :key #'car)) (max-vote 0) (winners nil))
		    (if (= i m)
			(nreverse winners)
			(let* ((candidate (aref a i))
			       (now-vote (cdr (rbtree-search votes candidate :default (cons -1 0)))))
			  (setf votes (rbtree-insert votes (cons candidate (1+ now-vote))))
			  (let ((prev-winner (car winners)))
			    (cond ((= (1+ now-vote) max-vote)
				   (if (< candidate prev-winner)
				       (rec (1+ i) votes max-vote (cons candidate winners))
				       (rec (1+ i) votes max-vote (cons prev-winner winners))))
				  ((> (1+ now-vote) max-vote)
				   (rec (1+ i) votes (1+ now-vote) (cons candidate winners)))
				  (t (rec (1+ i) votes max-vote (cons prev-winner winners))))))))))
      (mapc (lambda (winner)
	      (format t "~A~%" (1+ winner)))
	    result))))
