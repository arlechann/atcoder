...
(defun main ()
  (input* ((n fixnum)
           (s string))
    (let ((ss (make-array #.(1+ (- (char-code #\z) (char-code #\a)))
			  :element-type 'fixnum
			  :initial-element 0)))
      (nlet rec ((i 0) (prev (aref s 0)) (len 0))
	(if (= i n)
	    (setf (aref ss (char-index prev))
		  (max len (aref ss (char-index prev))))
	    (let ((c (aref s i)))
	      (if (char= c prev)
		  (rec (1+ i) c (1+ len))
		  (progn (setf (aref ss (char-index prev))
			       (max len (aref ss (char-index prev))))
			 (rec (1+ i) c 1))))))
      (format t "~A~%" (reduce #'+ (map 'list #'identity ss) :initial-value 0)))))
