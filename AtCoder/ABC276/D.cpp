...
int main() {
	...
	vector<unordered_map<ll, int>> factors(n);
	REP(i, n) {
		factors[i] = factor(a[i]);
	}

	int min_two = INF, min_three = INF;
	unordered_map<ll, int> m;
	REP(i, n) {
		// cout << a[i] << ": ";
		bool has_two = false, has_three = false;
		for(auto [fac, num] : factors[i]) {
			// cout << fac << "^" << num << ", ";
			if(fac == 2) {
				chmin(min_two, num);
				has_two = true;
		...
		// cout << endl;
		// cout << min_two << ", " << min_three << endl;
	}

	int two_result = 0, three_result = 0;
	REP(i, n) {
		for(auto [fac, num] : factors[i]) {
			if(fac == 2) {
				two_result += num - min_two;
				continue;
			}
			if(fac == 3) {
				three_result += num - min_three;
				continue;
			}
		}
	}

	cout << two_result + three_result << endl;
	return 0;
}
