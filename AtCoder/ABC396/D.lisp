...
(defparameter *max-w* (ash 1 60))

(defun solve (edges used node result)
  (when (zerop node)
    (return-from solve result))
  (setf (aref used node) t)
  (let ((ret *max-w*))
    (dolist (next (aref edges node))
      (let ((label (car next))
            (to (cdr next)))
        (unless (aref used to)
          (minf ret (solve edges used to (logxor label result))))))
    (setf (aref used node) nil)
    ret))
    ...
    (let ((edges (make-array n :initial-element nil))
      ...
      (dotimes (i m)
        (let* ((edge (aref uvw i))
               ...
               (to (second edge))
               (label (third edge)))
          (push (cons label to) (aref edges from))
          (push (cons label from) (aref edges to))))
      (let ((result (solve edges used (1- n) 0)))
        (println result)))))
