...
mod solve {
			...
			for i in 0..self.n {
				for j in 0..i {
					for k in 0..j {
						...
						let b = self.xy[j];
						let c = self.xy[k];
						let area = area_of_parallelogram(diff(b, a), diff(c, a));
						if area != 0 && area % 2 == 0 {
							result += 1;
						}
					}
	...
	fn diff(a: (isize, isize), b: (isize, isize)) -> (isize, isize) {
		(a.0 - b.0, a.1 - b.1)
	}

	fn area_of_parallelogram(a: (isize, isize), b: (isize, isize)) -> usize {
		(a.0 * b.1 - a.1 * b.0).abs() as usize
	}
}
