...
ll solve(const VLL& a, const VI2D& edges) {
	int n = a.size();
	...
	vector<pair<ll, int>> st_v(n);
	REP(i, n) {
		// cout << "costs[" << i << "]: " << costs[i] << endl;
		st_v[i] = {costs[i], i};
	}
	SegmentTree<pair<ll, int>> st(st_v,
	...
	ll ret = 0;
	REP(i, n) {
		auto [cost, node] = st.find(0, n);
		// cout << node << ":" << cost << ":" << a[node] << endl;
		st.update(node, {INFLL, node});
		is_erased[node] = 1;
		...
		EACH(next, edges[node]) {
			...
			st.update(next, {0, 0}, [&](pair<ll, int> x, pair<ll, int> y) {
				x.first -= a[node];
				return x;
			});
		}
