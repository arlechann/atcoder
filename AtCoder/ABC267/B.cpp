...
bool is_empty_row(string s, int row) {
	if(row == 0) {
		return s[6] == '0';
	}
	if(row == 1) {
...
bool is_left_all_empty(string s, int row) {
	REP(i, row) {
		if(!is_empty_row(s, i)) {
			return false;
		}
...
bool is_right_all_empty(string s, int row) {
	RANGE(i, row + 1, 7) {
		if(!is_empty_row(s, i)) {
			return false;
		}
...
int main() {
	...
	RANGE(i, 1, 6) {
		if(is_empty_row(s, i) && !is_left_all_empty(s, i) &&
		   !is_right_all_empty(s, i)) {
			cout << "Yes" << endl;
			return 0;
		}
	}
