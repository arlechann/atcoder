...
(defparameter *inf* 1000000000000000)

(defun main ()
  (input* ((n fixnum))
    (let ((costs (aprog1 (make-array (list n n))
                   (dotimes (i (1- n))
                     (loop for j from (1+ i) below n
                           do (setf (aref it i j) (read)))))))
      (loop for a from 0 below n
            do (loop for b from (1+ a) below n
                     do (loop for c from (1+ b) below n
                              for cost-1 = (aref costs a c)
                              for cost-2 = (+ (aref costs a b) (aref costs b c))
                              when (> cost-1 cost-2)
                                do (print-boolean t)
                                   (return-from main))))
      (print-boolean nil))))
