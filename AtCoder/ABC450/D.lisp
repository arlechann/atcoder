...
(defun main ()
  (input* ((n fixnum)
           (k fixnum)
           (a (vector fixnum n)))
    (let* ((a-mod-k (map 'list #'(lambda (x) (mod x k)) a))
           (a-mod-k-sorted (sort (copy-seq a-mod-k) #'<))
           (a-mod-k-unique (unique a-mod-k-sorted))
           (diffs (window-map 'vector 2
                              #'(lambda (a1 a2)
                                  (diff (+ a1 k) a2))
                              a-mod-k-unique))
           (last-diff (diff (last1 a-mod-k-unique) (elt a-mod-k-unique 0)))
           (result (reduce #'min diffs :initial-value last-diff)))
      (println result))))
