...
(defun dfs (maze y x checked)
  (setf (aref checked y x) t)
  (let* ((h (array-dimension maze 0))
         (w (array-dimension maze 1))
         (result (or (zerop y)
                     (zerop x)
                     (= y (1- h))
                     (= x (1- w)))))
    (do-4neighbors ((new-y new-x) (y x) result)
      (when (and (<= 0 new-y (1- h))
                 (<= 0 new-x (1- w))
                 (not (aref maze new-y new-x))
                 (not (aref checked new-y new-x)))
        (setf result (or (dfs maze new-y new-x checked) result))))))

(defun main ()
  (input* ((h fixnum)
           (w fixnum)
           (s (vector string h)))
    (let ((maze (aprog1 (make-array (list h w) :initial-element nil)
                  (dotimes (i h)
                    (dotimes (j w)
                      (when (char= (aref (aref s i) j) #\#)
                        (setf (aref it i j) t))))))
          (checked (make-array (list h w) :initial-element nil))
          (result 0))
      (loop for i from 0 below h
            do (loop for j from 0 below w
                     when (and (not (aref maze i j))
                               (not (aref checked i j))
                               (not (dfs maze i j checked)))
                       do (incf result)))
      (println result))))
