	...
	impl Solver {
		...
		pub fn solve(&mut self) -> usize {
			...
			let (p, c): (Vec<usize>, Vec<usize>) =
				self.pc.iter().map(|(p, c)| (p, c / 100)).unzip();
			self.p = p;
			self.c = c;
			self.dp = vec![vec![0; g + 1]; self.d + 1];
			self.rec(self.d, g as isize)
		}

		fn rec(&mut self, n: usize, s: isize) -> usize {
			if s <= 0 {
				return 0;
			}
			if n == 0 {
				return std::usize::MAX / 10;
			}
			if self.dp[n][s as usize] != 0 {
				return self.dp[n][s as usize];
			}
			let mut ret = std::usize::MAX;
			for i in 0..=self.p[n - 1] {
				if i == self.p[n - 1] {
					ret = min(
						ret,
						i + self.rec(n - 1, s - (n * i) as isize - self.c[n - 1] as isize),
					);
				} else {
					ret = min(ret, i + self.rec(n - 1, s - (n * i) as isize));
				}
			}
			self.dp[n][s as usize] = ret;
			ret
		}
	}
}
