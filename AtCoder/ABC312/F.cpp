...
const ll N_MAX = 200000;
const ll X_MAX = 1000000000;
...
int main() {
	...
	int easy_size = easy.size(), hard_size = hard.size(),
		opener_size = opener.size();
	VLL cumsum_easy(easy_size + 1, 0), cumsum_hard(hard_size + 1, 0),
		cumsum_opener(opener_size + 1, 0);
	REP(i, easy_size) {
		cumsum_easy[i + 1] = cumsum_easy[i] + easy[i];
	}
	REP(i, hard_size) {
		cumsum_hard[i + 1] = cumsum_hard[i] + hard[i];
	}
	REP(i, opener_size) {
		cumsum_opener[i + 1] = cumsum_opener[i] + opener[i];
	}

	ll result = bin_search<ll>(0, N_MAX * X_MAX + 1, [&](ll satisfy) {
		REP(i, min(m, easy_size) + 1) {
			ll sat = satisfy;
			sat -= cumsum_easy[i];
			if(sat <= 0) {
				return true;
			...
			if(cumsum_hard[hard_size] < sat) {
				continue;
			}

			int rest = m - i;
			int hnum = bin_search<int>(
				hard_size, 0, [&](int i) { return cumsum_hard[i] >= sat; });
			int op_num = rest - hnum;
			if(op_num <= 0) {
				continue;
			}
			if(cumsum_opener[min(op_num, opener_size)] >= hnum) {
				return true;
			}
		}
		return false;
