...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&mut self) -> u64 {
			...
			let mut distances = vec![vec![INF; self.n]; self.n];
			...
			self.memo = vec![None; self.n];
			let result = self.rec(&edges, &distances, self.b - 1);
			result
		}

		fn rec(&mut self, e: &[Vec<usize>], d: &[Vec<usize>], n: usize) -> u64 {
			if let Some(m) = self.memo[n] {
				return m;
			}

			let ret = if n == self.a - 1 {
				1
			} else {
				let mut ret = 0;
				for &i in e[n].iter() {
					if d[self.a - 1][i] + 1 == d[self.a - 1][n] {
						ret += self.rec(e, d, i);
						ret %= MOD;
					}
				}
