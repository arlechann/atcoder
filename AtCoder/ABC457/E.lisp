...
(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (lr (vector (cons* fixnum1 fixnum null) m))
           (q fixnum)
           (st (vector (cons* fixnum1 fixnum) q)))
    (let* ((indexed-lr (map-with-index 'vector
                                       #'(lambda (index pair)
                                           (cons index pair))
                                       lr))
           (l-sorted (sort (copy-seq indexed-lr)
                           #'(lambda (a b)
                               (let ((al (second a))
                                     (ar (third a))
                                     (bl (second b))
      ...
      (loop for i from 0 below q
            for (sq . tq) = (aref st i)
            for left-index = (meguru-method m -1
                                            #'(lambda (index)
                                                (dbind (i l r) (aref l-sorted index)
                                                       (declare (ignore i))
                                                       (if (= l sq)
                                                           (<= r tq)
                                                           (>= l sq)))))
            for left-sheet = (if (= left-index m) nil (aref l-sorted left-index))
            do #?left-sheet))))
