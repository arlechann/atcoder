...
(defun main ()
  (input* ((n fixnum)
    ...
    (let ((l (make-array n))
      ...
      (dotimes (i n)
        (setf (aref c i) (read)))
      (loop with b-length = 0
            for i from 0 below n
            for li = (aref l i)
            and ai = (aref a i)
            and ci = (aref c i)
            for append-length = (* li ci)
            if (> (+ b-length append-length)
                   k)
              do (let* ((count (floor (- k b-length) li))
                        (rest (- k b-length (* li count))))
                   (println (aref ai rest))
                   (return-from main))
            else
              do (incf b-length append-length)))))
