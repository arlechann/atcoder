...
(defun find-less (s tt)
  (loop for i from 0 below (length s)
        when (char> (aref s i) (aref tt i))
          return i
        finally (return (find-more-right s tt))))

(defun find-more-right (s tt)
  (loop for i downfrom (1- (length s)) downto 0
        when (char< (aref s i) (aref tt i))
          return i))
...
(defun main ()
  (input* ((s string)
    ...
    (let ((count 0)
          (result nil))
      (loop until (string= s tt)
            do (let ((index (find-less s tt)))
                 (setf (aref s index) (aref tt index))
                 (incf count)
                 (push (copy-seq s) result))
