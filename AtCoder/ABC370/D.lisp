...
(defun main ()
  (input* ((h fixnum)
           (w fixnum)
           (q fixnum)
           (rc (vector (cons* fixnum1 fixnum1) q)))
    (let ((grid (aprog1 (make-array (list h w) :initial-element t)))
          (next-wall (aprog1 (make-array (list 4 h w) :initial-element nil)
                       (dotimes (k 4)
                         (dotimes (i h)
                           (dotimes (j w)
                             (setf (aref it k i j) (cons i j)))))))
          (wall-buf (make-array 4))
          (count-break 0))
      (labels ((in-grid-p (r c) (and (<= 0 r (1- h)) (<= 0 c (1- w))))
               (break-wall (r c)
                 (incf count-break)
                 (setf (aref grid r c) nil)
                 (loop for dy in '(0 1 0 -1)
                       for dx in '(1 0 -1 0)
                       for dir in '(0 1 2 3)
                       for y = (+ r dy)
                       for x = (+ c dx)
                       when (in-grid-p y x)
                         do (setf (aref next-wall dir r c)
                                  (aref next-wall dir y x))))
               (on-edge-p (dir r c)
                 (case dir 
                   (0 (= c (1- w)))
                   (1 (= r (1- h)))
                   ...
                   (3 (zerop r))))
               (find-wall (dir r c)
                 (let ((next (aref next-wall dir r c)))
                   (cond ((null next) nil)
                         ((aref grid r c) next)
                         ((on-edge-p dir r c) nil)
                         (t (setf (aref next-wall dir r c)
                                  (find-wall dir (car next) (cdr next)))))))
               (find-walls (r c)
                 (dotimes (k 4 wall-buf)
                   (setf (aref wall-buf k) (find-wall k r c)))))
        (loop for (r . c) across rc
              if (aref grid r c)
                do (break-wall r c)
              else   
                do (map nil #'(lambda (pair)
                                (unless (null pair)
                                  (break-wall (car pair) (cdr pair))))
                        (find-walls r c)))
        (let ((result (- (* h w) count-break)))
          (format t "~A~%" result))))))
