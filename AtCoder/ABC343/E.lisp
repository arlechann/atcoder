...
(defun common-length2 (from1 to1 from2 to2)
  (max (- (min to1 to2)
          (max from1 from2))
       0))
  
(defun common-length3 (from1 to1 from2 to2 from3 to3)
  (max (- (min to1 to2 to3)
          (max from1 from2 from3))
       0))

(defun is-valid (v a2 b2 c2 a3 b3 c3)
  (let ((v1 (aref v 0))
        (v2 (aref v 1))
        (v3 (aref v 2)))
    (labels ((calc-common-volume2 (a1 b1 c1 a2 b2 c2)
               (* (common-length2 a1 (+ a1 7) a2 (+ a2 7))
                  (common-length2 b1 (+ b1 7) b2 (+ b2 7))
                  (common-length2 c1 (+ c1 7) c2 (+ c2 7)))))
      (let* ((vv3 (* (common-length3 0 7 a2 (+ a2 7) a3 (+ a3 7))
                     (common-length3 0 7 b2 (+ b2 7) b3 (+ b3 7))
                     (common-length3 0 7 c2 (+ c2 7) c3 (+ c3 7))))
             (vv2 (- (+ (calc-common-volume2 0 0 0 a2 b2 c2)
                        (calc-common-volume2 a2 b2 c2 a3 b3 c3)
                        (calc-common-volume2 a3 b3 c3 0 0 0))
                     (* vv3 3))))
        ;(format t "~A ~A ~A~%" vv1 vv2 vv3)
        (and (= v2 vv2) (= v3 vv3))))))
      ...
      (dotimes (b2 15)
        (dotimes (c2 15)
          (dotimes (a3 15)
            (dotimes (b3 15)
              (dotimes (c3 15)
                (when (is-valid v (- a2 7) (- b2 7) (- c2 7) (- a3 7) (- b3 7) (- c3 7))
                  (format t "Yes~%0 0 0 ~A ~A ~A ~A ~A ~A~%" (- a2 7) (- b2 7) (- c2 7) (- a3 7) (- b3 7) (- c3 7))
                  (return-from main))))))))
    (format t "No~%")))

(defun test ()
  (test-case "840 84 7" "Yes
0 0 0 0 6 0 6 0 0
")
  (test-case "343 34 3" "No")
  )
