...
(defun main ()
  (input* ((n fixnum)
           (time fixnum)
           (ab (vector (list fixnum1 fixnum) time)))
    (let ((players (make-array n :initial-element 0))
          (ht (make-hash-table)))
      (labels ((inc-point (player point)
                 (let* ((now-point (aref players player))
                        (new-point (+ now-point point)))
                   (decf (gethash now-point ht))
                   (when (= (gethash now-point ht) 0)
                     (remhash now-point ht))
                   (setf (aref players player) new-point)
                   (incf (gethash new-point ht 0))))
               (get-diversity ()
                 (hash-table-count ht)))
        (setf (gethash 0 ht) n)
        (dotimes (i time)
