...
(defun dfs (n m c zoo-animals now fee watch)
  (when (= now n)
    (return-from dfs
      (if (every #'(lambda (w) (>= w 2)) watch)
          fee
          +inf+)))
  (let ((ret +inf+)
        (animals (aref zoo-animals now)))
    (dotimes (i 3)
      (minf ret (dfs n m c zoo-animals (1+ now) (+ fee (* (aref c now) i)) watch))
      (unless (= i 2)
        (dolist (a animals)
          (incf (aref watch a)))))
    (dolist (a animals)
...
(defun solve (n m c k a)
  (let ((zoo-animals (aprog1 (make-array n :initial-element nil)
                       (dotimes (i m)
                         (dotimes (j (aref k i))
                           (let ((zoo (aref (aref a i) j)))
                             (push i (aref it zoo))))))))
    (let* ((watch (make-array m :initial-element 0))
           (result (dfs n m c zoo-animals 0 0 watch)))
      (println result))))
...
(defun main ()
  (input* ((n fixnum)
    ...
    (let* ((k (make-array m))
      ...
      (dotimes (i m)
        (let* ((ki (read))
               (ai (aprog1 (make-array ki)
                     (dotimes (j ki)
                       (setf (aref it j) (1- (read)))))))
          (setf (aref k i) ki
                (aref a i) ai)))
      (solve n m c k a))))
