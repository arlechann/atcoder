...
(defun main ()
  (input* ((n fixnum)
    ...
    (when (/= n m)
      (println "No")
      (return-from main))
    (let ((count (make-array n :initial-element 0))
      ...
      (dotimes (i m)
        (let* ((ab (aref ab i))
          ...
          (incf (aref count a))
          (incf (aref count b))
          (union-find-merge uf a b)))
      (print-boolean (and (every #'(lambda (x) (= x 2)) count)
                          (every #'(lambda (i) (union-find-unite-p uf i 0)) (iota n)))))))
