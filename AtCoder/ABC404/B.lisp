...
(defun rotate-array2d (a)
  (let* ((dimensions (array-dimensions a))
         (h (first dimensions))
         (w (second dimensions))
         (tmp (make-array (list w h))))
    (dotimes (i h)
      (dotimes (j w)
        (setf (aref tmp i j)
              (aref a (- h j 1) i))))
    (let* ((a-seq (make-array (* h w) :displaced-to a))
           (tmp-seq (make-array (* h w) :displaced-to tmp)))
      (replace a-seq tmp-seq)))
  a)
...
(defun main ()
  (input* ((n fixnum)
           (s (vector string n))
           (tt (vector string n)))
    (let ((s (aprog1 (make-array (list n n) :initial-element nil)
               (dotimes (i n)
                 (dotimes (j n)
                   (setf (aref it i j)
                         (char= (aref (aref s i) j) #\#))))))
          (tt (aprog1 (make-array (list n n) :initial-element nil)
                (dotimes (i n)
                  (dotimes (j n)
                    (setf (aref it i j)
                          (char= (aref (aref tt i) j) #\#)))))))
      (let ((result (* n n)))
        (dotimes (i 4)
          (let* ((s-seq (make-array (* n n) :displaced-to s))
                 (tt-seq (make-array (* n n) :displaced-to tt))
                 (count (reduce #'+
                            (map 'list #'(lambda (s tt) (if (eq s tt) 0 1)) s-seq tt-seq)
                            :initial-value 0)))
            (minf result (+ count i)))
          (rotate-array2d s))
        (println result)))))
