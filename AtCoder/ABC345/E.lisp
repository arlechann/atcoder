...
(defun main ()
  (input* ((n fixnum)
           (k fixnum)
           (cv (vector (list fixnum fixnum) n)))
    (let* ((memo (make-hash-table :test #'equal))
           (result (dp n k cv memo 0 0 -1)))
      (if (< result 0)
          (format t "-1~%")
          (format t "~A~%" result)))))

(defun dp (n k cv memo index rem last-color)
  (when (> rem k)
    (return-from dp -1))
  (when (= index n)
    (if (= rem k)
        (return-from dp 0)
        (return-from dp -1)))
  (let ((key (list index rem last-color)))
    (let ((memval (gethash key memo nil)))
      (when memval
        (return-from dp memval)))
    (setf (gethash key memo)
          (let* ((pair (aref cv index))
                 (c (car pair))
                 (v (cadr pair)))
            (max (dp n k cv memo (1+ index) (1+ rem) last-color)
                 (if (= c last-color)
                     -1
                     (let ((val (dp n k cv memo (1+ index) rem c)))
                       (if (< val 0)
                           -1
                           (+ v val)))))))))
