...
(defun is-valid (h w tiles)
  (when (< (reduce #'+
                   (mapcar (lambda (ab) (* (car ab) (cadr ab)))
                           tiles)
                   :initial-value 0)
           (* h w))
    (return-from is-valid nil))
  (labels ((find-empty (board)
             (dotimes (i h)
               (dotimes (j w)
                 (when (zerop (aref board i j))
                   (return-from find-empty (cons i j)))))
             nil)
           (set-board (board y x a b val)
             (dotimes (i a)
               (dotimes (j b)
                 (setf (aref board (+ y i) (+ x j)) val))))
           (is-puttable (board y x a b)
             (unless (and (<= (+ a y) h)
                          (<= (+ b x) w))
               (return-from is-puttable nil))
             (dotimes (i a)
               (dotimes (j b)
                 (unless (zerop (aref board (+ y i) (+ x j)))
                   (return-from is-puttable nil))))
             t)
           (put-tiles (board tiles)
             (let ((pos (find-empty board)))
               (when (null tiles)
                 (return-from put-tiles
                   (not pos)))
               (let* ((y (car pos))
                      (x (cdr pos))
                      (tile (car tiles))
                      (a (car tile))
                      (b (cadr tile)))
                 (when (is-puttable board y x a b)
                   (set-board board y x a b 1)
                   (when (put-tiles board (cdr tiles))
                     (return-from put-tiles t))
                   (set-board board y x a b 0))
                 (when (is-puttable board y x b a)
                   (set-board board y x b a 1)
                   (when (put-tiles board (cdr tiles))
                     (return-from put-tiles t))
                   (set-board board y x b a 0))))
             nil))
    (let ((board (make-array (list h w)
                             :element-type 'fixnum
                             :initial-element 0))
          (tile-perms (permutation tiles)))
      (some (lambda (ts)
              (put-tiles board ts))
            tile-perms))))

(defun pick-tiles (n h w ab index tiles)
  (when (= index n)
    (return-from pick-tiles
      (is-valid h w tiles)))
  (or (pick-tiles n h w ab (1+ index) tiles)
      (pick-tiles n h w ab (1+ index) (cons (aref ab index) tiles))))

(defun main ()
  (input* ((n fixnum)
           (h fixnum)
           (w fixnum)
           (ab (vector (list fixnum fixnum) n)))
    (let ((result (pick-tiles n h w ab 0 nil)))
      (format t (if result
                    "Yes~%"
                    "No~%")))))
