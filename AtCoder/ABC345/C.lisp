...
(defun char-index (c) (- (char-code c) 
                         #.(char-code #\a)))
(defun main ()
  (input* ((s string))
    (let ((n (length s))
          (alpha-counts (make-array (1+ (- #.(char-code #\z)
                                           #.(char-code #\a)))
                                    :element-type 'fixnum
                                    :initial-element 0)))
      (dotimes (i n)
        (incf (aref alpha-counts (char-index (aref s i)))))
      (let ((no-change-pairs 0))
        (dotimes (i (1+ (char-index #\z)))
          (let ((count (aref alpha-counts i)))
            (incf no-change-pairs
                  (floor (* count (1- count)) 2))))
        (format t "~A~%" (+ (- (floor (* n (1- n)) 2)
                               no-change-pairs)
                            (if (zerop no-change-pairs)
                                0 1)))))))
