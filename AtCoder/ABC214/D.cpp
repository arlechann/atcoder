...
int main() {
	...
	vector<pair<ll, pair<int, int>>> paths(n - 1);
	REP(i, n - 1) { paths[i] = make_pair(w[i], make_pair(u[i], v[i])); }
	sort(ALL(paths));

	ll result = 0;
	UnionFind uf(n);
	REP(i, n - 1) {
		ll weight = paths[i].first;
		int from = paths[i].second.first, to = paths[i].second.second;
		ll size_from = uf.size(from), size_to = uf.size(to);
		result += size_from * size_to * weight;
		uf.merge(from, to);
	}
