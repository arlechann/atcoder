...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			...
			let mut map = vec![vec![0isize; self.c]; self.r];
			for i in 0..self.r as isize {
				for j in 0..self.c as isize {
					if self.s[i as usize][j as usize] == 'x' {
						for x in 0..=(k - 1) * 2 {
							if 0 <= i - k + 1 + x && i - k + 1 + x < self.r as isize {
								map[(i - k + 1 + x) as usize]
									[max(j - min(x, (k - 1) * 2 - x), 0) as usize] += 1;
								if j + min(x, (k - 1) * 2 - x) + 1 < self.c as isize {
									map[(i - k + 1 + x) as usize]
										[(j + min(x, (k - 1) * 2 - x) + 1) as usize] -= 1;
								}
							}
						}
					}
			...
			let mut result = 0;
			for i in k - 1..self.r as isize - k + 1 {
				for j in k - 1..self.c as isize - k + 1 {
