...
int main() {
	...
	vector<mint> a(m + 1, mint(0));
	vector<SegmentTree<mint>> dp(n + 1, SegmentTree<mint>(a));
	RANGE(j, 1, m + 1) {
		dp[1].update(j, 1);
	}
	...
	RANGE(i, 1, n) {
		RANGE(j, 1, m + 1) {
			mint value = 0;
			if(j - k + 1 >= 0) {
				value += dp[i].find(0, j - k + 1);
			}
			if(k != 0 && j + k <= m) {
				value += dp[i].find(j + k, m + 1);
			}
			if(k == 0) {
				value += dp[i].find(j + 1, m + 1);
			}
			dp[i + 1].update(j, value);
		}
	}

	mint result = dp[n].find(1, m + 1);
	cout << result << endl;
	return 0;
}
