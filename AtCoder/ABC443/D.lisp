...
(defun solve ()
  (input* ((n fixnum)
           (r (vector fixnum1 n)))
    (let* ((from-right (aprog1 (make-array n :initial-element 0)
                         (setf (aref it (1- n)) (aref r (1- n)))
                         (loop for i from (- n 2) downto 0
                               if (<= (aref r i) (1+ (aref it (1+ i))))
                                 do (setf (aref it i) (aref r i))
                               else
                                 do (setf (aref it i) (1+ (aref it (1+ i)))))))
           (from-left (aprog1 (make-array n :initial-element 0)
                        (setf (aref it 0) (aref r 0))
                        (loop for i from 1 below n
                              if (<= (aref r i) (1+ (aref it (1- i))))
                                do (setf (aref it i) (aref r i))
                              else
                                do (setf (aref it i) (1+ (aref it (1- i)))))))
           (operated (aprog1 (make-array n :initial-element 1000000)
                       (dotimes (i n)
                         (minf (aref it i) (aref from-right i) (aref from-left i)))))
           (result (loop for i below n
                         sum (diff (aref operated i) (aref r i)))))
      (println result))))
