    ...
    (loop with result = 0
          ...
          if is-open
            do (incf result aoki)
               (setf closed-at aoki
                     is-open nil)
          else if (< closed-at (- aoki 100))
            do (incf result (- aoki closed-at 100))
               (setf closed-at aoki)
          finally (when is-open
                    (incf result tt))
                  (when (and (not is-open)
                             (< closed-at (- tt 100)))
                    (incf result (- tt closed-at 100)))
                  (println result))))
...
100 150 300 350 700
" "500")
  (test-case "0 1000000000" "1000000000")
  (test-case "10 1234
