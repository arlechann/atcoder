...
(defun major-index-to-row (index n)
  (values (floor index n)))

(defun major-index-to-column (index n)
  (mod index n))

(defun on-\\-p (row column n)
  (declare (ignore n))
  (= row column))
...
(defun main ()
  (input* ((n fixnum)
           (turn fixnum)
           (a (vector fixnum1 turn)))
    (let ((sheet (make-array (list n n) :initial-element nil))
          (row-fill-count (make-array n :initial-element 0))
          (column-fill-count (make-array n :initial-element 0))
          (cross-fill-count (make-array 2 :initial-element 0)))
      (dotimes (i turn)
        (let* ((index (aref a i))
               (row (major-index-to-row index n))
               (column (major-index-to-column index n)))
          (unless (aref sheet row column)
            (setf (aref sheet row column) t)
            (incf (aref row-fill-count row))
            (incf (aref column-fill-count column))
            (when (on-\\-p row column n)
              (incf (aref cross-fill-count 0)))
            (when (on-/-p row column n)
              (incf (aref cross-fill-count 1)))
            (when (or (= (aref row-fill-count row) n)
                      (= (aref column-fill-count column) n)
                      (= (aref cross-fill-count 0) n)
                      (= (aref cross-fill-count 1) n))
              (format t "~A~%" (1+ i))
              (return-from main)))))
      (format t "-1~%"))))
