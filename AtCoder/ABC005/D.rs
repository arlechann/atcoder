...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> String {
			let mut cumsum = vec![vec![0; self.n + 1]; self.n + 1];
			for i in 0..self.n {
				for j in 0..self.n {
					cumsum[i + 1][j + 1] =
						cumsum[i + 1][j] + cumsum[i][j + 1] - cumsum[i][j] + self.d[i][j];
				}
			}

			let mut size_max = vec![0; self.n * self.n + 1];
			for i1 in 0..self.n {
				for j1 in 0..self.n {
					for i2 in i1..self.n + 1 {
						for j2 in j1..self.n + 1 {
							size_max[(i2 - i1) * (j2 - j1)] = max(
								size_max[(i2 - i1) * (j2 - j1)],
								get_square(&cumsum, (i1, j1), (i2, j2)),
							);
						}
					}
				}
	...
	fn get_square(v: &[Vec<usize>], (y1, x1): (usize, usize), (y2, x2): (usize, usize)) -> usize {
		v[y2][x2] + v[y1][x1] - v[y1][x2] - v[y2][x1]
	}
}
