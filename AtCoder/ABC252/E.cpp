...
struct ListGraph {
	...
	void add_edge_with_index(std::size_t from,
							 std::size_t to,
							 Weight weight,
							 int index) {
		Edge e = Edge(from, to, weight);
		e.index = index;
		this->edges[from].push_back(e);
	}
...
struct Undirected {
	...
	template <typename G, typename W>
	void add_edge_with_index(
		G& graph, std::size_t u, std::size_t v, W weight, int index) const {
		graph.add_edge_with_index(u, v, weight, index);
		graph.add_edge_with_index(v, u, weight, index);
	}
...
struct Directed {
	...
	template <typename G, typename W>
	void add_edge_with_index(
		G& graph, std::size_t from, std::size_t to, W weight, int index) const {
		graph.add_edge(from, to, weight, index);
	}
...
struct Graph {
	...
	void
	add_edge_with_index(std::size_t u, std::size_t v, Weight c, int index) {
		this->directed.add_edge_with_index(this->graph, u, v, c, index);
	}
...
// 最小全域木
template <typename G>
VI solve(const G& graph, size_t s) {
	using Weight = typename G::Weight;
	using Node = std::pair<Weight, std::size_t>;
	size_t n = graph.size();
	std::vector<bool> used(n, false);
...
int main() {
	...
	UndirectedListGraph g(n);
	REP(i, m) {
		g.add_edge_with_index(a[i], b[i], c[i], i);
	}

	auto result = solve(g, 0);
