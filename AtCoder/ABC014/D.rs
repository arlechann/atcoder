...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&mut self) -> String {
			let mut edges = vec![vec![]; self.n];
			for &(x, y) in self.xy.iter() {
				edges[x - 1].push(y - 1);
				edges[y - 1].push(x - 1);
			}

			let mut parents = vec![0; self.n];
			let mut depths = vec![0; self.n];
			dfs(&edges, &mut parents, &mut depths, 0, 0, 0);

			let doubling = Doubling::new(&parents);
			let mut result = Vec::with_capacity(self.q);
			for &(a, b) in self.ab.iter() {
				let (_lca, distance) =
					doubling.common_ancestor(a - 1, depths[a - 1], b - 1, depths[b - 1]);
				result.push((distance + 1).to_string());
			}
			result.join("\n")
		}
	}
	...
	fn dfs(
		...
		for &next in edges[node].iter() {
			if next != prev {
				dfs(edges, parents, depths, next, node, depth + 1);
			}
		}
	}
}
...
mod doubling {
	...
	impl Doubling {
		pub fn new(a: &[usize]) -> Self {
			let len = a.len();
			let mut doubling = vec![vec![0; len]; USIZE_BITS];
			doubling[0] = a.to_vec();
		...
		pub fn query(&self, mut now: usize, mov: usize) -> usize {
		...
		pub fn common_ancestor(
			...
			if a_depth > b_depth {
				return self.common_ancestor(b, b_depth, a, a_depth);
			}

			let depth_diff = b_depth - a_depth;
			let b = self.query(b, depth_diff);
			let lca_depth = bin_search(a_depth as isize, -1, |d| {
				let a_ancestor = self.query(a, d as usize);
				let b_ancestor = self.query(b, d as usize);
				a_ancestor == b_ancestor
			});
			let lca = self.query(a, lca_depth as usize);
			(lca, lca_depth as usize * 2 + depth_diff)
		}
	}
}
