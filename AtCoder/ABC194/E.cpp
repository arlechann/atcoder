...
int main() {
	...
	VI segtree_seed(n + 1);
	REP(i, n + 1) { segtree_seed[i] = i; }

	auto st = SegmentTree<int>(
		segtree_seed, [](int a, int b) { return min(a, b); }, INF);
	unordered_map<int, int> count;
	REP(i, m - 1) {
		st.update(min(a[i], n), INF);
		count[min(a[i], n)]++;
	}

	int result = INF;
	REP(i, n - m + 1) {
		st.update(min(a[i + m - 1], n), INF);
		count[a[i + m - 1]]++;
		chmin(result, st.find(0, n + 1));
		count[a[i]]--;
		if(a[i] < n && count[a[i]] == 0) {
			st.update(a[i], i);
		}
	}
