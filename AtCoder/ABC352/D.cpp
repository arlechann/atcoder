...
int main() {
	...
	SegmentTree<int> st_max(
		indecies, [](int a, int b) { return max(a, b); }, -1);
	SegmentTree<int> st_min(
		indecies, [](int a, int b) { return min(a, b); }, INF);

	int result = INF;
	REP(i, n - k + 1) {
		chmin(result, st_max.find(i, i + k) - st_min.find(i, i + k));
	}
