...
(defun union-find-merge (uf a b)
  (let ((ar (union-find-root uf a))
    ...
    (cond ((= ar br) nil)
          ...
          (t (incf (union-find-group-size uf ar)
...
(defun make-operation (cost nodes)
  (cons cost nodes))
(defun operation-cost (ope) (car ope))
(defun operation-nodes (ope) (cdr ope))
(defun operation-represent-node (ope) (cadr ope))

(defun main ()
  (input* ((n fixnum)
           (m fixnum))
    (let* ((k (make-array m :element-type 'fixnum))
           (operations (make-array m)))
      (dotimes (i m)
        (input* ((ki fixnum)
                 (ci fixnum)
                 (ai (list fixnum1 ki)))
          (setf (aref k i) ki
                (aref operations i) (make-operation ci ai))))
      (setf operations
            (sort operations
                  (lambda (a b)
                    (< (operation-cost a) (operation-cost b)))))
      (let ((uf (make-union-find n))
            (result 0))
        (dotimes (i m)
          (let* ((ope (aref operations i))
                 (root (operation-represent-node ope)))
            (dolist (node (operation-nodes ope))
              (unless (union-find-unite-p uf root node)
                (union-find-merge uf root node)
                (incf result (operation-cost ope))))))
        (format t "~A~%"
                (if (= n
                       (union-find::union-find-group-size
                        uf
                        (union-find::union-find-root uf 0)))
                    result
                    -1))))))
