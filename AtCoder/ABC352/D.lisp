         ...
         (bintree (cond (element-type-p
                        ...
                        ((typep seq 'array)
                         (make-bintree bintree-size
                                       ...
                                       :initial-element id))
                        (t (make-bintree bintree-size :initial-element id)))))
...
(defun main ()
  (input* ((n fixnum)
           (k fixnum)
           (p (vector fixnum1 n)))
    (let ((indecies (make-array n :element-type 'fixnum)))
      (dotimes (i n)
        (setf (aref indecies (aref p i)) i))
      (let ((st-max (make-segment-tree-from-sequence indecies
                                                     #'max
                                                     -1))
            (st-min (make-segment-tree-from-sequence indecies
                                                     #'min
                                                     +INF+)))
        (format t "~A~%"
                (loop for i from 0 to (- n k)
                      minimize (- (segment-tree-fold st-max i (+ i k))
                                  (segment-tree-fold st-min i (+ i k)))))))))
