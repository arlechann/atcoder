...
std::vector<Weight> dijkstra(const Graph& graph, const size_t s) {
	...
	while(!pq.empty()) {
		Edge edge = pq.top();
		pq.pop();
		if(distances[edge.to] < edge.cost) {
			continue;
		}
		used[edge.to] = true;
...
int main() {
	...
	REP(i, m) {
		if(h[u[i]] > h[v[i]]) {
			g.link(u[i], v[i], -(h[u[i]] - h[v[i]]));
			g.link(v[i], u[i], -2 * (h[v[i]] - h[u[i]]));
		} else {
			g.link(v[i], u[i], -(h[v[i]] - h[u[i]]));
			g.link(u[i], v[i], -2 * (h[u[i]] - h[v[i]]));
		}
	}

	auto distancies = dijkstra(g, 0);
	ll result = -(*min_element(ALL(distancies)));
