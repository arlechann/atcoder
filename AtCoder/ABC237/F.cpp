...
int main() {
	...
	auto dp = make_vector({n + 1, m + 1, m + 1, m + 1, 3 + 1}, mint(0));
	// dp[数列長][1番目][2番目][3番目][最長増加部分列長]
	REP(i, n) {
		if(i == 0) {
			dp[1][0][m][m][1] = 1;
		}
		REP(j, 3) {
			REP(s, m) {
				REP(t, m) {
					REP(u, m) {
						RANGE(x, s, t) {
							dp[i + 1][s][t][u][j + 1] += dp[i][x][t][u][j + 1];
						}
						RANGE(x, t, u) {
							dp[i + 1][s][t][u][j + 1] += dp[i][s][x][u][j + 1];
						}
						dp[i + 1][s][t][u][j + 1] += dp[i][s][t][u][j + 1];
					}
				}
			}
		}
