...
int main() {
	...
	REP(i, n) { points[i] = make_pair(p[i][0] + p[i][1] + p[i][2], i); }

	auto sorted_points = points;
	sort(RALL(sorted_points));

	VI result(n, 0);
	VI ranks(n);
	REP(i, n) {
		int student = sorted_points[i].first + 300;
		int rank = bin_search<int>(n - 1, -1, [&](int x) {
			return sorted_points[x].first <= student;
		});
		ranks[sorted_points[i].second] = rank;
		result[sorted_points[i].second] = static_cast<int>(rank < k);
	}

	REP(i, n) { yes_no(result[i]); }
	return 0;
}
