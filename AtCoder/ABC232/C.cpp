...
int main() {
	...
	VI2D sunuke_edges(n, VI(n, 0));
	REP(i, m) { sunuke_edges[a[i]][b[i]] = sunuke_edges[b[i]][a[i]] = 1; }
	VI2D aoki_edges(n, VI(n, 0));
	REP(i, m) { aoki_edges[c[i]][d[i]] = aoki_edges[d[i]][c[i]] = 1; }

	VI arr(n);
	REP(i, n) { arr[i] = i; }
	bool result = false;
	call_with_all_permutations(arr, [&](auto a) {
		REP(i, n) {
			REP(j, i) {
				bool is_same = sunuke_edges[i][j] == aoki_edges[a[i]][a[j]];
				if(!is_same) {
					return true;
				}
			}
