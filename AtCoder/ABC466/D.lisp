...
(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (rc (vector (cons* fixnum1 fixnum1) m)))
    (loop with result = 0 
          and rows = (make-array n :initial-element -1)
          and cols = (make-array n :initial-element -1)
          for i from (1- m) downto 0
          for (r . c) = (aref rc i)
          when (and (= (aref rows r) -1)
                    (= (aref cols c) -1))
            do (incf result)
          do (setf (aref rows r) 1
                   (aref cols c) 1)
