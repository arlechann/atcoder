...
(defparameter *n-max* 200000)
(defparameter *a-max* 1000000000)

(defun main ()
  (input* ((n fixnum)
           (k fixnum)
           (ab (vector (cons* fixnum fixnum) n)))
    (dp-labels ((max-sum :for ((flip (1+ (* k 2)))
                               (i (1+ n)))
                         :uncomputed most-negative-fixnum
                         :case (((= i 0) 0)
                                ((zerop flip)
                                 (+ (max-sum flip (1- i))
                                    (car (aref ab (1- i)))))
                                ((evenp flip)
                                 (+ (max (max-sum flip (1- i))
                                         (max-sum (1- flip) (1- i)))
                                    (car (aref ab (1- i)))))
                                (t
                                 (+ (max (max-sum flip (1- i))
