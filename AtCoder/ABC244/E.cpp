...
int used[K_MAX + 1][N_MAX][2];
mint memo[K_MAX + 1][N_MAX][2];

mint dp(const VI2D& edges, int index, int node, int is_odd) {
	mint& ret = memo[index][node][is_odd];
	if(used[index][node][is_odd] != 0) {
		return ret;
	}
	used[index][node][is_odd] = 1;
	if(index == k && node == t && is_odd == 0) {
		return ret = 1;
	} else if(index == k) {
		return ret = 0;
	}

	int next_is_odd = node == x ? ((is_odd + 1) & 1) : is_odd;
	EACH(next, edges[node]) { ret += dp(edges, index + 1, next, next_is_odd); }
	return ret;
}
