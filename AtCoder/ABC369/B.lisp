...
(defun main ()
  (input* ((n fixnum)
           (as (vector (cons* fixnum symbol) n)))
    (let* ((left (map 'vector (lambda (pair) (car pair))
                      (remove-if (lambda (hand) (not (eq hand 'l)))
                                 as :key #'cdr)))
           (right (map 'vector (lambda (pair) (car pair))
                       (remove-if (lambda (hand) (not (eq hand 'r)))
                                  as :key #'cdr)))
           (result (+ (loop with prev = nil
                            for i from 0 below (length left)
                            unless (null prev)
                              sum (diff prev (aref left i))
                            do (setf prev (aref left i)))
                      (loop with prev = nil
                            for i from 0 below (length right)
                            unless (null prev)
                              sum (diff prev (aref right i))
                            do (setf prev (aref right i))))))
      (format t "~A~%" result))))
