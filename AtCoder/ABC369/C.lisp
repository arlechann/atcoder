...
(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum n)))
    (let* ((diffs (loop with arr = (make-array (1- n) :initial-element 0)
                        for i below (1- n)
                        do (setf (aref arr i) (diff (aref a i) (aref a (1+ i))))
                        finally (return arr)))
           (runs (loop with prev = nil
                       and run = 0
                       and runs = nil
                       for i below (1- n)
                       if (null prev)
                         do (setf prev (aref diffs i))
                            (incf run)
                       else if (= prev (aref diffs i))
                         do (incf run)
                       else
                         do (setf prev (aref diffs i))
                            (push (1+ run) runs)
                            (setf run 1)
                       finally (push (1+ run) runs)
                               (return (nreverse runs))))
           (result (+ (loop for run in runs
                            sum (floor (* run (1- run)) 2))
                      n)))
      (format t "~A~%" result))))
