...
(defun make-str-block (level)
  (list level
        nil))

(defun str-block-push (sb char)
  (push char (second sb))
  sb)

(defun str-block-rev-p (sb)
  (not (zerop (mod (first sb) 2))))

(defun str-block-combine (sb)
  (let ((revp (str-block-rev-p sb))
        (chars (second sb)))
    (mapcar (lambda (char)
              (if (characterp char)
                  char
                  (str-block-combine char)))
            (if revp
                (nreverse
                 (mapcar (lambda (char)
                           (if (characterp char)
...
(defun parse-block (s i level)
  (let ((sb (make-str-block level)))
    (nlet rec ((i i))
      (if (< i 0) sb
          (let ((c (aref s i)))
            (cond ((char= c #\()
                   (values sb i))
                  ((char= c #\))
                   (multiple-value-bind (sb2 index)
                       (parse-block s (1- i) (1+ level))
                     (str-block-push sb sb2)
                     (rec (1- index))))
                  (t
                   (str-block-push sb c)
                   (rec (1- i)))))))))
...
(defun solve (s)
  (let* ((len (length s))
         (sb (parse-block s (1- len) 0)))
    (let ((result (coerce (flatten (str-block-combine sb)) 'string)))
      (format t "~A~%" result))))
...
(defun test ()
  (test-case "((A)y)x" "YAx")
  (test-case "((XYZ)n(X(y)Z))" "XYZNXYZ")
  (test-case "(((()))(()))(())" "")
  (test-case "dF(qT(plC())NnnfR(GsdccC))PO()KjsiI((ysA)eWW)ve" "dFGsdccCrFNNnplCtQPOKjsiIwwEysAve")
  )
