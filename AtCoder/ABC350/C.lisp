...
(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum1 n)))
    (let ((where (make-array n :initial-element 0)))
      (dotimes (i n)
        (let ((v (aref a i)))
          (setf (aref where v) i)))
      (let ((result
              (remove-if #'null 
                         (loop for i from 0 below n
                               #|do (progn 
                                    ...
                                    (format t "where: ")
                                    (dotimes (i n)
                                      (format t "~A " (aref where i)))
                                    (terpri))|#
                               collect (let ((i-index (aref where i))
                                             (i-val (aref a i)))
                                         (rotatef (aref a i) (aref a i-index))
                                         (rotatef (aref where i) (aref where i-val))
                                         (unless (= i i-index)
                                           (cons i i-index)))))))
        (format t "~A~%" (length result))
        (dolist (pair result)
          (format t "~A ~A~%" (1+ (car pair)) (1+ (cdr pair))))))))
