...
(defun connected-component-sizes (graph)
  (let ((used-nodes (make-hash-table)))
    (labels ((use-node (node)
               (setf (gethash node used-nodes) t))
             (used-node-p (node)
               (gethash node used-nodes nil))
             (dfs (node)
               ;(format t "dfs: ~A~%" node)
               (when (used-node-p node)
                 (return-from dfs 0))
               (use-node node)
               (let ((neighbors (graph-neighbors graph node)))
                 (reduce #'+
                         (mapcar #'dfs neighbors)
                         :initial-value 1))))
      (remove 0 (mapcar #'dfs (iota (graph-size graph)))))))

(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (ab (vector (list fixnum1 fixnum1) m)))
    (let* ((graph (make-graph-from-edges n ab))
           (sizes (connected-component-sizes graph))
           (sum (reduce #'+
                        (mapcar (lambda (size)
                                  (/2 (* size (1- size))))
                                sizes)
