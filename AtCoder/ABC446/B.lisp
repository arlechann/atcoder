    ...
    (let ((l (make-array n))
      ...
      (dotimes (i n)
        (let ((len (read)))
          (setf (aref l i) len)
          (setf (aref x i)
                (aprog1 (make-array len)
                  (dotimes (i len)
                    (setf (aref it i) (1- (read))))))))
      (let ((juices (make-array m :initial-element t)))
        (dotimes (i n)
          (let ((result (find-if #'(lambda (wish)
                                     (and (aref juices wish)
                                          (progn (setf (aref juices wish) nil)
                                                 t)))
                                 (aref x i))))
            (println (1+ (or result -1)))))))))
