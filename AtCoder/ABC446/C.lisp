...
(defun solve ()
  (input* ((n fixnum)
           ...
           (a (vector fixnum n))
           (b (vector fixnum n)))
    (let ((b-cumsum (concatenate 'vector (cumulate b) (list *inf*))))
      (loop with carry = 0
            for i from 0 below n
            for eggs = (+ (aref a i) carry)
            for need = (aref b i)
            for need-d-days = (- (aref b-cumsum (min (+ i d 1) (1+ n))) (aref b-cumsum i))
            ;; do #?(list i eggs need need-d-days)
            do (setf carry (- (min eggs need-d-days) need))
            ;; do #?carry
            finally (println (- eggs need))))))
