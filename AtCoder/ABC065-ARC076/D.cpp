...
// 最小全域木
std::pair<Weight, Graph> prim(const Graph& graph) {
	size_t n = graph.node;
...
int main() {
	...
	REP(i, n) { sorted_x[i] = make_pair(x[i], i); }
	sort(ALL(sorted_x));

	vector<pair<int, int>> sorted_y(n);
	REP(i, n) { sorted_y[i] = make_pair(y[i], i); }
	sort(ALL(sorted_y));

	Graph g(n);
	REP(i, n - 1) {
		g.link(sorted_x[i].second,
			   sorted_x[i + 1].second,
			   sorted_x[i + 1].first - sorted_x[i].first);
		g.link(sorted_y[i].second,
			   sorted_y[i + 1].second,
			   sorted_y[i + 1].first - sorted_y[i].first);
	}

	auto [cost, mintree] = prim(g);
