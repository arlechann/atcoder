...
(defun main ()
  (input* ((h fixnum)
           (w fixnum)
           (m fixnum)
           (tax (vector (list fixnum fixnum1 fixnum) m)))
    (let ((used-row (make-hash-table))
          (used-row-count 0)
          (used-col (make-hash-table))
          (used-col-count 0)
          (color-cells (make-hash-table)))
      (dotimes (i m)
        (let* ((ri (- m i 1))
               (op (aref tax ri))
               (ty (first op))
               ...
               (x (third op)))
          (cond ((= ty 1)
                 (unless (gethash a used-row nil)
                   (setf (gethash a used-row) t)
                   (incf used-row-count)
                   (incf (gethash x color-cells 0)
                         (- w used-col-count))))
                (t
                 (unless (gethash a used-col nil)
                   (setf (gethash a used-col) t)
                   (incf used-col-count)
                   (incf (gethash x color-cells 0)
                         (- h used-row-count)))))))
      (let ((result (hash-table-key-values color-cells)))
        (setf result (remove-if (lambda (pair) (zerop (cdr pair))) result))
        ...
        (let ((zero-count (- (* h w)
                             (reduce (lambda (acc pair)
                                     ...
                                     result
                                     :initial-value 0))))
          (unless (zerop zero-count)
            (if (zerop (caar result))
                (setf (cdar result) zero-count)
                (push (cons 0 zero-count) result))))
        (format t "~A~%" (length result))
        (dolist (pair result)
          (format t "~A ~A~%" (car pair) (cdr pair)))))))

(defun hash-table-key-values (ht)
  (let ((kv nil))
    (maphash (lambda (k v)
               (push (cons k v) kv))
             ht)
