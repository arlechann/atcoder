...
(defun solve (n s c)
  (let ((memo (make-hash-table :test 'equal))
        (s (mapcar #'char-to-int
                   (coerce s 'list)))
        (c (coerce c 'list)))
    (dp memo n s c 0 -1 nil)))

(defun flip (x) (- 1 x))

(defun dp (memo n s c index prev already)
  (when (= index n)
    (return-from dp
      (if already 0 +inf+)))
  (let ((key (list index prev already)))
    (let ((val (gethash key memo nil)))
      (when val (return-from dp val)))
    (setf (gethash key memo)
          (cond ((zerop index)
                 (min (dp memo n (cdr s) (cdr c) (1+ index) (car s) nil)
                      (+ (car c)
                         (dp memo n (cdr s) (cdr c) (1+ index) (flip (car s)) nil))))
                (already
                 (if (= (car s) prev)
                     (+ (car c)
                        (dp memo n (cdr s) (cdr c) (1+ index) (flip (car s)) t))
                     (dp memo n (cdr s) (cdr c) (1+ index) (car s) t)))
                (t
                 (if (= (car s) prev)
                     (min (dp memo n (cdr s) (cdr c) (1+ index) (car s) t)
                          (+ (car c)
                             (dp memo n (cdr s) (cdr c) (1+ index) (flip (car s)) nil)))
                     (min (dp memo n (cdr s) (cdr c) (1+ index) (car s) nil)
                          (+ (car c)
