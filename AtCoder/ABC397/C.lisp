...
(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum n)))
    (let* ((variety-from-left (aprog1 (make-array n :initial-element 0)
                                (let ((ht (make-hash-table)))
                                  (dotimes (i n)
                                    (setf (gethash (aref a i) ht) t)
                                    (setf (aref it i) (hash-table-count ht))))))
           (variety-from-right (aprog1 (make-array n :initial-element 0)
                                 (let ((ht (make-hash-table)))
                                   (loop for i from (1- n) downto 0
                                         do (setf (gethash (aref a i) ht) t)
                                            (setf (aref it i) (hash-table-count ht))))))
           (result (loop for i from 0 to (- n 2)
                         maximize (+ (aref variety-from-left i)
                                     (aref variety-from-right (1+ i))))))
      (println result))))
