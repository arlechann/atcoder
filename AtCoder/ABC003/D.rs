...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> i64 {
			...
			let area_pattern = (self.r - self.x + 1) * (self.c - self.y + 1);
			let area = self.x * self.y;
			let desk_lack_space_pattern = count_pattern(area, self.d, self.l);
			let left_empty_pattern = count_pattern(area - self.y, self.d, self.l);
			let top_empty_pattern = count_pattern(area - self.x, self.d, self.l);
			let left_and_top_empty_pattern =
				count_pattern(area - self.x - self.y + 1, self.d, self.l);
			let left_and_right_empty_pattern = count_pattern(area - self.x * 2, self.d, self.l);
			let top_and_bottom_empty_pattern = count_pattern(area - self.y * 2, self.d, self.l);
			let left_top_and_right_empty_pattern =
				count_pattern(area - self.x - self.y * 2 + 2, self.d, self.l);
			let left_top_and_bottom_empty_pattern =
				count_pattern(area - self.x * 2 - self.y + 2, self.d, self.l);
			let all_side_empty_pattern =
				count_pattern(area - self.x * 2 - self.y * 2 + 4, self.d, self.l);

			let mut pattern = desk_lack_space_pattern;
			pattern -= left_empty_pattern * 2 % MOD;
			if pattern < 0 {
				pattern += MOD;
			}
			pattern -= top_empty_pattern * 2 % MOD;
			if pattern < 0 {
				pattern += MOD;
			}
			pattern += left_and_top_empty_pattern * 4 % MOD;
			pattern %= MOD;
			pattern += left_and_right_empty_pattern;
			pattern %= MOD;
			pattern += top_and_bottom_empty_pattern;
			pattern %= MOD;
			pattern -= left_top_and_right_empty_pattern * 2 % MOD;
			if pattern < 0 {
				pattern += MOD;
			}
			pattern -= left_top_and_bottom_empty_pattern * 2 % MOD;
			if pattern < 0 {
				pattern += MOD;
			}
			pattern += all_side_empty_pattern;
			pattern %= MOD;
	...
	fn count_pattern(a: i64, d: i64, l: i64) -> i64 {
		combination(a, d) * combination(a - d, l) % MOD
	}
