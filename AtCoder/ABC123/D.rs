...
mod solve {
	...
	impl Solver {
		...
		fn dijkstra(&self) -> Vec<i64> {
			...
			let c = self.a[self.x - 1] + self.b[self.y - 1] + self.c[self.z - 1];
			pq.push((c, self.x - 1, self.y - 1, self.z - 1));
			let mut counter = 0;
			let mut ret = vec![];
			while !pq.is_empty() && counter < self.k {
				let node = pq.pop().unwrap();
				if is_used.contains(&node) {
					continue;
				}
				is_used.insert(node);
				counter += 1;
				ret.push(self.a[node.1] + self.b[node.2] + self.c[node.3]);
				{
					let mut nn = node;
					if nn.1 == 0 {
						continue;
					...
					nn.1 -= 1;
					nn.0 = self.a[nn.1] + self.b[nn.2] + self.c[nn.3];
					costs
						.entry((nn.1, nn.2, nn.3))
						.and_modify(|cost| {
							if nn.0 > *cost {
								*cost = nn.0;
								pq.push(nn);
