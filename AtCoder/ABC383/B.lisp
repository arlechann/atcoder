...
(defun manhattan-distance (i1 j1 i2 j2)
...
(defun main ()
  (input* ((h fixnum)
           ...
           (d fixnum)
           (s (vector string h)))
    (let ((office (aprog1 (make-array (list h w) :initial-element nil)
                    (dotimes (i h)
                      (dotimes (j w)
                        (let ((c (aref (aref s i) j)))
                          (setf (aref it i j) (char= c #\#))))))))
      (labels ((floorp (i j)
                 (null (aref office i j)))
               (count-humidifiered-floors (i1 j1 i2 j2)
                 (let ((grid (make-array (list h w) :initial-element nil))
                       (ret 0))
                   (dotimes (i h)
                     (dotimes (j w)
                       (when (and (floorp i j)
                                  (<= (min (manhattan-distance i1 j1 i j)
                                           (manhattan-distance i2 j2 i j))
                                      d))
