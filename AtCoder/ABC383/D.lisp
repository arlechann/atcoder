...
(defun take-while (lst predicate)
...
(defun main ()
  (input* ((n fixnum))
    (let* ((primes (coerce (primes 1000000) 'vector))
           (primes-length (length primes))
           (result 0))
      (loop for i from 0
            for prime = (aref primes i)
            while (<= (pow prime 8) n)
            do (incf result))
      (loop for i from 0
            for prime = (aref primes i)
            while (<= (* (square prime) (square prime)) n)
            do (let* ((square-prime (square prime))
                      (other-index
                        (meguru-method i primes-length
                                       #'(lambda (index)
                                           (<= (* square-prime
                                                  (square (aref primes index)))
                                               n)))))
                 (incf result (- other-index i))))
      (println result))))
            

(defun test ()
  (test-case "200" "3")
  (test-case "4000000000000" "407073")
  )
