...
(defun multi-start-bfs (graph &optional starts)
  (loop with size = (graph-size graph)
        with distances = (make-array size :initial-element +graph-cost-infinity+)
        and usedp = (make-array size :initial-element nil)
        and deque = (make-deque)
        initially (loop for start in starts
                        do (setf (aref distances start) 0
                                 (aref usedp start) t)
                           (deque-push-back deque start))
        until (deque-empty-p deque)
        do (let ((node (deque-peak-front deque)))
...
(defun main ()
  (input* ((h fixnum)
           ...
           (d fixnum)
           (s (vector string h)))
    (let* ((grid-graph (make-grid-graph h w s))
           (humidifiers (loop for i from 0 below h
                              nconc
                              (loop for j from 0 below w
                                    when (char= (aref (aref s i) j) #\H)
                                      collect (cons i j))))
           (distances (multi-start-bfs grid-graph
                                       (nmap #'(lambda (point)
                                                 (graph::grid-point-to-index
                                                  grid-graph (car point) (cdr point)))
                                             humidifiers)))
           (result (count-if #'(lambda (x) (<= x d)) distances)))
      (println result))))
