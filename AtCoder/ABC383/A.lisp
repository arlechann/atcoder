...
(defparameter *t-max* 100)

(defun main ()
  (input* ((n fixnum)
           (tv (vector (cons* fixnum1 fixnum) n)))
    (let ((events (aprog1 (make-array *t-max* :initial-element 0)
                    (loop for (time . v) across tv
                          do (setf (aref it time) v))))
          (max-t (car (aref tv (1- n))))
          (water 0))
      (loop for i to max-t
            for v = (aref events i)
            when (> water 0)
