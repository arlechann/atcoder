...
(defun solve ()
  (input* ((n fixnum)
    ...
    (let* ((cumsum (cumulate a))
           (dp (array-dp dp ((l (1+ (* n 2))) (r (1+ (* n 2))))
                 (when (= (- r l) 2)
                   (return-from dp (aref a l)))
                 (let ((center (floor (+ l r) 2)))
                   (max (if (evenp center)
                            (- (aref cumsum center) (aref cumsum l))
                            0)
                        (loop for d from (+ l 2) to (- r 2) by 2
                              maximize (+ (dp l d)
                                          (dp d r))))))))
      (println (funcall dp 0 (* n 2))))))
