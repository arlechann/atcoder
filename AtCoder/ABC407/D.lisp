...
(defun put-horizontally (h w a board r c cont)
  (declare (ignore h a))
  (let ((c2 (1+ c)))
    (when (or (= c2 w)
              (aref board r c)
              (aref board r c2))
      (return-from put-horizontally -1))
    (setf (aref board r c) t
          (aref board r c2) t)
    (prog1 (funcall cont)
      (setf (aref board r c) nil
            (aref board r c2) nil))))

(defun put-vertically (h w a board r c cont)
  (declare (ignore w a))
  (let ((r2 (1+ r)))
    (when (or (= r2 h)
              (aref board r c)
              (aref board r2 c))
      (return-from put-vertically -1))
    (setf (aref board r c) t
          (aref board r2 c) t)
    (prog1 (funcall cont)
      (setf (aref board r c) nil
...
(defun score (h w a board)
  (let ((score 0))
    (dotimes (i h)
      (dotimes (j w)
        (unless (aref board i j)
          (setf score (logxor score (aref a i j))))))
    score))

(defun dfs (h w a board r c)
  (when (= r h)
    (return-from dfs (score h w a board)))
  (when (= c w)
    (return-from dfs (dfs h w a board (1+ r) 0)))
  (max (dfs h w a board r (1+ c))
       (put-horizontally h w a board r c
                         #'(lambda () (dfs h w a board r (1+ c))))
       (put-vertically h w a board r c
                       #'(lambda () (dfs h w a board r (1+ c))))))

(defun main ()
  (input* ((h fixnum)
           (w fixnum)
           (a (vector (vector fixnum w) h)))
    (let* ((board-size (list h w))
           (a (aprog1 (make-array board-size)
                (dotimes (i h)
                  (dotimes (j w)
                    (setf (aref it i j) (aref (aref a i) j))))))
           (board (make-array board-size :initial-element nil)))
      (println (dfs h w a board 0 0)))))
