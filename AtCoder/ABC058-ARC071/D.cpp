...
int main() {
	...
	vector<mint> x_d(n - 1);
	vector<mint> y_d(m - 1);
	REP(i, n - 1) { x_d[i] = x[i + 1] - x[i]; }
	REP(i, m - 1) { y_d[i] = y[i + 1] - y[i]; }

	vector<mint> x_k(n - 1);
	vector<mint> y_k(m - 1);
	REP(i, n - 1) { x_k[i] = (i + 1) * (n - i - 1); }
	REP(i, m - 1) { y_k[i] = (i + 1) * (m - i - 1); }

	mint x_d_sum = 0;
	mint y_d_sum = 0;
	REP(i, n - 1) { x_d_sum += x_k[i] * x_d[i]; }
	REP(i, m - 1) { y_d_sum += y_k[i] * y_d[i]; }

	mint result = x_d_sum * y_d_sum;
	cout << result << endl;
	return 0;
}
