...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&mut self) -> u64 {
			...
			let mut fib = vec![1; 10];
			for i in 2..fib.len() {
				fib[i] = fib[i - 1] + fib[i - 2];
			}

			let mut dp = vec![vec![0u64; self.w]; self.h + 1];
			dp[0][0] = 1;

			for i in 0..self.h {
				for j in 0..self.w {
					if j == 0 {
						dp[i + 1][j] = dp[i][j] * fib[self.w - j - 1] % MOD
							+ dp[i][j + 1] * fib[self.w - j - 2] % MOD;
						dp[i + 1][j] %= MOD;
					} else if j == self.w - 1 {
						dp[i + 1][j] = dp[i][j] * fib[j] % MOD + dp[i][j - 1] * fib[j - 1] % MOD;
						dp[i + 1][j] %= MOD;
					} else {
						dp[i + 1][j] = dp[i][j] * fib[j] * fib[self.w - j - 1] % MOD
							+ dp[i][j + 1] * fib[j] * fib[self.w - j - 2] % MOD
							+ dp[i][j - 1] * fib[j - 1] * fib[self.w - j - 1] % MOD;
						dp[i + 1][j] %= MOD;
					}
				}
			}
