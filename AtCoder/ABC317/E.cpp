...
int bfs(const vector<string>& maze, int sy, int sx, int gy, int gx) {
	int h = maze.size();
	int w = maze[0].size();
	...
	while(!q.empty()) {
		...
		REP(i, 4) {
			...
			if(distances[ny][nx] < INF || maze[ny][nx] == '#') {
				continue;
			}
			distances[ny][nx] = d + 1;
...
int main() {
	...
	RREP(i, h) {
		RREP(j, w) {
			if(a[i][j] == '<' || a[i][j] == 'L') {
				if(j != 0 && (a[i][j - 1] == '.' || is_check(a[i][j - 1]))) {
					a[i][j - 1] = 'L';
				}
			}
		}
	...
	RREP(i, h) {
		RREP(j, w) {
			if(a[i][j] == '^' || a[i][j] == 'U') {
				if(i != 0 && (a[i - 1][j] == '.' || is_check(a[i - 1][j]))) {
					a[i - 1][j] = 'U';
				}
			}
		}
	...
	REP(i, h) {
		REP(j, w) {
			if(a[i][j] == '>' || a[i][j] == 'v' || a[i][j] == '<' ||
			   a[i][j] == '^' || is_check(a[i][j])) {
				a[i][j] = '#';
			}
			if(a[i][j] == 'S') {
				sy = i;
