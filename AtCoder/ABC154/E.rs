...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			...
			let mut dp = vec![vec![vec![0; 2]; k + 1]; len + 1];
			dp[0][0][1] = 1;
			for i in 0..len {
				for j in 0..=k {
					if j == 0 {
						dp[i + 1][0][1] = if s[i] == 0 { dp[i][0][1] } else { 0 };
						dp[i + 1][0][0] = dp[i][0][0] + if s[i] == 0 { 0 } else { dp[i][0][1] };
					} else {
						dp[i + 1][j][1] = if s[i] == 0 {
							dp[i][j][1]
						} else {
						...
						dp[i + 1][j][0] =
							dp[i][j][0]
								+ dp[i][j - 1][0] * 9 + dp[i][j - 1][1] * (s[i].saturating_sub(1))
								+ if s[i] == 0 { 0 } else { dp[i][j][1] };
					}
				}
			}
