...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&mut self) -> u64 {
			...
			self.memo = vec![vec![0; 2]; self.n];
			let result = (self.dp(&edges, 0, 0, 0) + self.dp(&edges, 0, 0, 1)) % MOD;
			// for i in 0..self.n {
			// 	println!("{:?}", self.memo[i]);
			// }
			result
		...
		pub fn dp(&mut self, edges: &[Vec<usize>], node: usize, prev: usize, color: usize) -> u64 {
			if self.memo[node][color] != 0 {
				return self.memo[node][color];
			}

			if node != prev && edges[node].len() == 1 {
				self.memo[node][color] = 1;
				return 1;
			}
			...
			for &next in edges[node].iter() {
				...
				let mut tmp = 0;
				tmp += self.dp(edges, next, node, 1);
				if color == 1 {
					tmp += self.dp(edges, next, node, 0);
				}
				tmp %= MOD;
				ret *= tmp;
				ret %= MOD;
			}
			self.memo[node][color] = ret;
			ret
		}
	}
}
