...
(defparameter *max-x* 500000)

(defun main ()
  (input* ((n fixnum)
           (lr (vector (cons* fixnum1 fixnum1) n))
           (q fixnum)
           (x (vector fixnum1 q)))
    (let ((st (make-segment-tree (1+ *max-x*) #'+ 0
                                 :initial-contents (make-list (1+ *max-x*) :initial-element 1))))
      (setf (segment-tree-ref st 0) 0)
      (dotimes (i n)
        (let* ((range (aref lr i))
               (l (car range))
               (r (cdr range))
               (l-index (meguru-method *max-x* -1
                                       (lambda (i)
                                         (>= (segment-tree-fold st 0 (1+ i))
                                             l))))
               (r-index (meguru-method *max-x* -1
                                       (lambda (i)
                                         (> (segment-tree-fold st 0 (1+ i))
                                            r)))))
          (incf (segment-tree-ref st l-index))
          (decf (segment-tree-ref st r-index))))
      (print-sequence
       (nmap (lambda (x)
               (1+ (segment-tree-fold st 0 (1+ x))))
             x)
       *standard-output*
       :spacer #\lf))))
