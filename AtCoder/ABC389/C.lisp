...
(defun main ()
  (input* ((q fixnum))
    (let ((snakes (make-deque))
          (removed-length 0))
      (dotimes (i q)
        (let ((ty (read)))
          (case ty
            (1 (let ((l (read)))
                 (if (deque-empty-p snakes)
                     (deque-push-back snakes (cons 0 l))
                     (let* ((last (deque-peak-back snakes))
                            (last-pos (car last))
                            (last-len (cdr last)))
                       (deque-push-back snakes (cons (+ last-pos last-len) l))))))
            (2 (let ((front (deque-peak-front snakes)))
                 (incf removed-length (cdr front))
                 (deque-pop-front snakes)))
            (3 (let* ((k (read))
                      (kth (deque-ref snakes (1- k))))
                 (println (- (car kth) removed-length))))))))))
