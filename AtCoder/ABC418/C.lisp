...
(defun main ()
  (input* ((n fixnum)
    ...
    (let* ((sum-a (sum a))
           (max-a (reduce #'max a))
           (b-max (min sum-a *b-max*))
           (row-rest (aprog1 (make-array (1+ b-max) :initial-element 0)
                       (let ((a-index 0))
                         (dotimes (i (1+ b-max))
                           (loop while (and (< a-index n)
                                            (>= i (aref a a-index)))
                                 do (incf a-index))
                           (setf (aref it i) (- n a-index))))))
           (cumsum (cumulate row-rest)))
      (dotimes (i q)
        (let ((b (aref b i)))
          (if (> b max-a)
              (println -1)
              (println (1+ (aref cumsum (1- b))))))))))
