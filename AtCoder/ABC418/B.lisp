  ...
  (input* ((s string))
    (let ((n (length s))
      ...
      (dotimes (i n)
        (dotimes (j n)
          (when (<= (+ i 2) j)
            (let ((tt (subseq s i (1+ j))))
              (maxf result (if (and (char= (aref s i) #\t)
                                    (char= (aref s j) #\t))
                               (coerce (/ (- (count #\t tt) 2) (- j i 1)) 'double-float)
                               0))))))
      (print-double result))))

(defun test ()
  (test-case "attitude" "0.50000000000000000")
  (test-case "ottottott" "0.66666666666666667")
  (test-case "coffeecup" "0.00000000000000000")
  )
