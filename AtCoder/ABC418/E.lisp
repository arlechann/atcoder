...
(defun choice2 (n) (values (floor (* n (1- n)) 2)))

(defun make-frac (a b) (cons a b))
(defun frac-numer (f) (car f))
(defun frac-denom (f) (cdr f))
(defun frac-invalidp (f) (zerop (frac-denom f)))
(defun frac< (a b)
  (cond ((frac-invalidp a) nil)
        ((frac-invalidp b) t)
      (t (< (* (frac-numer a) (frac-denom b))
            (* (frac-numer b) (frac-denom a))))))
(defun frac= (a b)
  (if (or (frac-invalidp a)
          (frac-invalidp b))
      (and (frac-invalidp a) (frac-invalidp b))
      (= (* (frac-numer a) (frac-denom b))
         (* (frac-numer b) (frac-denom a)))))

(defun make-point (x y) (cons x y))
(defun point-x (p) (car p))
(defun point-y (p) (cdr p))
(defun point-x= (p1 p2) (= (point-x p1) (point-x p2)))
(defun point-y= (p1 p2) (= (point-y p1) (point-y p2)))
(defun point-x< (p1 p2) (< (point-x p1) (point-x p2)))
(defun point-y< (p1 p2) (< (point-y p1) (point-y p2)))
(defun point= (p1 p2) (and (point-x= p1 p2) (point-y= p1 p2)))
(defun point< (p1 p2)
  (if (point-x= p1 p2)
      (point-y< p1 p2)
      (point-x< p1 p2)))
...
(defun gradient (p1 p2)
  (make-frac (- (point-y p2) (point-y p1))
...
(defun main ()
  (input* ((n fixnum)
    ...
    (let ((angles (loop for i from 0 below n
                        nconc (loop for j from (1+ i) below n
                                    for p1 = (aref xy i)
                                    and p2 = (aref xy j)
                                    when (point-x< p2 p1)
                                      do (rotatef p1 p2)
                                    collect (gradient p1 p2))))
          (centers (loop for i from 0 below n
                         nconc (loop for j from (1+ i) below n
                                     for p1 = (aref xy i)
                                     and p2 = (aref xy j)
                                     collect (center p1 p2)))))
      (sortf angles #'frac<)
      (sortf centers #'point<)
      (let* ((count-angles (run-length-encode angles :test #'frac=))
             (count-centers (run-length-encode centers :test #'point=))
             (result (- (loop for c in count-angles
                              sum (choice2 c))
                        (loop for c in count-centers
                              sum (choice2 c)))))
        (println result)))))
