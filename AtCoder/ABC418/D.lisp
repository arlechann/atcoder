...
(defun main ()
  (input* ((n fixnum)
           (s string))
    (declare (ignore n))
    (let* ((s (map 'vector #'char-digit s))
           (inv-s (map 'vector #'(lambda (b) (- 1 b)) s))
           (cumsum (cumulate inv-s))
           (counts (map 'vector #'(lambda (n) (if (oddp n) 1 0)) cumsum))
           (odd-count (count 1 counts))
           (even-count (count 0 counts)))
      (println (+ (choice2 odd-count)
                  (choice2 even-count))))))
