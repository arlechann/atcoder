...
int main() {
	...
	int k = 0;
	EACH(line, s) { k += count(ALL(line), '.'); }

	vector<ModInt<>> mod_pow(k + 1, 1);
	REP(i, k) { mod_pow[i + 1] = 2 * mod_pow[i]; }

	VI2D light_up(h, VI(w, 0));
	VI2D light_down(h, VI(w, 0));
	VI2D light_right(h, VI(w, 0));
	VI2D light_left(h, VI(w, 0));
	REP(i, h) {
		REP(j, w) {
			if(s[i][j] == '.') {
				if(i == 0) {
					...
					light_up[i][j] = light_up[i - 1][j] + 1;
				}
				if(j == 0) {
					light_left[i][j] = 1;
				} else {
					light_left[i][j] = light_left[i][j - 1] + 1;
				}
			}
		}
	}
		...
		for(int j = w - 1; j >= 0; j--) {
			if(s[i][j] == '.') {
				if(i == h - 1) {
					...
					light_down[i][j] = light_down[i + 1][j] + 1;
				}
				if(j == w - 1) {
					light_right[i][j] = 1;
				} else {
					light_right[i][j] = light_right[i][j + 1] + 1;
				}
			}
		}
	}
	...
	REP(i, h) {
		REP(j, w) {
			if(s[i][j] == '.') {
				light[i][j] = light_up[i][j] + light_down[i][j] +
							  light_left[i][j] + light_right[i][j] - 3;
			}
		}
	}
	...
	REP(i, h) {
		REP(j, w) {
			if(s[i][j] == '.') {
				result += mod_pow[k - light[i][j]] * (mod_pow[light[i][j]] - 1);
			}
		}
	}
