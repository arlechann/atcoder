...
ModInt<> solve() {
	...
	ModInt blue = square(n - (a - 1));
	ModInt red = square(n - (b - 1));

	ModInt intersect = square(n - (b - 1)) * square(a + b - 1 - (a - 1));
	return blue + red - intersect;
}
...
int main() {
	...
	vector<ModInt<>> results(t);
	REP(i, t) { results[i] = solve(); }
