...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			const DAYS_OF_MONTH: [usize; 13] = [0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
			let md = self
				.md
				.iter()
				.map(|s| {
					let mut it = s.split("/").map(|x| x.parse::<usize>().unwrap());
					(it.next().unwrap(), it.next().unwrap())
				})
				.collect_vec();
			let mut holidays = vec![vec![0; 32]; 13];
			for &(m, d) in md.iter() {
				holidays[m][d] += 1;
			}

			let days = (1..=12)
				.flat_map(|m| (1..=DAYS_OF_MONTH[m]).map(move |d| (m, d)))
				.enumerate()
				.map(|(i, (m, d))| {
					if i % 7 == 0 || i % 7 == 6 {
						(m, d, 1)
				...
				.map(|(m, d, c)| (m, d, c + holidays[m][d]))
				.scan(0usize, |state, (m, d, c)| {
					let ret = (m, d, *state + c);
					*state = (*state + c).saturating_sub(1usize);
					Some(ret)
				})
				.collect_vec();
			days.into_iter()
				.scan(0usize, |state, (_, _, c)| {
					*state = if c == 0 { 0 } else { *state + 1 };
					Some(*state)
				})
				.max()
				.unwrap()
