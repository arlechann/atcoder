...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> usize {
			let mut wh = self.wh.clone();
			wh.sort_by_key(|&(w, h)| (w, std::cmp::Reverse(h)));
			let h = wh.into_iter().map(|(_, h)| h).collect_vec();

			let mut dp = vec![];
			for i in 0..self.n {
				if h[i] > *dp.last().unwrap_or(&0) {
					dp.push(h[i]);
				} else {
					let j = dp.lower_bound(h[i]);
					dp[j] = h[i];
				}
			}
			dp.len()
