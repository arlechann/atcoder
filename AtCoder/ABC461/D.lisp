...
(defun main ()
  (input* ((h fixnum)
           ...
           (k fixnum)
           (s (vector string h)))
    (let* ((grid (aprog1 (make-array (list h w) :initial-element 0)
                   (dotimes (i h)
                     (dotimes (j w)
                       (setf (aref it i j)
                             (char-digit (aref (aref s i) j)))))))
           (cumsum (aprog1 (make-array (list (1+ h) (1+ w)) :initial-element 0)
                     (dotimes (i h)
                       (dotimes (j w)
                         (setf (aref it (1+ i) (1+ j))
                               (aref grid i j))))
                     (dotimes (i (1+ h))
                       (dotimes (j w)
                         (incf (aref it i (1+ j)) (aref it i j))))
                     (dotimes (i h)
                       (dotimes (j (1+ w))
                         (incf (aref it (1+ i) j) (aref it i j))))))
           )
      #?grid #?cumsum)))
