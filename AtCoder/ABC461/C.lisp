...
(defun main ()
  (input* ((n fixnum)
           (k fixnum)
           (m fixnum)
           (cv (vector (cons* fixnum1 fixnum) n)))
    (let* ((max-values (aprog1 (make-array n :initial-element (cons 0 0))
                         (loop for i from 0 below n
                               for (c . v) = (aref cv i)
                               when (> v (car (aref it c)))
                                 do (setf (aref it c)
                                          (cons v i)))))
           (picked-gems (take (sort (loop for i from 0 below n
                                          collect (cons i (aref max-values i)))
                                    #'>
                                    :key #'cadr)
                              m))
           (picked-gem-indexes-set (aprog1 (make-hash-table)
                                     (loop for (c v . i) in picked-gems
                                           do (setf (gethash i it) t))))
           (picked-value-sum (loop for (c v . i) in picked-gems
                                   sum v))
           (rest-gems (sort (loop for i from 0 below n
                                  for gem across cv
                                  unless (gethash i picked-gem-indexes-set)
                                    collect gem)
                            #'>
                            :key #'cdr))
           (result (+ picked-value-sum
                      (loop for (c . v) in (take rest-gems (- k m))
                            sum v))))
      (println result))))
