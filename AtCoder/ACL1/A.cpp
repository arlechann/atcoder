...
int main() {
	...
	REP(i, n) { points[i] = {x[i], y[i], i}; }
	sort(ALL(points));

	UnionFind uf(n);
	set<Point, comparePoint> connect_list;
	REP(i, n) {
		Point min_y = points[i];
		for(auto ittr = connect_list.upper_bound(min_y);
			ittr != connect_list.end();) {
			uf.merge(points[i].index, (*ittr).index);
			if(min_y.y > (*ittr).y) {
				min_y = *ittr;
				ittr = connect_list.erase(ittr);
			}
		}
		connect_list.insert(min_y);
	}
