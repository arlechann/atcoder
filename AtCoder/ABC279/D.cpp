...
int main() {
	...
	auto f = [&](ll x) {
		return static_cast<long double>(x) * b +
			   a / sqrt(static_cast<long double>(x) + 1);
	};
	ll x = bin_search<ll>(0, a + 1, [&](ll x) {
		long double fx = f(x - 1), fy = f(x);
		return fx > fy;
	});
	// cout << x << endl;
	cout << PRECISION(16) << f(x) << endl;
	return 0;
}
