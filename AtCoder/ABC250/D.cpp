...
// エラトステネスの篩
template <typename T>
std::vector<T> sieve_of_eratosthenes(int n) {
...
// 素数リスト
template <typename T>
std::vector<T> prime_list(int n) {
	std::vector<T> primes = sieve_of_eratosthenes<T>(n);
	primes.erase(std::remove(primes.begin(), primes.end(), 0), primes.end());
	return primes;
}

template <typename T>
bool finds(T ok, T ng) {
	return diff(ok, ng) <= 1;
}
...
int main() {
	...
	auto primes = prime_list<ll>(10000100);
	...
	RANGE(i, 1, size) {
		...
		result += bin_search<int>(0,
								  size,
								  [&](int pi) {
									  ll p = primes[pi];
									  if(is_mul_overflow(p, q3)) {
										  return false;
									  }
									  return p < q && p * q3 <= n;
								  }) +
