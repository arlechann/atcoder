...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&mut self) -> usize {
			self.memo = vec![vec![None; self.k]; self.n + 1];
			(0..self.k).map(|x| self.dp(self.n, x)).sum()
		}

		fn dp(&mut self, n: usize, x: usize) -> usize {
			if let Some(m) = self.memo[n][x] {
				return m;
			}
			...
			let ret = {
				(0..self.k)
					.map(|new_x| if new_x == x { 0 } else { self.dp(n - 1, new_x) })
					.sum()
			};
			self.memo[n][x] = Some(ret);
			ret
		}
	}
}
