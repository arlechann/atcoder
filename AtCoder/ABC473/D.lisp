...
(defun print-reverse (list &optional stream)
  (labels ((rec (list firstp)
             (when (null list)
               (return-from rec))
             (rec (cdr list) nil)
             (if firstp
...
(defun dfs (n k)
  (let ((vec (make-array n :element-type 'fixnum :initial-element 0)))
    (labels ((rec (i sum)
               (when (= sum k)
                 (print-sequence vec)
                 (return-from rec))
