...
(defun main ()
  (input* ((n fixnum)
           (k fixnum)
           (ab (vector (cons* fixnum1 fixnum1) (1- n)))
           (v (vector fixnum1 k)))
    (let* ((edges (loop with edges = (make-array n :initial-element nil)
                        for edge across ab
                        do (push (car edge) (aref edges (cdr edge)))
                           (push (cdr edge) (aref edges (car edge)))
                        finally (return edges)))
           (degrees (loop with degrees = (make-array n :initial-element 0)
                          for edge across ab
                          do (incf (aref degrees (car edge)))
                             (incf (aref degrees (cdr edge)))
                          finally (return degrees)))
           (needs (loop with needs = (make-array n :initial-element nil)
                        for node across v
                        do (setf (aref needs node) t)
                        finally (return needs)))
           (removed (make-array n :initial-element nil))
           (deque (make-deque)))
      (dotimes (i n)
        (when (and (not (aref needs i))
                   (= (aref degrees i) 1))
          (deque-push-back deque i)
          (setf (aref removed i) t)))
      (loop until (deque-empty-p deque)
            ;do (progn #?degrees #?deque)
            do (let ((node (deque-peak-front deque)))
                 (deque-pop-front deque)
                 (do-seq (neighbor (aref edges node))
                   (when (and (not (aref needs neighbor))
                              (not (aref removed neighbor)))
                     (decf (aref degrees neighbor))
                     (when (= (aref degrees neighbor) 1)
                       (deque-push-back deque neighbor)
                       (setf (aref removed neighbor) t))))))
      (let ((result (count-if #'(lambda (x) (not x)) removed)))
        (format t "~A~%" result)))))
