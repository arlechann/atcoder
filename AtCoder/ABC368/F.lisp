    ...
    (loop for i from 2
          ...
          do (let ((times 0))
               (loop while (zerop (mod n i))
                     do (setf n (floor n i))
                        (incf times))
               (when (not (zerop times))
                 (push (cons i times) insuu)))
          finally (unless (= n 1)
                    (push (cons n 1) insuu))
                  (return insuu))))
...
(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum n)))
    (map-into a #'(lambda (x)
                    (reduce #'(lambda (acc insuu)
                                (+ acc (cdr insuu)))
                            (soinsuu-bunkai x)
                            :initial-value 0))
              a)
    (let ((result (reduce #'logxor a :initial-value 0)))
      (format t "~A~%" (if (not (zerop result)) "Anna" "Bruno")))))
