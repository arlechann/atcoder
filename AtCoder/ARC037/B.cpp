...
class UnionFind {
	...
	std::vector<size_t> edge_count;

	public:
	UnionFind(size_t size)
		: parents(size), rank(size, 0), tree_size(size, 1),
		  edge_count(size, 0) {
	...
	bool merge(size_t a, size_t b) {
		...
		this->edge_count[ar] += this->edge_count[br] + 1;
	...
	vector<vector<size_t>> groups() {
		...
		vector<vector<size_t>> g(this->parents.size());
		for(size_t i = 0; i < this->parents.size(); i++) {
			g[this->parents[i]].push_back(i);
		}
	...
	size_t edge_num(size_t n) { return this->edge_count[this->root(n)]; }
...
int main() {
	...
	int result = 0;
	EACH(group, g) {
		if(uf.size(group[0]) - 1 == uf.edge_num(group[0])) {
			result++;
		}
	}
