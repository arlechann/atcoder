...
(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (uvw (vector (cons* fixnum1 fixnum1 fixnum nil) m)))
    (let* ((graph (aprog1 (make-array n :initial-element nil)
                    (dotimes (i m)
                      (let* ((edge (aref uvw i))
                             (from (first edge))
                             (to (second edge))
                             (cost (third edge)))
                        (push edge (aref it from))
                        (push (list to from (- cost)) (aref it to))))))
           (used-nodes (make-array n :initial-element nil))
           (nodes (make-array n :initial-element 0)))
      (labels ((neighbors (node) (aref graph node))
               (edge-from (edge) (first edge))
               (edge-to (edge) (second edge))
               (edge-cost (edge) (third edge))
               (use-node (node) (setf (aref used-nodes node) t))
               (used-node-p (node) (aref used-nodes node))
               (node-value (node) (aref nodes node))
               ((setf node-value) (value node) (setf (aref nodes node) value)))
        (dotimes (start n)
          (unless (used-node-p start)
            (use-node start)
            (let ((deque (make-deque)))
              (deque-push-back deque start)
              (loop (when (deque-empty-p deque)
                      (return))
                    (let ((node (prog1 (deque-peak-front deque)
                                  (deque-pop-front deque))))
                      (dolist (edge (neighbors node))
                        (let ((from (edge-from edge))
                              (to (edge-to edge))
                              (cost (edge-cost edge)))
                          (unless (prog1 (used-node-p to)
                                    (use-node to))
                            (setf (node-value to) (+ (node-value from) cost))
                            (deque-push-back deque to)))))))))
        (dotimes (i n)
          (format t "~A " (node-value i)))
        (terpri)))))
