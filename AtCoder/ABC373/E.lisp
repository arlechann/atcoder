...
(defun main ()
  (input* ((n fixnum)
           ...
           (a (vector fixnum n)))
    (let* ((rest (- k (sum a)))
           (in-top-m-p (make-array n :initial-element nil))
           (top-m1 (aprog1 (make-array (1+ m) :initial-element nil)
                     (let ((heap (make-binary-heap (lambda (a b) (> (car a) (car b))))))
                       (dotimes (i n)
                         (binary-heap-push (cons (aref a i) i) heap))
                       (dotimes (i (1+ m))
                         (let ((top (prog1 (binary-heap-top heap)
                                      (binary-heap-pop heap))))
                           (unless (= i m)
                             (setf (aref in-top-m-p (cdr top)) i))
                           (setf (aref it i) (car top)))))))
           (st (make-segment-tree (1+ m) #'+ 0 :initial-contents top-m1)))
      (dotimes (i n)
        (let ((result
                (meguru-method -1 (1+ rest)
                               (lambda (c)
                                 (let ((v (+ c (aref a i))))
                                   (if (< v (aref top-m1 (1- m)))
                                       nil
                                       (let ((le-v-index
                                               (meguru-method m -1
                                                              (lambda (i)
                                         ...
                                         (if (= le-v-index (1+ m))
                                             nil
                                             (aif (aref in-top-m-p i)
                                                  (not (> (- (+ (segment-tree-fold st le-v-index it)
                                                               (segment-tree-fold st (1+ it) (1+ m)))
                                                            (* v (- m le-v-index)))
                                                         rest))
                                                  (not (> (- (segment-tree-fold st le-v-index m)
                                                             (* v (- (1+ m) le-v-index)))
                                                          rest)))))))))))
          (format t "~A " (if (> #?result rest) -1 result)))))))
