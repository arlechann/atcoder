...
(defun main ()
  (input* ((s string))
    (let ((char-index (aprog1 (make-array (count-alphabet) :initial-element 0)
                        (dotimes (i (count-alphabet))
                          (setf (aref it (char-to-index (aref s i))) i)))))
      (format t "~A~%"
              (loop for i below (1- (count-alphabet))
                    sum (diff (aref char-index i) (aref char-index (1+ i))))))))

(defun test ()
  (test-case "ABCDEFGHIJKLMNOPQRSTUVWXYZ" "25")
  (test-case "MGJYIZDKSBHPVENFLQURTCWOAX" "223")
  )
