...
int main() {
	...
	VLL2D ml(n + 1, VLL(3, 0));
	REP(i, n) {
		ml[i + 1][0] = ml[i][0] + ((s[i] == 'M' && a[i] == 0) ? 1 : 0);
		ml[i + 1][1] = ml[i][1] + ((s[i] == 'M' && a[i] == 1) ? 1 : 0);
		ml[i + 1][2] = ml[i][2] + ((s[i] == 'M' && a[i] == 2) ? 1 : 0);
	}
	VLL2D xr(n + 1, VLL(3, 0));
	RREP(i, n) {
		xr[i][0] = xr[i + 1][0] + ((s[i] == 'X' && a[i] == 0) ? 1 : 0);
		xr[i][1] = xr[i + 1][1] + ((s[i] == 'X' && a[i] == 1) ? 1 : 0);
		xr[i][2] = xr[i + 1][2] + ((s[i] == 'X' && a[i] == 2) ? 1 : 0);
	}
	...
	REP(i, n) {
		...
		if(a[i] == 0) {
			result += (ml[i][0] * xr[i + 1][0]) + (ml[i][0] * xr[i + 1][2]) +
					  (ml[i][2] * xr[i + 1][0]) + (ml[i][2] * xr[i + 1][2]);
			result += ((ml[i][0] * xr[i + 1][1]) + (ml[i][1] * xr[i + 1][0]) +
					   (ml[i][1] * xr[i + 1][1])) *
					  2;
			result +=
				((ml[i][1] * xr[i + 1][2]) + (ml[i][2] * xr[i + 1][1])) * 3;
		} else if(a[i] == 1) {
			result += ((ml[i][0] * xr[i + 1][0]) + (ml[i][0] * xr[i + 1][1]) +
					   (ml[i][1] * xr[i + 1][0])) *
					  2;
			result +=
				((ml[i][0] * xr[i + 1][2]) + (ml[i][2] * xr[i + 1][0])) * 3;
		} else {
			result += (ml[i][0] * xr[i + 1][0]) + (ml[i][0] * xr[i + 1][2]) +
					  (ml[i][2] * xr[i + 1][0]);
			result +=
				((ml[i][0] * xr[i + 1][1]) + (ml[i][1] * xr[i + 1][0])) * 3;
		}
	}
