...
(defun (setf a2ref) (v a i j)
  (setf (aref (aref a i) j) v))

(defparameter *inf* 1000000000)

(defun solve (n m k a b)
  (let* ((food-days (aprog1 (make-array n)
                      (dotimes (i n)
                        (setf (aref it (aref b i)) i))))
         (latest-eat-days (aprog1 (make-array m :initial-element 0)
                            (dotimes (i m)
                              (dotimes (j (aref k i))
                                (maxf (aref it i)
                                      (aref food-days (a2ref a i j)))))))
         (result (aprog1 (make-array n :initial-element 0)
                   (dotimes (i m)
                     (incf (aref it (aref latest-eat-days i))))
                   (dotimes (i (1- n))
                     (incf (aref it (1+ i)) (aref it i))))))
...
(defun main ()
  (input* ((n fixnum)
    ...
    (let* ((k (make-array m))
      ...
      (dotimes (i m)
        (input* ((ki fixnum)
                 (ai (vector fixnum1 ki)))
          (setf (aref k i) ki
                (aref a i) ai)))
      (input* ((b (vector fixnum1 n)))
        (solve n m k a b)))))
