...
(defun main ()
  (input* ((n fixnum)
           (q fixnum))
    (let* ((pigeons (aprog1 (make-array n)
                      (dotimes (i n)
                        (setf (aref it i) i))))
           (holes (make-array n :initial-element 1))
           (count 0))
      (dotimes (k q)
        (let* ((type (read))
               (p (when (= type 1) (1- (read))))
               (h (when (= type 1) (1- (read)))))
          (if (= type 1)
              (progn
                (when (= (aref holes (aref pigeons p)) 2)
                  (decf count))
                (decf (aref holes (aref pigeons p)))
                (incf (aref holes h))
                (setf (aref pigeons p) h)
                (when (= (aref holes h) 2)
                  (incf count)))
              (println count)))))))
