...
(defun main ()
  (input* ((n fixnum)
           (w fixnum)
           (xy (vector (cons* fixnum1 fixnum1) n))
           (q fixnum)
           (ta (vector (cons* fixnum fixnum1) q)))
    (let* ((columns (aprog1 (make-array w)
                      (dotimes (i w)
                        (setf (aref it i) (dvector)))
                      (dotimes (i n)
                        (let* ((block (aref xy i))
                               (x (car block)))
                          (vector-push-extend (cons i block) (aref it x))))
                      (dotimes (i w)
                        (sortf (aref it i)
                               #'(lambda (a b)
                                   (< (cddr a) (cddr b)))))))
           (block-rows (aprog1 (make-array n)
                         (dotimes (i w)
                           (let ((column (aref columns i)))
                             (dotimes (j (length column))
                               (let ((block (aref column j)))
                                 (setf (aref it (car block)) j)))))))
           (delete-rows (loop for i below w
                              minimize (length (aref columns i))))
           (row-deleted-turns (aprog1 (make-array delete-rows)
                               (dotimes (k delete-rows)
                                 (setf (aref it k)
                                       (loop for i below w
                                             maximize (1+ (cddr (aref (aref columns i) k)))))))))
      ;;#?columns #?block-rows #?delete-rows #?row-deleted-turns
      (dotimes (k q)
        (let* ((query (aref ta k))
               (time (car query))
               (a (cdr query))
               (block-row (aref block-rows a)))
          ;;#?time #?a #?block-row
          (if (or (>= block-row delete-rows)
                  (< time (aref row-deleted-turns block-row)))
              (println "Yes")
              (println "No")))))))
