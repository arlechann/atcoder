...
(defun main ()
  (input* ((n fixnum)
           (a string))
    (let* ((dp2 (array-dp dp2 ((index (expt 3 n)) (hierarchy (1+ n)) (change-to 2))
                  (when (= hierarchy 0)
                    (return-from dp2
                      (if (= change-to (char-digit (aref a index)))
                          0
                          1)))
                  (let ((jump (expt 3 (1- hierarchy)))
                        (sum-costs 0)
                        (max-cost 0))
                    (loop for i below 3
                          do (let ((cost (dp2 (+ index (* i jump))
                                              (1- hierarchy)
                                              change-to)))
                               (incf sum-costs cost)
                               (maxf max-cost cost)))
                    (- sum-costs max-cost)))))
      (println (max (funcall dp2 0 n 0)
                    (funcall dp2 0 n 1))))))
