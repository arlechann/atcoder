...
int main() {
	...
	vector<unordered_set<int>> unconnectable(n);
	REP(i, k) {
		if(uf.is_same(x[i], y[i])) {
			is_already_bad = true;
		}
		int xroot = uf.root(x[i]);
		int yroot = uf.root(y[i]);
		unconnectable[xroot].insert(yroot);
		unconnectable[yroot].insert(xroot);
		// cout << "X " << x[i] << " - " << y[i] << " => " << xroot << " - "
		// 	 << yroot << endl;
	}
	...
	REP(i, Q) {
		...
		if(uf.is_same(p[i], q[i])) {
			cout << "Yes" << endl;
			continue;
		}
		int proot = uf.root(p[i]);
		int qroot = uf.root(q[i]);
		// cout << "Q " << p[i] << " - " << q[i] << " => " << proot << " - "
		// 	 << qroot << endl;
		if(unconnectable[proot].find(qroot) != unconnectable[proot].end()) {
			cout << "No" << endl;
		} else {
			cout << "Yes" << endl;
		}
