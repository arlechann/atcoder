	...
	impl Solver {
		...
		pub fn solve(&self) -> String {
			let se: Vec<(usize, usize)> = self
				...
				.map(|(s, e)| ((s / 5) * 5, ((e + 4) / 5) * 5))
				.map(|(s, e)| {
					let ns = if s % 100 == 60 { s + 100 - 60 } else { s };
					let ne = if e % 100 == 60 { e + 100 - 60 } else { e };
					(ns, ne)
				})
				.collect_vec();
			let mut imos = Imos::with_capacity(2410);
			for &(s, e) in se.iter() {
				imos.query(s, e + 1, 1);
			}
			let v = imos.apply();
			...
			results
				.into_iter()
				.map(|(s, e)| format!("{:04}", s) + "-" + &format!("{:04}", e))
				.collect_vec()
				.join("\n")
		}
	}
