...
constexpr long long LL_INF = 1e18;
...
int main() {
	...
	sort(ALL(h));
	VLL cumsum_left(n / 2 + 1, 0);
	VLL cumsum_right(n / 2 + 1, 0);
	REP(i, n / 2) {
		cumsum_left[i + 1] = cumsum_left[i] + (h[i * 2 + 1] - h[i * 2]);
	}
	for(int i = n / 2; i > 0; i--) {
		cumsum_right[i - 1] = cumsum_right[i] + (h[i * 2] - h[i * 2 - 1]);
	}
