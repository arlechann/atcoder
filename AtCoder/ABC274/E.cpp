...
int is_used[1 << 18][18];
double memo[1 << 18][18];
double dp(int visited, int now) {
	double& ret = memo[visited][now];
	if(is_used[visited][now] != 0) {
		return ret;
	}
	...
	REP(i, n + m + 1) {
		if((visited & (1 << i)) == 0) {
			chmin(ret, dp(visited | (1 << i), i) + distances[now][i] / speed);
		}
	}
	return ret;
}
...
int main() {
	...
	distances =
		vector<vector<double>>(n + m + 1, vector<double>(n + m + 1, INFLL));
	REP(i, n) {
		distances[0][m + i + 1] = distances[m + i + 1][0] = hypot(x[i], y[i]);
		REP(j, n) {
			if(i == j) {
				continue;
			}
			...
			distances[m + i + 1][m + j + 1] = distances[m + j + 1][m + i + 1] =
				d;
		}
	}

	REP(i, m) {
		distances[0][i + 1] = distances[i + 1][0] = hypot(p[i], q[i]);
		REP(j, m) {
			if(i == j) {
				continue;
			}
	...
	REP(i, n) {
		REP(j, m) {
			...
			ll dy = y[i] - q[j];
			double d = hypot(dx, dy);
			distances[m + i + 1][j + 1] = distances[j + 1][m + i + 1] = d;
		}
	}
