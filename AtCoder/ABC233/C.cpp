...
bool is_mul_overflow(ll a, ll b) {
	return b > x / a;
}

ll dfs(const VI& l, const VI2D& a, int bag, ll pro) {
	if(bag == n) {
		return static_cast<int>(pro == x);
	}

	int size = l[bag];
	ll result = 0;
	REP(i, size) {
		if(!is_mul_overflow(pro, a[bag][i])) {
			result += dfs(l, a, bag + 1, pro * a[bag][i]);
		}
	}
	return result;
}
