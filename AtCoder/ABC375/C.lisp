...
(defun rotate0 (size y x)
  (values (- size x 1) y))

(defun rotate1 (size y x)
  (multiple-value-call #'rotate0 size (rotate0 size y x)))

(defun rotate2 (size y x)
  (multiple-value-call #'rotate0 size (rotate1 size y x)))

(defun rotate3 (size y x)
  (declare (ignore size))
  (values y x))

(defun solve (size grid result level)
  (when (>= (* level 2) size)
    (return-from solve))
  (let ((rotate-fn (case (mod level 4)
                     (0 #'rotate0)
                     (1 #'rotate1)
                     (2 #'rotate2)
                     (3 #'rotate3))))
    (dotimes (i (- size (* level 2)))
      (let ((i (+ i level))
            (left level)
            (right (- size level 1)))
        (multiple-value-bind (y x) (funcall rotate-fn size i left)
          (setf (aref result i left)
                (aref grid y x)))
        (multiple-value-bind (y x) (funcall rotate-fn size i right)
          (setf (aref result i right)
                (aref grid y x)))))
    (dotimes (j (- size (* level 2)))
      (let ((j (+ j level))
            (top level)
            (bottom (- size level 1)))
        (multiple-value-bind (y x) (funcall rotate-fn size top j)
          (setf (aref result top j)
                (aref grid y x)))
        (multiple-value-bind (y x) (funcall rotate-fn size bottom j)
          (setf (aref result bottom j)
                (aref grid y x)))))
    (solve size grid result (1+ level))))

(defun main ()
  (input* ((n fixnum)
           (a (vector string n)))
    (let ((grid (aprog1 (make-array (list n n) :initial-element 0)
                  (dotimes (i n)
                    (dotimes (j n)
                      (when (char= (aref (aref a i) j) #\#)
                        (setf (aref it i j) 1))))))
          (result (make-array (list n n) :initial-element 0)))
      (solve n grid result 0)
      (dotimes (i n)
        (dotimes (j n)
          (write-char (if (zerop (aref result i j))
                          #\. #\#)))
        (terpri)))))
