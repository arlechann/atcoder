...
(defun index-to-upper (index)
  (code-char (+ index #.(char-code #\A))))

(defun index-to-lower (index)
  (code-char (+ index #.(char-code #\a))))
...
(defun count-chars (string)
  (aprog1 (make-array (count-alphabet) :initial-element 0)
    (do-seq (c string)
      (incf (aref it (char-to-index c))))))

(defun run-length (string)
  (let ((n (length string)))
...
(defun main ()
  (input* ((s string))
    (let ((n (length s))
          (char-count (make-array (count-alphabet) :initial-element 0))
          (char-last-index (make-array (count-alphabet) :initial-element nil))
          (result 0))
      (labels ((last-index (c)
                 (aref char-last-index (char-to-index c)))
               ((setf last-index) (value c)
                 (setf (aref char-last-index (char-to-index c)) value))
               (found-char-count (c)
                 (aref char-count (char-to-index c)))
               ((setf found-char-count) (value c)
                 (setf (aref char-count (char-to-index c)) value)))
        (dotimes (i n)
          (let ((c (aref s i)))
            (when-let ((last (last-index c)))
              (let ((cc (found-char-count c))
                    (dist (diff last i)))
                (incf result (* (1- cc) dist))
                (incf result (1- dist))))
            (setf (last-index c) i)
            (incf (found-char-count c))))
        (format t "~A~%" result)))))

(defun test ()
  (test-case "ABCACC" "5")
  (test-case "OOOOOOOO" "56")
  (test-case "XYYXYYXYXXX" "75")
  )
