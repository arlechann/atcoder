...
ll check(const int n, const vector<string>& a, int y, int x) {
	ll ret = 0;
	// right
	string right;
	REP(j, n) {
	...
	string right_down;
	REP(i, n) {
		right_down += a[(y + i) % n][(x + i) % n];
	}
	chmax(ret, stoll(right_down));

	// right up
	string right_up;
	REP(i, n) {
		right_up += a[(y - i + n) % n][(x + i) % n];
	}
	chmax(ret, stoll(right_up));

	// left down
	string left_down;
	REP(i, n) {
		left_down += a[(y + i) % n][(x - i + n) % n];
	}
	chmax(ret, stoll(left_down));

	// left up
	string left_up;
	REP(i, n) {
		left_up += a[(y - i + n) % n][(x - i + n) % n];
	}
	chmax(ret, stoll(left_up));
	return ret;
}
