...
(defun main ()
  (input* ((n fixnum)
           (d (vector fixnum n)))
    (nlet rec ((month 1) (day 1) (result 0))
      (if (> month n)
          (format t "~A~%" result)
          (let* ((str (concatenate 'string
            ...
            (nlet rec2 ((i 0))
              (if (= i len)
                  (if (= day (aref d (1- month)))
                      (rec (1+ month) 1 (1+ result))
                      (rec month (1+ day) (1+ result)))
                  (if (char= (aref str i) c)
                      (rec2 (1+ i))
                      (if (= day (aref d (1- month)))
                          (rec (1+ month) 1 result)
                          (rec month (1+ day) result))))))))))
