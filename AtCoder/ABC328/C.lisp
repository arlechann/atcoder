...
(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (s string)
           (lr (vector (list fixnum1 fixnum1) q)))
    (let ((l (map 'vector #'car lr))
          (r (map 'vector #'cadr lr)))
      (let ((nei (make-array (1- n) :initial-element 0))
            (cumsum (make-array n :initial-element 0)))
        (dotimes (i (1- n))
          (setf (aref nei i) (if (char= (aref s i)
                                        (aref s (1+ i)))
                                 1
                                 0)))
        (dotimes (i (1- n))
          (setf (aref cumsum (1+ i)) (+ (aref cumsum i)
                                        (aref nei i))))
        ;; s      mississippi
        ;; nei    0010010010
        ;; cumsum 00011122233
