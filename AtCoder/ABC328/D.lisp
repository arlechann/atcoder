...
(defun main ()
  (input* ((s string))
    (let ((len (length s)))
    (nlet rec ((i 0) (now 0) (stack nil))
      ;;(format t "stack: ~S~%" stack)
      (if (= i len)
          (format t "~A~%" (concatenate 'string
                                        (nreverse (mapcar #'car stack))))
          (let ((c (aref s i)))
            (cond ((char= c #\A)
                   (rec (1+ i) 1 (cons (cons c 1) stack)))
                  ((and (= now 1) (char= c #\B))
                   (rec (1+ i) (1+ now) (cons (cons c (1+ now)) stack)))
                  ((and (= now 2) (char= c #\C))
                   (let* ((stack (cddr stack))
                          (top-now (or (cdar stack) 0)))
                     (rec (1+ i) top-now stack)))
                  (t (rec (1+ i) 0 (cons (cons c 0) stack))))))))))
