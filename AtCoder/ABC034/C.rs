...
mod solve {
	...
	impl Solver {
		...
		pub fn solve(&self) -> u64 {
			let mod_comb = ModComb::new();
			mod_comb.call(self.w + self.h - 2, self.h - 1)
		}
	}

	const COMB_MAX: usize = 200000;

	struct ModComb {
		fact: [u64; COMB_MAX],
		fact_inv: [u64; COMB_MAX],
		inv: [u64; COMB_MAX],
	}
	...
	impl ModComb {
		...
		fn init(&mut self) {
			...
			for i in 2..COMB_MAX {
				self.fact[i] = self.fact[i - 1] * i as u64 % MOD;
				self.inv[i] = MOD - (self.inv[MOD as usize % i] * (MOD / i as u64)) % MOD;
				self.fact_inv[i] = self.fact_inv[i - 1] * self.inv[i] % MOD;
			}
		}

		fn call(&self, n: usize, r: usize) -> u64 {
			if n < r {
				0
			} else {
