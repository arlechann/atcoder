...
int main() {
	...
	sort(ALL(d));
	vector<mint> a(n, 0);
	vector<SegmentTree<mint>> dp(5, SegmentTree(a));
	REP(i, n) { dp[1].update(i, 1); }
	RANGE(k, 2, 5) {
		REP(i, n) {
			int x = bin_search<int>(
				n - 1, -1, [&](int x) { return d[x] * 2 > d[i]; });
			dp[k].update(i, dp[k - 1].find(0, x));
		}
	}

	cout << dp[4].find(0, n) << endl;
	return 0;
}
