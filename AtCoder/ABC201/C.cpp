...
bool check(const string& s, string& num) {
	VI count_used(10, 0);
	REP(i, 4) { count_used[num[i] - '0']++; }
	REP(i, S_SIZE) {
		if((s[i] == 'o' && count_used[i] == 0) ||
		   (s[i] == 'x' && count_used[i] != 0)) {
			return false;
		}
	}
	return true;
...
int main() {
	...
	REP(i, 10000) {
		...
		int zero = 4 - num.size();
		num = string(zero, '0') + num;
		result += static_cast<int>(check(s, num));
	}
