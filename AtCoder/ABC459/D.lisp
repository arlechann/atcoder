...
(defun solve ()
  (input* ((s string))
    (sortf s #'char<)
    (let ((heap (make-binary-heap #'(lambda (a b)
                                      (> (cdr a) (cdr b))))))
      (mapc #'(lambda (x)
                (binary-heap-push x heap))
            (run-length-encode s))
      (loop with result = nil
            and first = (prog1 (binary-heap-top heap)
                           (binary-heap-pop heap))
            and second = (if (binary-heap-empty-p heap)
                             nil
                             (prog1 (binary-heap-top heap)
                               (binary-heap-pop heap)))
            ;; do #?(list first second)
               ;; #?result
            do (push (car first) result)
               (decf (cdr first))
               (when (and (zerop (cdr first)) (null second))
                 (println "Yes")
                 (println (coerce (nreverse result) 'string))
                 (return-from solve))
               (when (null second)
                 (println "No")
                 (return-from solve))
               (unless (zerop (cdr first))
                 (binary-heap-push first heap))
               (setf first second)
               (if (binary-heap-empty-p heap)
                   (setf second nil)
                   (setf second (prog1 (binary-heap-top heap)
                                  (binary-heap-pop heap))))))))
