...
(defparameter *y-max* 300000)

(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (queries (vector (cons fixnum fixnum) q)))
    (let ((cells (make-array n :initial-element 0))
          (st (make-segment-tree (1+ (* *y-max* 2)) #'+ 0))
          (drop-count 0))
      (setf (segment-tree-ref st 0) n)
      (loop for (type . xy) across queries
            for x = (1- xy)
            and y = xy
            if (= type 1)
              do (decf (segment-tree-ref st (aref cells x)))
                 (incf (aref cells x))
                 (incf (segment-tree-ref st (aref cells x)))
                 (when (zerop (segment-tree-fold st 0 (1+ drop-count)))
                   (incf drop-count))
            else
              do (println (segment-tree-fold st (+ y drop-count) (1+ q)))))))
