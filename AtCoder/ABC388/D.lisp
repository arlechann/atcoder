...
(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum n)))
    (let ((imos (aprog1 (make-array n :initial-element 0)
                  (setf (aref it 0) (aref a 0))
                  (loop for i from 1 below n
                        do (setf (aref it i) (- (aref a i)
                                                (aref a (1- i))))))))
      (loop for i from (1- n) downto 0
            do (incf (aref imos i) i)
               (unless (= (1+ i) n)
                 (decf (aref imos (1+ i)) i))
            do (decf (aref imos 0))
               (incf (aref imos i)))
      (loop for i from 0 below n
            unless (zerop i)
              do (incf (aref imos i) (aref imos (1- i)))
            when (< (aref imos i) 0)
              do (decf (aref imos (+ n (aref imos i)))))
      (loop for i from 0 below n
            when (< (aref imos i) 0)
              do (setf (aref imos i) 0))
      (print-sequence imos))))
