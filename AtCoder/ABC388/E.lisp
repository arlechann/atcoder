...
(defun main ()
  (input* ((n fixnum)
           (a (vector fixnum n)))
    (let* ((pair-indeces (aprog1 (make-array n :initial-element n)
                           (dotimes (i (ceiling n 2))
                             (setf (aref it i)
                                   (lower-bound a (* (aref a i) 2) :start (ceiling n 2))))))
           (base-count (aprog1 (make-array n :initial-element 0)
                         (dotimes (i (ceiling n 2))
                           (unless (= (aref pair-indeces i) n)
                             (incf (aref it (aref pair-indeces i)))))))
           (result 0))
      ;#?pair-indeces #?base-count
      (loop with no-paired = 0
            for i from (1- n) downto (ceiling n 2)
            for count = (aref base-count i)
            ;do #?i #?no-paired
            do (incf no-paired)
               (incf result (min count no-paired))
               (setf no-paired (max (- no-paired count) 0)))
      (println result))))
...
;; 307 321 330 339 349 392 422 430 477 481 488 537
;; 541 571 575 602 614 660 669 678 712 723 785 792
;;   0   0   0   0   1   2   0   1   1   0   1   0
