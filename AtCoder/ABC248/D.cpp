...
int main() {
	...
	vector<tuple<int, int, int>> left(q);
	REP(i, q) { left[i] = make_tuple(l[i], x[i], i); }
	sort(ALL(left));
	vector<tuple<int, int, int>> right(q);
	REP(i, q) { right[i] = make_tuple(r[i], x[i], i); }
	sort(ALL(right));
	...
	VI left_count(q);
	VI count(n, 0);
	while(get<0>(left[li]) == 0) {
		left_count[get<2>(left[li])] = 0;
		li++;
	}
	REP(i, n) {
		count[a[i]]++;
		while(get<0>(left[li]) == i + 1) {
			left_count[get<2>(left[li])] = count[get<1>(left[li])];
			li++;
		}
		while(get<0>(right[ri]) == i + 1) {
			result[get<2>(right[ri])] =
				count[get<1>(right[ri])] - left_count[get<2>(right[ri])];
			ri++;
		}
	}
