...
(defun main ()
  (input* ((n fixnum)
           ...
           (ab (vector (cons* fixnum1 fixnum1) q)))
    (let* ((xy-with-quadrant (map 'vector
                                  #'(lambda (xy)
                                      (let* ((x (car xy))
                                             (y (cdr xy))
                                             (quadrant (if (or (> y 0)
                                        ...
                                        (cons quadrant xy)))
                                  xy))
           (monsters (sort (copy-seq xy-with-quadrant)
                           #'(lambda (a b)
                               (let* ((a-q (car a))
                                      (a-x (cadr a))
                                      (a-y (cddr a))
      ...
      (dotimes (i q)
        (let* ((ab (aref ab i))
               (a (car ab))
               (b (cdr ab))
               (a-qxy (aref xy-with-quadrant a))
               (b-qxy (aref xy-with-quadrant b))
               (a-q (car a-qxy))
               (a-x (cadr a-qxy))
               (a-y (cddr a-qxy))
               (b-q (car b-qxy))
               ...
               (b-y (cddr b-qxy))
               (a-q (if (/= a-q b-q)
                        (if (> a-q b-q) a-q (+ a-q 2))
                        (if (< (* a-y b-x) (* a-x b-y))
                            (+ a-q 2)
                            a-q)))
               (a-qxy (cons a-q (cdr a-qxy)))
               (b-index (algorithm::find-bound monsters b-qxy
                                               :compare 
                                               #'(lambda (a b)
                                                   (let* ((a-q (car a))
                                                          (a-x (cadr a))
                                                     ...
                                                     (if (/= a-q b-q)
                                                         ...
                                                         (<= (* a-y b-x) (* a-x b-y)))))
                                               :end n))
               (a-index (algorithm::find-bound monsters a-qxy
                                               :compare
                                               #'(lambda (a b)
                                                   (let* ((a-q (car a))
                                                          (a-x (cadr a))
