...
(defun main ()
  (input* ((n fixnum)
           (q fixnum)
           (a (vector fixnum n)))
    (let ((st (make-segment-tree n #'+ 0 :initial-contents (coerce a 'list))))
      (dotimes (_ q)
        (let ((type (read)))
          (if (= type 1)
              (let* ((x (1- (read)))
                     (ax (aref a x))
                     (ax+1 (aref a (1+ x))))
                (setf (segment-tree-ref st x) ax+1
                      (segment-tree-ref st (1+ x)) ax)
                (rotatef (aref a x) (aref a (1+ x))))
              (let ((l (1- (read)))
                    (r (read)))
                (println (segment-tree-fold st l r)))))))))
