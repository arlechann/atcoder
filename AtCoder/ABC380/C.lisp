...
(defun main ()
  (input* ((n fixnum)
           ...
           (s string))
    (declare (ignore n))
    (let ((ss (run-length-encode s)))
      (loop with one-count = 0
            with next = nil
            for i from 0
            for chunks on ss
            when (char= (caar chunks) #\1)
              do (incf one-count)
            if next
              do (dotimes (i (cdr next))
                   (write-char (car next)))
                 (setf next nil)
            else if (and (= one-count (1- k))
                         (char= (caar chunks) #\0))
              do (dotimes (i (cdadr chunks))
                   (write-char (caadr chunks)))
                 (setf next (car chunks))
            else
              do (dotimes (i (cdar chunks))
                   (write-char (caar chunks)))
            finally (terpri)))))
