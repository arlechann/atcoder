...
(defun main ()
  (input* ((s string)
    ...
    (let* ((n (length s)))
      (dotimes (i q)
        (let* ((index (aref k i))
               (bin-index (floor index n))
               (swap-count (logcount bin-index)))
          (format t "~A " (if (oddp swap-count)
                              (swap-case (aref s (mod index n)))
                              (aref s (mod index n))))))
      (terpri))))
...
" "a B A b A b a B A b a B a B A b")
  (test-case "qWeRtYuIoP
8
1 1 2 3 5 8 13 21
" "q q W e t I E Q")
  (test-case "AnUoHrjhgfLMcDIpzxXmEWPwBZvbKqQuiJTtFSlkNGVReOYCdsay
5
