...
(defun digit-to-number (c)
  (- (char-code c) #.(char-code #\0)))

(defun main ()
  (input* ((s string))
    (let* ((s (map 'list #'digit-to-number s))
           (rle (run-length-encode s)))
      (loop with result = 0
            for lst on rle
            for first = (car lst)
            for second = (cadr lst)
            until (null second)
            when (= (1+ (car first)) (car second))
              do (incf result (min (cdr first) (cdr second)))
...
(defun test ()
  ...
  (test-case "7788788" "3")
  (test-case "2025" "0")
  (test-case "1112222334445556555" "11")
  )
