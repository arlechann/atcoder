...
(defun solve (n m x y)
  (let* ((size (* n m))
         (inf *inf*)
         (-inf (- *inf*))
         (result (make-array (list n m) :initial-element inf))
         (fixed (make-array (list n m) :initial-element nil))
         (st (make-segment-tree size #'max -inf
                                :initial-contents (iota size))))
    (flet ((set-use-value (v)
             (setf (segment-tree-ref st v) -inf)
             t)
           (get-unused-value (max)
             (segment-tree-fold st 0 max)))
      (dotimes (i n)
        (dotimes (j m)
          (minf (aref result i j) (aref x i))
          (minf (aref result i j) (aref y j))))
      result)))
