...
(defun upper-bound (vector element &key (compare #'<) (start 0) end)
  (find-bound vector element :compare compare :start start :end (or end (length vector))))
...
(defun main ()
  (input* ((n fixnum)
           (m fixnum)
           (a (vector fixnum n)))
    (let* ((digits (map 'vector #'count-digit a))
           (exps (aprog1 (make-array 11 :initial-element 1)
                   (dotimes (d 11)
                     (unless (zerop d)
                       (setf (aref it d)
                             (* (aref it (1- d)) 10))))))
           (exp-mod-a (aprog1 (make-array 11)
                        (dotimes (d 11)
                          (setf (aref it d)
                                (make-array n :initial-element 0))
                          (dotimes (i n)
                            (setf (aref (aref it d) i)
                                  (mod (* (aref a i)
                                          (aref exps d))
                                       m)))
                          (sortf (aref it d) #'<))))
           (result 0))
      ;#?digits #?exps #?exp-mod-a
      (dotimes (i n)
        (let* ((d (aref digits i))
               (rest (mod (- m (mod (aref a i) m)) m)))
          ;#?d #?rest
          (incf result
                (- (upper-bound (aref exp-mod-a d) rest)
                   (lower-bound (aref exp-mod-a d) rest)))))
      (println result))))
