...
// 計算量 前処理:O(n) クエリ:O(1)
template <long long MOD = 1000000007, long long COMB_MAX = 100000>
class mod_comb {
...
int main() {
	...
	int n = s.size();
	VI alphabet_count(ALPHABET_SIZE, 0);
	REP(i, n) { alphabet_count[s[i] - 'a']++; }
	...
	auto result = mint(0);
	REP(i, n) { result += dp[ALPHABET_SIZE][i + 1]; }
	cout << result << endl;
	return 0;
}
